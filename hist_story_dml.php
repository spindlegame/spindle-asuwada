...
function hist_story_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	$res = sql("SELECT `id` FROM `hist_story` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_encounter` WHERE `hist_story`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
...
function hist_story_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		$combo_tags->SelectedText = ( $_REQUEST['FilterField'][1] == '10' && $_REQUEST['FilterOperator'][1] == '<=>' ? $_REQUEST['FilterValue'][1] : '');
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#hist_author_id-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_team: filterer_team, id: AppGini.current_hist_author_id__RAND__.value, t: 'hist_story', f: 'hist_author_id' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_team: filterer_team, s: term, p: page, t: 'hist_story', f: 'hist_author_id' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#hist_author_id-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_hist_author_id__RAND__.value, t: 'hist_story', f: 'hist_author_id' },
					success: function(resp) {
						$j('[name="hist_author_id"]').val(resp.results[0].id);
						$j('[id=hist_author_id-container-readonly__RAND__]').html('<span id="hist_author_id-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=hist_author_view_parent]').hide(); } else { $j('.btn[id=hist_author_view_parent]').show(); }
	...
	$lookup_fields = array('team' => array('hist_team', 'Equipo'), 'hist_author_id' => array('hist_author', 'ID histori&#243;grafo'), 'community_id' => array('hist_community', 'Comunidad'), 'genre' => array('class_bibliography_genre', 'G&#233;nero literario'), 'collaboration_status' => array('class_story_collab_type', 'Nivel de colaboraci&#243;n'), 'language' => array('class_language', 'Idioma'), );
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'hist_story_autofill.php?rnd1=$rnd1&mfk=hist_author_id&id=' + encodeURIComponent('".addslashes($row['hist_author_id'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'hist_story_autofill.php?rnd1=$rnd1&mfk=hist_author_id&id=' + encodeURIComponent(AppGini.current_hist_author_id{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
