...
// Data functions (insert, update, delete, form) for table game_agent

// This script and data application were generated by AppGini 5.94
// Download AppGini for free from https://bigprof.com/appgini/download/

function game_agent_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('game_agent');
	...
	$data = [
		...
		'img' => Request::fileUpload('img', [
			...
			'success' => function($name, $selected_id) {
				createThumbnail($name, getThumbnailSpecs('game_agent', 'img', 'tv'));
				createThumbnail($name, getThumbnailSpecs('game_agent', 'img', 'dv'));
			},
			'failure' => function($selected_id, $fileRemoved) {
				if(!isset($_REQUEST['SelectedID'])) return '';

				/* for empty upload fields, when saving a copy of an existing record, copy the original upload field */
				return existing_value('game_agent', 'img', $_REQUEST['SelectedID']);
			},
		]),
		'groupID' => Request::val('groupID', ''),
		'selection_class' => Request::val('selection_class', ''),
		'agenttype1' => Request::val('agenttype1', ''),
		'agenttype2' => Request::val('agenttype2', ''),
		'gender' => Request::val('gender', ''),
		'last_name' => Request::val('last_name', ''),
		'first_name' => Request::val('first_name', ''),
		'other_name' => Request::val('other_name', ''),
		'titles' => Request::val('titles', ''),
		'titles_academic' => Request::val('titles_academic', ''),
		'titles_nobility' => br2nl(Request::val('titles_nobility', '')),
		'birthday' => mysql_datetime(Request::val('birthday', '')),
		'birth_location' => Request::val('birth_location', ''),
		'birth_location_map' => Request::val('birth_location_map', ''),
		'deathday' => mysql_datetime(Request::val('deathday', '')),
		'death_location' => Request::val('death_location', ''),
		'workplace' => Request::val('workplace', ''),
		'knows' => Request::val('knows', ''),
		'shortbio' => br2nl(Request::val('shortbio', '')),
		'img_gallery' => Request::val('img_gallery', ''),
		'data_evaluation' => Request::val('data_evaluation', ''),
	];


	// hook: game_agent_before_insert
	if(function_exists('game_agent_before_insert')) {
		$args = [];
		if(!game_agent_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('game_agent', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('game_agent', $recID, calculated_fields()['game_agent']);

	// hook: game_agent_after_insert
	if(function_exists('game_agent_after_insert')) {
		$res = sql("SELECT * FROM `game_agent` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!game_agent_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('game_agent', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) game_agent_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function game_agent_copy_children($destination_id, $source_id) {
	global $Translation;
...
function game_agent_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('game_agent', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: game_agent_before_delete
	if(function_exists('game_agent_before_delete')) {
		$args = [];
		if(!game_agent_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `biblio_author` WHERE `author_id`='" . makeSafe($id[0]) . "'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'biblio_author', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="button" value="' . $Translation['yes'] . '" onClick="window.location = \'game_agent_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="button" value="' . $Translation[ 'no'] . '" onClick="window.location = \'game_agent_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `biblio_code_demo` WHERE `game_agent`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	// child table: biblio_analyst
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `biblio_analyst` WHERE `author_id`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'biblio_analyst', $RetMsg);
		return $RetMsg;
	} elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation['confirm delete'];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `biblio_analyst` WHERE `author_memberid`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	// child table: bio_author
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_author` WHERE `author_id`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'bio_author', $RetMsg);
		return $RetMsg;
	} elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation['confirm delete'];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_story` WHERE `agent_id`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_chr` WHERE `agent_id`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_encounter` WHERE `authorA`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_encounter` WHERE `authorB`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_code_herme` WHERE `agent_id`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	// child table: hist_author
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_author` WHERE `agent_id`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'hist_author', $RetMsg);
		return $RetMsg;
	} elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation['confirm delete'];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_chr` WHERE `agent_id`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_chr_scene` WHERE `agent_id`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_encounter` WHERE `agentA`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_encounter` WHERE `agentB`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	// child table: ecounter_analyst
	$res = sql("SELECT `id` FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `ecounter_analyst` WHERE `agent_id`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'ecounter_analyst', $RetMsg);
		return $RetMsg;
	} elseif($rirow[0] && $AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation['confirm delete'];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	sql("DELETE FROM `game_agent` WHERE `id`='{$selected_id}'", $eo);

	// hook: game_agent_after_delete
	if(function_exists('game_agent_after_delete')) {
		$args = [];
		game_agent_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='game_agent' AND `pkValue`='{$selected_id}'", $eo);
}

function game_agent_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('game_agent', $selected_id, 'edit')) return false;
	...
	$data = [
		...
		'img' => Request::fileUpload('img', [
			...
			'failure' => function($selected_id, $fileRemoved) {
				...
				return existing_value('game_agent', 'img', $selected_id);
			},
		]),
		'groupID' => Request::val('groupID', ''),
		'selection_class' => Request::val('selection_class', ''),
	...
	// get existing values
	$old_data = getRecord('game_agent', $selected_id);
	...
	// hook: game_agent_before_update
	if(function_exists('game_agent_before_update')) {
		$args = ['old_data' => $old_data];
		if(!game_agent_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="game_agent_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('game_agent', $data['selectedID'], calculated_fields()['game_agent']);

	// hook: game_agent_after_update
	if(function_exists('game_agent_after_update')) {
		$res = sql("SELECT * FROM `game_agent` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!game_agent_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='game_agent' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function game_agent_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$filterer_selection_class = thisOr($_REQUEST['filterer_selection_class'], '');
	$filterer_agenttype1 = thisOr($_REQUEST['filterer_agenttype1'], '');
	$filterer_agenttype2 = thisOr($_REQUEST['filterer_agenttype2'], '');
	$filterer_gender = thisOr($_REQUEST['filterer_gender'], '');
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='game_agent' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='game_agent' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `game_agent` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'game_agent_view.php', false);
		}
			...
			$j("#selection_class-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_selection_class__RAND__.value, t: 'game_agent', f: 'selection_class' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'game_agent', f: 'selection_class' }; },
			...
			$j("#agenttype1-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_agenttype1__RAND__.value, t: 'game_agent', f: 'agenttype1' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'game_agent', f: 'agenttype1' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#agenttype2-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_agenttype2__RAND__.value, t: 'game_agent', f: 'agenttype2' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'game_agent', f: 'agenttype2' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#gender-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_gender__RAND__.value, t: 'game_agent', f: 'gender' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'game_agent', f: 'gender' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#data_evaluation-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_data_evaluation__RAND__.value, t: 'game_agent', f: 'data_evaluation' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'game_agent', f: 'data_evaluation' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/game_agent_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/game_agent_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return game_agent_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return game_agent_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return game_agent_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)) {
		...
		$jsReadOnly .= "\tjQuery('#titles').replaceWith('<div class=\"form-control-static\" id=\"titles\">' + (jQuery('#titles').val() || '') + '</div>');\n";
		$jsReadOnly .= "\tjQuery('#titles_academic').replaceWith('<div class=\"form-control-static\" id=\"titles_academic\">' + (jQuery('#titles_academic').val() || '') + '</div>');\n";
		$jsReadOnly .= "\tjQuery('#titles_nobility').replaceWith('<div class=\"form-control-static\" id=\"titles_nobility\">' + (jQuery('#titles_nobility').val() || '') + '</div>');\n";
		...
		$jsReadOnly .= "\tjQuery('#img_gallery').replaceWith('<div class=\"form-control-static\" id=\"img_gallery\">' + (jQuery('#img_gallery').val() || '') + '</div>');\n";
		$jsReadOnly .= "\tjQuery('#img_gallery, #img_gallery-edit-link').hide();\n";
	...
	$lookup_fields = array('selection_class' => array('class_agent_selection', 'Clase de selecci&#243;n'), 'agenttype1' => array('class_agent_type1', 'Typo de agente 1'), 'agenttype2' => array('class_agent_type2', 'Tipo de agente 2'), 'gender' => array('class_gender', 'G&#233;nero'), 'data_evaluation' => array('class_evaluation', 'Evaluaci&#243;n de datos'), );
	...
	$templateCode = str_replace('<%%UPLOADFILE(img)%%>', ($noUploads ? '' : "<div>{$Translation['upload image']}</div>" . '<i class="glyphicon glyphicon-remove text-danger clear-upload hidden"></i> <input type="file" name="img" id="img" data-filetypes="jpg|jpeg|gif|png" data-maxsize="307200" accept=".jpg,.jpeg,.gif,.png">'), $templateCode);
	...
	$templateCode = str_replace('<%%UPLOADFILE(titles)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(titles_academic)%%>', '', $templateCode);
	$templateCode = str_replace('<%%UPLOADFILE(titles_nobility)%%>', '', $templateCode);
	...
	$templateCode = str_replace('<%%UPLOADFILE(life_span)%%>', '', $templateCode);
	...
	$templateCode = str_replace('<%%UPLOADFILE(img_gallery)%%>', '', $templateCode);
	...
	if($selected_id) {
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(titles)%%>', safe_html($urow['titles']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(titles)%%>', html_attr($row['titles']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(titles)%%>', urlencode($urow['titles']), $templateCode);
		if( $dvprint) $templateCode = str_replace('<%%VALUE(titles_academic)%%>', safe_html($urow['titles_academic']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(titles_academic)%%>', html_attr($row['titles_academic']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(titles_academic)%%>', urlencode($urow['titles_academic']), $templateCode);
		if($dvprint || (!$AllowUpdate && !$AllowInsert)) {
			$templateCode = str_replace('<%%VALUE(titles_nobility)%%>', safe_html($urow['titles_nobility']), $templateCode);
		} else {
			$templateCode = str_replace('<%%VALUE(titles_nobility)%%>', html_attr($row['titles_nobility']), $templateCode);
		}
		$templateCode = str_replace('<%%URLVALUE(titles_nobility)%%>', urlencode($urow['titles_nobility']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(life_span)%%>', safe_html($urow['life_span']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(life_span)%%>', urlencode($urow['life_span']), $templateCode);
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(img_gallery)%%>', safe_html($urow['img_gallery']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(img_gallery)%%>', html_attr($row['img_gallery']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(img_gallery)%%>', urlencode($urow['img_gallery']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(titles)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(titles)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(titles_academic)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(titles_academic)%%>', urlencode(''), $templateCode);
		$templateCode = str_replace('<%%VALUE(titles_nobility)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(titles_nobility)%%>', urlencode(''), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(life_span)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(life_span)%%>', urlencode(''), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(img_gallery)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(img_gallery)%%>', urlencode(''), $templateCode);
	...
	if($_REQUEST['dvprint_x'] == '') {
		...
		if(!$selected_id) {
			$templateCode.="\n\tif(document.getElementById('img_galleryEdit')) { document.getElementById('img_galleryEdit').style.display='inline'; }";
			$templateCode.="\n\tif(document.getElementById('img_galleryEditLink')) { document.getElementById('img_galleryEditLink').style.display='none'; }";
		}
	...
	$rdata = $jdata = get_defaults('game_agent');
	if($selected_id) {
		$jdata = get_joined_record('game_agent', $selected_id);
		if($jdata === false) $jdata = get_defaults('game_agent');
		$rdata = $row;
	}
	$templateCode .= loadView('game_agent-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: game_agent_dv
	if(function_exists('game_agent_dv')) {
		$args=[];
		game_agent_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
