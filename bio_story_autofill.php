...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$table_perms = getTablePermissions('bio_story');
	...
	switch($mfk) {
		...
		case 'author_id':
			...
			$res = sql("SELECT `bio_author`.`id` as 'id', IF(    CHAR_LENGTH(`bio_team1`.`team`), CONCAT_WS('',   `bio_team1`.`team`), '') as 'team', IF(    CHAR_LENGTH(`game_agent1`.`id`) || CHAR_LENGTH(`game_agent1`.`memberID`), CONCAT_WS('',   `game_agent1`.`id`, '   ', `game_agent1`.`memberID`), '') as 'author_id', IF(    CHAR_LENGTH(`game_agent1`.`memberID`), CONCAT_WS('',   `game_agent1`.`memberID`), '') as 'author_memberid', IF(    CHAR_LENGTH(`game_agent1`.`last_name`), CONCAT_WS('',   `game_agent1`.`last_name`), '') as 'last_name', IF(    CHAR_LENGTH(`game_agent1`.`first_name`), CONCAT_WS('',   `game_agent1`.`first_name`), '') as 'first_name' FROM `bio_author` LEFT JOIN `bio_team` as bio_team1 ON `bio_team1`.`id`=`bio_author`.`team` LEFT JOIN `game_agent` as game_agent1 ON `game_agent1`.`id`=`bio_author`.`author_id`  WHERE `bio_author`.`id`='{$id}' limit 1", $eo);
