...
function class_dynamicstorypoints_insert(){
	global $Translation;
	...
	$data['cat3'] = makeSafe($_REQUEST['cat3']);
		if($data['cat3'] == empty_lookup_value){ $data['cat3'] = ''; }

	// hook: class_dynamicstorypoints_before_insert
	if(function_exists('class_dynamicstorypoints_before_insert')){
		$args=array();
		if(!class_dynamicstorypoints_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `class_dynamicstorypoints` set       `term`=' . (($data['term'] !== '' && $data['term'] !== NULL) ? "'{$data['term']}'" : 'NULL') . ', `value`=' . (($data['value'] !== '' && $data['value'] !== NULL) ? "'{$data['value']}'" : 'NULL') . ', `description`=' . (($data['description'] !== '' && $data['description'] !== NULL) ? "'{$data['description']}'" : 'NULL') . ', `cat1`=' . (($data['cat1'] !== '' && $data['cat1'] !== NULL) ? "'{$data['cat1']}'" : 'NULL') . ', `cat2`=' . (($data['cat2'] !== '' && $data['cat2'] !== NULL) ? "'{$data['cat2']}'" : 'NULL') . ', `cat3`=' . (($data['cat3'] !== '' && $data['cat3'] !== NULL) ? "'{$data['cat3']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"class_dynamicstorypoints_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
...
function class_dynamicstorypoints_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('class_dynamicstorypoints');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='class_dynamicstorypoints' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='class_dynamicstorypoints' and pkValue='$selected_id'");
	...
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `mc_resolve`='".addslashes($id[0])."'", $eo);
...
function class_dynamicstorypoints_update($selected_id){
	global $Translation;
	...
	if(function_exists('class_dynamicstorypoints_before_update')){
		$args=array();
		if(!class_dynamicstorypoints_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `class_dynamicstorypoints` set       `term`=' . (($data['term'] !== '' && $data['term'] !== NULL) ? "'{$data['term']}'" : 'NULL') . ', `value`=' . (($data['value'] !== '' && $data['value'] !== NULL) ? "'{$data['value']}'" : 'NULL') . ', `description`=' . (($data['description'] !== '' && $data['description'] !== NULL) ? "'{$data['description']}'" : 'NULL') . ', `cat1`=' . (($data['cat1'] !== '' && $data['cat1'] !== NULL) ? "'{$data['cat1']}'" : 'NULL') . ', `cat2`=' . (($data['cat2'] !== '' && $data['cat2'] !== NULL) ? "'{$data['cat2']}'" : 'NULL') . ', `cat3`=' . (($data['cat3'] !== '' && $data['cat3'] !== NULL) ? "'{$data['cat3']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function class_dynamicstorypoints_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	$filterer_cat3 = thisOr(undo_magic_quotes($_REQUEST['filterer_cat3']), '');
