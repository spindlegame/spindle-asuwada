	...
	function get_tables_info($skip_authentication = false) {
		...
		$all_tables = array(
			/* ['table_name' => [table props assoc array] */   
				'assignments' => array(
					'Caption' => 'Assignments',
					'Description' => 'Here you can assign resources to projects.',
				...
				'resources' => array(
					'Caption' => 'Resources',
					'Description' => 'Define your resources here',
		...
		if($schema === null) {
			...
			$schema = [
				'assignments' => [
					...
					'ResourceId' => ['appgini' => "INT UNSIGNED NULL"],
	...
	*/
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			'assignments' => [
				'ProjectId' => 'SELECT `projects`.`Id`, `projects`.`Name` FROM `projects` ORDER BY 2',
				'ProjectDuration' => 'SELECT `projects`.`Id`, IF(CHAR_LENGTH(if(`projects`.`StartDate`,date_format(`projects`.`StartDate`,\'%d/%m/%Y\'),\'\')) || CHAR_LENGTH(if(`projects`.`EndDate`,date_format(`projects`.`EndDate`,\'%d/%m/%Y\'),\'\')), CONCAT_WS(\'\', if(`projects`.`StartDate`,date_format(`projects`.`StartDate`,\'%d/%m/%Y\'),\'\'), \' <b>to</b> \', if(`projects`.`EndDate`,date_format(`projects`.`EndDate`,\'%d/%m/%Y\'),\'\')), \'\') FROM `projects` ORDER BY 2',
				'ResourceId' => 'SELECT `Id`, `Name` FROM `resources` WHERE `Available`=\'1\' ORDER BY 2',
			],
			'resources' => [
			],
			'projects' => [
