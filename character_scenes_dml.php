...
// Data functions (insert, update, delete, form) for table character_scenes

// This script and data application were generated by AppGini 5.76
// Download AppGini for free from https://bigprof.com/appgini/download/

function character_scenes_insert(){
	global $Translation;

	// mm: can member insert record?
	$arrPerm=getTablePermissions('character_scenes');
	if(!$arrPerm[1]){
		return false;
	}
...
function character_scenes_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('character_scenes');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='character_scenes' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='character_scenes' and pkValue='$selected_id'");
	...
	// hook: character_scenes_before_delete
	if(function_exists('character_scenes_before_delete')){
		$args=array();
		if(!character_scenes_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	// child table: storylines
	$res = sql("select `id` from `character_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		...
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "storylines", $RetMsg);
		$RetMsg = str_replace("<Delete>", "<input type=\"button\" class=\"button\" value=\"".$Translation['yes']."\" onClick=\"window.location='character_scenes_view.php?SelectedID=".urlencode($selected_id)."&delete_x=1&confirmed=1';\">", $RetMsg);
		$RetMsg = str_replace("<Cancel>", "<input type=\"button\" class=\"button\" value=\"".$Translation['no']."\" onClick=\"window.location='character_scenes_view.php?SelectedID=".urlencode($selected_id)."';\">", $RetMsg);
		return $RetMsg;
	}

	sql("delete from `character_scenes` where `id`='$selected_id'", $eo);

	// hook: character_scenes_after_delete
	if(function_exists('character_scenes_after_delete')){
		$args=array();
		character_scenes_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='character_scenes' and pkValue='$selected_id'", $eo);
}

function character_scenes_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('character_scenes');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='character_scenes' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='character_scenes' and pkValue='".makeSafe($selected_id)."'");
	...
	// hook: character_scenes_before_update
	if(function_exists('character_scenes_before_update')){
		$args=array();
		if(!character_scenes_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `character_scenes` set       `author`=' . (($data['author'] !== '' && $data['author'] !== NULL) ? "'{$data['author']}'" : 'NULL') . ', `bibliography`=' . (($data['bibliography'] !== '' && $data['bibliography'] !== NULL) ? "'{$data['bibliography']}'" : 'NULL') . ', `transcript`=' . (($data['transcript'] !== '' && $data['transcript'] !== NULL) ? "'{$data['transcript']}'" : 'NULL') . ', `token_sequence`=' . (($data['token_sequence'] !== '' && $data['token_sequence'] !== NULL) ? "'{$data['token_sequence']}'" : 'NULL') . ', `token`=' . (($data['token'] !== '' && $data['token'] !== NULL) ? "'{$data['token']}'" : 'NULL') . ', `invivo_code`=' . (($data['invivo_code'] !== '' && $data['invivo_code'] !== NULL) ? "'{$data['invivo_code']}'" : 'NULL') . ', `startdate`=' . (($data['startdate'] !== '' && $data['startdate'] !== NULL) ? "'{$data['startdate']}'" : 'NULL') . ', `enddate`=' . (($data['enddate'] !== '' && $data['enddate'] !== NULL) ? "'{$data['enddate']}'" : 'NULL') . ', `person`=' . (($data['person'] !== '' && $data['person'] !== NULL) ? "'{$data['person']}'" : 'NULL') . ', `place`=' . (($data['place'] !== '' && $data['place'] !== NULL) ? "'{$data['place']}'" : 'NULL') . ', `herme_code`=' . (($data['herme_code'] !== '' && $data['herme_code'] !== NULL) ? "'{$data['herme_code']}'" : 'NULL') . ', `impression`=' . (($data['impression'] !== '' && $data['impression'] !== NULL) ? "'{$data['impression']}'" : 'NULL') . ', `noetictension`=' . (($data['noetictension'] !== '' && $data['noetictension'] !== NULL) ? "'{$data['noetictension']}'" : 'NULL') . ', `pc`=' . (($data['pc'] !== '' && $data['pc'] !== NULL) ? "'{$data['pc']}'" : 'NULL') . ', `comment`=' . (($data['comment'] !== '' && $data['comment'] !== NULL) ? "'{$data['comment']}'" : 'NULL') . ', `scene`=' . (($data['scene'] !== '' && $data['scene'] !== NULL) ? "'{$data['scene']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="character_scenes_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: character_scenes_after_update
	if(function_exists('character_scenes_after_update')){
		$res = sql("SELECT * FROM `character_scenes` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!character_scenes_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='character_scenes' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function character_scenes_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `character_scenes` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'character_scenes_view.php', false);
		}
			...
			$j("#author-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_author__RAND__.value, t: 'character_scenes', f: 'author' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'character_scenes', f: 'author' }; },
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#bibliography-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, id: AppGini.current_bibliography__RAND__.value, t: 'character_scenes', f: 'bibliography' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author: filterer_author, s: term, p: page, t: 'character_scenes', f: 'bibliography' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#bibliography-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bibliography__RAND__.value, t: 'character_scenes', f: 'bibliography' },
					success: function(resp){
						$j('[name="bibliography"]').val(resp.results[0].id);
						$j('[id=bibliography-container-readonly__RAND__]').html('<span id="bibliography-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=biblio_doc_view_parent]').hide(); }else{ $j('.btn[id=biblio_doc_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#transcript-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, id: AppGini.current_transcript__RAND__.value, t: 'character_scenes', f: 'transcript' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, s: term, p: page, t: 'character_scenes', f: 'transcript' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#transcript-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_transcript__RAND__.value, t: 'character_scenes', f: 'transcript' },
					success: function(resp){
						$j('[name="transcript"]').val(resp.results[0].id);
						$j('[id=transcript-container-readonly__RAND__]').html('<span id="transcript-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=biblio_transcript_view_parent]').hide(); }else{ $j('.btn[id=biblio_transcript_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#token_sequence-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, id: AppGini.current_token_sequence__RAND__.value, t: 'character_scenes', f: 'token_sequence' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, s: term, p: page, t: 'character_scenes', f: 'token_sequence' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#token_sequence-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_token_sequence__RAND__.value, t: 'character_scenes', f: 'token_sequence' },
					success: function(resp){
						$j('[name="token_sequence"]').val(resp.results[0].id);
						$j('[id=token_sequence-container-readonly__RAND__]').html('<span id="token_sequence-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=biblio_token_view_parent]').hide(); }else{ $j('.btn[id=biblio_token_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#invivo_code-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, id: AppGini.current_invivo_code__RAND__.value, t: 'character_scenes', f: 'invivo_code' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, s: term, p: page, t: 'character_scenes', f: 'invivo_code' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#invivo_code-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_invivo_code__RAND__.value, t: 'character_scenes', f: 'invivo_code' },
					success: function(resp){
						$j('[name="invivo_code"]').val(resp.results[0].id);
						$j('[id=invivo_code-container-readonly__RAND__]').html('<span id="invivo_code-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=code_invivo_view_parent]').hide(); }else{ $j('.btn[id=code_invivo_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#startdate-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						...
						<?php if(!$selected_id && !$filterer_startdate){ ?>
							data: { text: '1', t: 'character_scenes', f: 'startdate' },
						<?php }else{ ?>
							data: { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, id: AppGini.current_startdate__RAND__.value, t: 'character_scenes', f: 'startdate' },
						<?php } ?>
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, s: term, p: page, t: 'character_scenes', f: 'startdate' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#startdate-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_startdate__RAND__.value, t: 'character_scenes', f: 'startdate' },
					success: function(resp){
						$j('[name="startdate"]').val(resp.results[0].id);
						$j('[id=startdate-container-readonly__RAND__]').html('<span id="startdate-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=code_invivo_view_parent]').hide(); }else{ $j('.btn[id=code_invivo_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#herme_code-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, id: AppGini.current_herme_code__RAND__.value, t: 'character_scenes', f: 'herme_code' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, s: term, p: page, t: 'character_scenes', f: 'herme_code' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#herme_code-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_herme_code__RAND__.value, t: 'character_scenes', f: 'herme_code' },
					success: function(resp){
						$j('[name="herme_code"]').val(resp.results[0].id);
						$j('[id=herme_code-container-readonly__RAND__]').html('<span id="herme_code-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=code_herme_view_parent]').hide(); }else{ $j('.btn[id=code_herme_view_parent]').show(); }
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/character_scenes_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/character_scenes_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return character_scenes_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return character_scenes_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return character_scenes_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'character_scenes_autofill.php?rnd1=$rnd1&mfk=token_sequence&id=' + encodeURIComponent('".addslashes($row['token_sequence'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'character_scenes_autofill.php?rnd1=$rnd1&mfk=token_sequence&id=' + encodeURIComponent(AppGini.current_token_sequence{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'character_scenes_autofill.php?rnd1=$rnd1&mfk=invivo_code&id=' + encodeURIComponent('".addslashes($row['invivo_code'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'character_scenes_autofill.php?rnd1=$rnd1&mfk=invivo_code&id=' + encodeURIComponent(AppGini.current_invivo_code{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'character_scenes_autofill.php?rnd1=$rnd1&mfk=herme_code&id=' + encodeURIComponent('".addslashes($row['herme_code'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'character_scenes_autofill.php?rnd1=$rnd1&mfk=herme_code&id=' + encodeURIComponent(AppGini.current_herme_code{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$rdata = $jdata = get_defaults('character_scenes');
	if($selected_id){
		$jdata = get_joined_record('character_scenes', $selected_id);
		if($jdata === false) $jdata = get_defaults('character_scenes');
		$rdata = $row;
	}
	$templateCode .= loadView('character_scenes-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: character_scenes_dv
	if(function_exists('character_scenes_dv')){
		$args=array();
		character_scenes_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
