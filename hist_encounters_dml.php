...
// Data functions (insert, update, delete, form) for table hist_encounters

// This script and data application were generated by AppGini 5.94
// Download AppGini for free from https://bigprof.com/appgini/download/

function hist_encounters_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('hist_encounters');
	...
	// hook: hist_encounters_before_insert
	if(function_exists('hist_encounters_before_insert')) {
		$args = [];
		if(!hist_encounters_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('hist_encounters', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('hist_encounters', $recID, calculated_fields()['hist_encounters']);

	// hook: hist_encounters_after_insert
	if(function_exists('hist_encounters_after_insert')) {
		$res = sql("SELECT * FROM `hist_encounters` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!hist_encounters_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('hist_encounters', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) hist_encounters_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function hist_encounters_copy_children($destination_id, $source_id) {
	global $Translation;
...
function hist_encounters_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('hist_encounters', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: hist_encounters_before_delete
	if(function_exists('hist_encounters_before_delete')) {
		$args = [];
		if(!hist_encounters_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `hist_encounters` WHERE `id`='{$selected_id}'", $eo);

	// hook: hist_encounters_after_delete
	if(function_exists('hist_encounters_after_delete')) {
		$args = [];
		hist_encounters_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='hist_encounters' AND `pkValue`='{$selected_id}'", $eo);
}

function hist_encounters_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('hist_encounters', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('hist_encounters', $selected_id);
	...
	// hook: hist_encounters_before_update
	if(function_exists('hist_encounters_before_update')) {
		$args = ['old_data' => $old_data];
		if(!hist_encounters_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="hist_encounters_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('hist_encounters', $data['selectedID'], calculated_fields()['hist_encounters']);

	// hook: hist_encounters_after_update
	if(function_exists('hist_encounters_after_update')) {
		$res = sql("SELECT * FROM `hist_encounters` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!hist_encounters_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='hist_encounters' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function hist_encounters_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($filterer_bio_storyline && !$filterer_bio_chrA) $filterer_bio_chrA = sqlValue("select character from bio_storylines where id='" . makeSafe($filterer_bio_storyline) . "'");
	if($filterer_bio_storyline && !$filterer_bio_storyA) $filterer_bio_storyA = sqlValue("select biography from bio_storylines where id='" . makeSafe($filterer_bio_storyline) . "'");
	if($filterer_bio_storytext && !$filterer_bio_chrA) $filterer_bio_chrA = sqlValue("select character from bio_storylines where id='" . makeSafe($filterer_bio_storytext) . "'");
	if($filterer_bio_storytext && !$filterer_bio_storyA) $filterer_bio_storyA = sqlValue("select biography from bio_storylines where id='" . makeSafe($filterer_bio_storytext) . "'");
	if($filterer_sceneA && !$filterer_bio_storyA) $filterer_bio_storyA = sqlValue("select bio_story from hist_chr_scenes where id='" . makeSafe($filterer_sceneA) . "'");
	if($filterer_sceneA && !$filterer_bio_storyline) $filterer_bio_storyline = sqlValue("select bio_storyline_no from hist_chr_scenes where id='" . makeSafe($filterer_sceneA) . "'");
	if($filterer_sceneA && !$filterer_bio_storytext) $filterer_bio_storytext = sqlValue("select bio_storyline_no from hist_chr_scenes where id='" . makeSafe($filterer_sceneA) . "'");
	if($filterer_bio_storylineB && !$filterer_bio_chrB) $filterer_bio_chrB = sqlValue("select character from bio_storylines where id='" . makeSafe($filterer_bio_storylineB) . "'");
	if($filterer_bio_storylineB && !$filterer_bio_storyB) $filterer_bio_storyB = sqlValue("select biography from bio_storylines where id='" . makeSafe($filterer_bio_storylineB) . "'");
	if($filterer_bio_storytextB && !$filterer_bio_chrB) $filterer_bio_chrB = sqlValue("select character from bio_storylines where id='" . makeSafe($filterer_bio_storytextB) . "'");
	if($filterer_bio_storytextB && !$filterer_bio_storyB) $filterer_bio_storyB = sqlValue("select biography from bio_storylines where id='" . makeSafe($filterer_bio_storytextB) . "'");
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='hist_encounters' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='hist_encounters' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `hist_encounters` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'hist_encounters_view.php', false);
		}
			...
			$j("#bio_chrA-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_bio_chrA__RAND__.value, t: 'hist_encounters', f: 'bio_chrA' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'hist_encounters', f: 'bio_chrA' }; },
			...
			$j("#bio_storyA-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_bio_storyA__RAND__.value, t: 'hist_encounters', f: 'bio_storyA' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'hist_encounters', f: 'bio_storyA' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#bio_storyline-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_bio_chrA: filterer_bio_chrA, filterer_bio_storyA: filterer_bio_storyA, id: AppGini.current_bio_storyline__RAND__.value, t: 'hist_encounters', f: 'bio_storyline' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_bio_chrA: filterer_bio_chrA, filterer_bio_storyA: filterer_bio_storyA, s: term, p: page, t: 'hist_encounters', f: 'bio_storyline' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#bio_storyline-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bio_storyline__RAND__.value, t: 'hist_encounters', f: 'bio_storyline' },
					success: function(resp) {
						$j('[name="bio_storyline"]').val(resp.results[0].id);
						$j('[id=bio_storyline-container-readonly__RAND__]').html('<span id="bio_storyline-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_storylines_view_parent]').hide(); } else { $j('.btn[id=bio_storylines_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#bio_storytext-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_bio_chrA: filterer_bio_chrA, filterer_bio_storyA: filterer_bio_storyA, id: AppGini.current_bio_storytext__RAND__.value, t: 'hist_encounters', f: 'bio_storytext' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_bio_chrA: filterer_bio_chrA, filterer_bio_storyA: filterer_bio_storyA, s: term, p: page, t: 'hist_encounters', f: 'bio_storytext' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#bio_storytext-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bio_storytext__RAND__.value, t: 'hist_encounters', f: 'bio_storytext' },
					success: function(resp) {
						$j('[name="bio_storytext"]').val(resp.results[0].id);
						$j('[id=bio_storytext-container-readonly__RAND__]').html('<span id="bio_storytext-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_storylines_view_parent]').hide(); } else { $j('.btn[id=bio_storylines_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#sceneA-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_bio_storyA: filterer_bio_storyA, filterer_bio_storyline: filterer_bio_storyline, filterer_bio_storytext: filterer_bio_storytext, id: AppGini.current_sceneA__RAND__.value, t: 'hist_encounters', f: 'sceneA' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
							...
							if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=hist_chr_scenes_view_parent]').hide(); } else { $j('.btn[id=hist_chr_scenes_view_parent]').show(); }
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_bio_storyA: filterer_bio_storyA, filterer_bio_storyline: filterer_bio_storyline, filterer_bio_storytext: filterer_bio_storytext, s: term, p: page, t: 'hist_encounters', f: 'sceneA' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
				...
				if(e.added.id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=hist_chr_scenes_view_parent]').hide(); } else { $j('.btn[id=hist_chr_scenes_view_parent]').show(); }
			...
			if(!$j("#sceneA-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_sceneA__RAND__.value, t: 'hist_encounters', f: 'sceneA' },
					success: function(resp) {
						$j('[name="sceneA"]').val(resp.results[0].id);
						$j('[id=sceneA-container-readonly__RAND__]').html('<span id="sceneA-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=hist_chr_scenes_view_parent]').hide(); } else { $j('.btn[id=hist_chr_scenes_view_parent]').show(); }
			...
			$j("#bio_chrB-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_bio_chrB__RAND__.value, t: 'hist_encounters', f: 'bio_chrB' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'hist_encounters', f: 'bio_chrB' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#bio_storyB-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_bio_storyB__RAND__.value, t: 'hist_encounters', f: 'bio_storyB' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'hist_encounters', f: 'bio_storyB' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#bio_storylineB-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_bio_chrB: filterer_bio_chrB, filterer_bio_storyB: filterer_bio_storyB, id: AppGini.current_bio_storylineB__RAND__.value, t: 'hist_encounters', f: 'bio_storylineB' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_bio_chrB: filterer_bio_chrB, filterer_bio_storyB: filterer_bio_storyB, s: term, p: page, t: 'hist_encounters', f: 'bio_storylineB' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#bio_storylineB-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bio_storylineB__RAND__.value, t: 'hist_encounters', f: 'bio_storylineB' },
					success: function(resp) {
						$j('[name="bio_storylineB"]').val(resp.results[0].id);
						$j('[id=bio_storylineB-container-readonly__RAND__]').html('<span id="bio_storylineB-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_storylines_view_parent]').hide(); } else { $j('.btn[id=bio_storylines_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#bio_storytextB-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_bio_chrB: filterer_bio_chrB, filterer_bio_storyB: filterer_bio_storyB, id: AppGini.current_bio_storytextB__RAND__.value, t: 'hist_encounters', f: 'bio_storytextB' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_bio_chrB: filterer_bio_chrB, filterer_bio_storyB: filterer_bio_storyB, s: term, p: page, t: 'hist_encounters', f: 'bio_storytextB' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#bio_storytextB-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bio_storytextB__RAND__.value, t: 'hist_encounters', f: 'bio_storytextB' },
					success: function(resp) {
						$j('[name="bio_storytextB"]').val(resp.results[0].id);
						$j('[id=bio_storytextB-container-readonly__RAND__]').html('<span id="bio_storytextB-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_storylines_view_parent]').hide(); } else { $j('.btn[id=bio_storylines_view_parent]').show(); }
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/hist_encounters_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/hist_encounters_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return hist_encounters_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return hist_encounters_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return hist_encounters_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	$lookup_fields = array('bio_chrA' => array('bio_chrs', 'Author A'), 'bio_storyA' => array('bio_story', 'Bibliography A'), 'bio_storyline' => array('bio_storylines', 'Transcript A'), 'bio_storytext' => array('bio_storylines', 'Token A'), 'sceneA' => array('hist_chr_scenes', 'Scene A'), 'bio_chrB' => array('bio_chrs', 'Author B'), 'bio_storyB' => array('bio_story', 'Author name B'), 'bio_storylineB' => array('bio_storylines', 'Bibliography B'), 'bio_storytextB' => array('bio_storylines', 'Transcript B'), );
	...
	$rdata = $jdata = get_defaults('hist_encounters');
	if($selected_id) {
		$jdata = get_joined_record('hist_encounters', $selected_id);
		if($jdata === false) $jdata = get_defaults('hist_encounters');
		$rdata = $row;
	}
	$templateCode .= loadView('hist_encounters-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: hist_encounters_dv
	if(function_exists('hist_encounters_dv')) {
		$args=[];
		hist_encounters_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
