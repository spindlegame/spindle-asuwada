...
// Data functions (insert, update, delete, form) for table chr_dev

// This script and data application were generated by AppGini 5.76
// Download AppGini for free from https://bigprof.com/appgini/download/

function chr_dev_insert(){
	global $Translation;

	// mm: can member insert record?
	$arrPerm=getTablePermissions('chr_dev');
	if(!$arrPerm[1]){
		return false;
	}
	...
	$data['dp_resolve'] = makeSafe($_REQUEST['dp_resolve']);
		if($data['dp_resolve'] == empty_lookup_value){ $data['dp_resolve'] = ''; }
	...
	if(is_array($_REQUEST['mc_growth'])){
		$MultipleSeparator=', ';
		foreach($_REQUEST['mc_growth'] as $k => $v)
			$data['mc_growth'] .= makeSafe($v) . $MultipleSeparator;
		$data['mc_growth'] = substr($data['mc_growth'], 0, -1 * strlen($MultipleSeparator));
	}else{
		$data['mc_growth']='';
	}
	$data['illust_growth'] = makeSafe($_REQUEST['illust_growth']);
		if($data['illust_growth'] == empty_lookup_value){ $data['illust_growth'] = ''; }
	if(is_array($_REQUEST['mc_approach'])){
		$MultipleSeparator=', ';
		foreach($_REQUEST['mc_approach'] as $k => $v)
			$data['mc_approach'] .= makeSafe($v) . $MultipleSeparator;
		$data['mc_approach'] = substr($data['mc_approach'], 0, -1 * strlen($MultipleSeparator));
	}else{
		$data['mc_approach']='';
	}
	$data['illust_approach'] = makeSafe($_REQUEST['illust_approach']);
		if($data['illust_approach'] == empty_lookup_value){ $data['illust_approach'] = ''; }
	if(is_array($_REQUEST['mc_ps_style'])){
		$MultipleSeparator=', ';
		foreach($_REQUEST['mc_ps_style'] as $k => $v)
			$data['mc_ps_style'] .= makeSafe($v) . $MultipleSeparator;
		$data['mc_ps_style'] = substr($data['mc_ps_style'], 0, -1 * strlen($MultipleSeparator));
	}else{
		$data['mc_ps_style']='';
	}
...
function chr_dev_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('chr_dev');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='chr_dev' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='chr_dev' and pkValue='$selected_id'");
	...
	// hook: chr_dev_before_delete
	if(function_exists('chr_dev_before_delete')){
		$args=array();
		if(!chr_dev_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	// child table: story_chrs
	$res = sql("select `id` from `chr_dev` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `story_chrs` where `character`='".addslashes($id[0])."'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		...
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "story_chrs", $RetMsg);
		$RetMsg = str_replace("<Delete>", "<input type=\"button\" class=\"button\" value=\"".$Translation['yes']."\" onClick=\"window.location='chr_dev_view.php?SelectedID=".urlencode($selected_id)."&delete_x=1&confirmed=1';\">", $RetMsg);
		$RetMsg = str_replace("<Cancel>", "<input type=\"button\" class=\"button\" value=\"".$Translation['no']."\" onClick=\"window.location='chr_dev_view.php?SelectedID=".urlencode($selected_id)."';\">", $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("select `id` from `chr_dev` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `storydynamic` where `mc_problem`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_dev` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `storydynamic` where `mc_growth`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_dev` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `storydynamic` where `mc_approach`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_dev` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `storydynamic` where `mc_ps_style`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	sql("delete from `chr_dev` where `id`='$selected_id'", $eo);

	// hook: chr_dev_after_delete
	if(function_exists('chr_dev_after_delete')){
		$args=array();
		chr_dev_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='chr_dev' and pkValue='$selected_id'", $eo);
}

function chr_dev_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('chr_dev');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='chr_dev' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='chr_dev' and pkValue='".makeSafe($selected_id)."'");
	...
	// hook: chr_dev_before_update
	if(function_exists('chr_dev_before_update')){
		$args=array();
		if(!chr_dev_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `chr_dev` set       `agent_id`=' . (($data['agent_id'] !== '' && $data['agent_id'] !== NULL) ? "'{$data['agent_id']}'" : 'NULL') . ', `agent_name`=' . (($data['agent_name'] !== '' && $data['agent_name'] !== NULL) ? "'{$data['agent_name']}'" : 'NULL') . ', `cw_name`=' . (($data['cw_name'] !== '' && $data['cw_name'] !== NULL) ? "'{$data['cw_name']}'" : 'NULL') . ', ' . ($data['img']!='' ? "`img`='{$data['img']}'" : ($_REQUEST['img_remove'] != 1 ? '`img`=`img`' : '`img`=NULL')) . ', `dp_resolve`=' . (($data['dp_resolve'] !== '' && $data['dp_resolve'] !== NULL) ? "'{$data['dp_resolve']}'" : 'NULL') . ', `mc_resolve`=' . (($data['mc_resolve'] !== '' && $data['mc_resolve'] !== NULL) ? "'{$data['mc_resolve']}'" : 'NULL') . ', `illust_resolve`=' . (($data['illust_resolve'] !== '' && $data['illust_resolve'] !== NULL) ? "'{$data['illust_resolve']}'" : 'NULL') . ', `mc_growth`=' . (($data['mc_growth'] !== '' && $data['mc_growth'] !== NULL) ? "'{$data['mc_growth']}'" : 'NULL') . ', `illust_growth`=' . (($data['illust_growth'] !== '' && $data['illust_growth'] !== NULL) ? "'{$data['illust_growth']}'" : 'NULL') . ', `mc_approach`=' . (($data['mc_approach'] !== '' && $data['mc_approach'] !== NULL) ? "'{$data['mc_approach']}'" : 'NULL') . ', `illust_approach`=' . (($data['illust_approach'] !== '' && $data['illust_approach'] !== NULL) ? "'{$data['illust_approach']}'" : 'NULL') . ', `mc_ps_style`=' . (($data['mc_ps_style'] !== '' && $data['mc_ps_style'] !== NULL) ? "'{$data['mc_ps_style']}'" : 'NULL') . ', `illust_ps_style`=' . (($data['illust_ps_style'] !== '' && $data['illust_ps_style'] !== NULL) ? "'{$data['illust_ps_style']}'" : 'NULL') . ', `cw_age`=' . (($data['cw_age'] !== '' && $data['cw_age'] !== NULL) ? "'{$data['cw_age']}'" : 'NULL') . ', `cw_gender`=' . (($data['cw_gender'] !== '' && $data['cw_gender'] !== NULL) ? "'{$data['cw_gender']}'" : 'NULL') . ', `cw_communication_style`=' . (($data['cw_communication_style'] !== '' && $data['cw_communication_style'] !== NULL) ? "'{$data['cw_communication_style']}'" : 'NULL') . ', `cw_background`=' . (($data['cw_background'] !== '' && $data['cw_background'] !== NULL) ? "'{$data['cw_background']}'" : 'NULL') . ', `cw_appearance`=' . (($data['cw_appearance'] !== '' && $data['cw_appearance'] !== NULL) ? "'{$data['cw_appearance']}'" : 'NULL') . ', `cw_relationships`=' . (($data['cw_relationships'] !== '' && $data['cw_relationships'] !== NULL) ? "'{$data['cw_relationships']}'" : 'NULL') . ', `cw_ambition`=' . (($data['cw_ambition'] !== '' && $data['cw_ambition'] !== NULL) ? "'{$data['cw_ambition']}'" : 'NULL') . ', `cw_defects`=' . (($data['cw_defects'] !== '' && $data['cw_defects'] !== NULL) ? "'{$data['cw_defects']}'" : 'NULL') . ', `cw_thoughts`=' . (($data['cw_thoughts'] !== '' && $data['cw_thoughts'] !== NULL) ? "'{$data['cw_thoughts']}'" : 'NULL') . ', `cw_relatedness`=' . (($data['cw_relatedness'] !== '' && $data['cw_relatedness'] !== NULL) ? "'{$data['cw_relatedness']}'" : 'NULL') . ', `cw_restrictions`=' . (($data['cw_restrictions'] !== '' && $data['cw_restrictions'] !== NULL) ? "'{$data['cw_restrictions']}'" : 'NULL') . ', `locations`=' . (($data['locations'] !== '' && $data['locations'] !== NULL) ? "'{$data['locations']}'" : 'NULL') . ', `persons`=' . (($data['persons'] !== '' && $data['persons'] !== NULL) ? "'{$data['persons']}'" : 'NULL') . ', `events`=' . (($data['events'] !== '' && $data['events'] !== NULL) ? "'{$data['events']}'" : 'NULL') . ', `noetictension`=' . (($data['noetictension'] !== '' && $data['noetictension'] !== NULL) ? "'{$data['noetictension']}'" : 'NULL') . ', `illust_nt`=' . (($data['illust_nt'] !== '' && $data['illust_nt'] !== NULL) ? "'{$data['illust_nt']}'" : 'NULL') . ', `impression`=' . (($data['impression'] !== '' && $data['impression'] !== NULL) ? "'{$data['impression']}'" : 'NULL') . ', `illust_im`=' . (($data['illust_im'] !== '' && $data['illust_im'] !== NULL) ? "'{$data['illust_im']}'" : 'NULL') . ', `mcs_problem`=' . (($data['mcs_problem'] !== '' && $data['mcs_problem'] !== NULL) ? "'{$data['mcs_problem']}'" : 'NULL') . ', `illust_mcs_problem`=' . (($data['illust_mcs_problem'] !== '' && $data['illust_mcs_problem'] !== NULL) ? "'{$data['illust_mcs_problem']}'" : 'NULL') . ', `mcs_solution`=' . (($data['mcs_solution'] !== '' && $data['mcs_solution'] !== NULL) ? "'{$data['mcs_solution']}'" : 'NULL') . ', `illust_mcs_solution`=' . (($data['illust_mcs_solution'] !== '' && $data['illust_mcs_solution'] !== NULL) ? "'{$data['illust_mcs_solution']}'" : 'NULL') . ', `mcs_symptom`=' . (($data['mcs_symptom'] !== '' && $data['mcs_symptom'] !== NULL) ? "'{$data['mcs_symptom']}'" : 'NULL') . ', `illust_mcs_symptom`=' . (($data['illust_mcs_symptom'] !== '' && $data['illust_mcs_symptom'] !== NULL) ? "'{$data['illust_mcs_symptom']}'" : 'NULL') . ', `mcs_response`=' . (($data['mcs_response'] !== '' && $data['mcs_response'] !== NULL) ? "'{$data['mcs_response']}'" : 'NULL') . ', `illust_mcs_response`=' . (($data['illust_mcs_response'] !== '' && $data['illust_mcs_response'] !== NULL) ? "'{$data['illust_mcs_response']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="chr_dev_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: chr_dev_after_update
	if(function_exists('chr_dev_after_update')){
		$res = sql("SELECT * FROM `chr_dev` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!chr_dev_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='chr_dev' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function chr_dev_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	$filterer_dp_resolve = thisOr(undo_magic_quotes($_REQUEST['filterer_dp_resolve']), '');
	...
	if($filterer_mc_resolve && !$filterer_dp_resolve) $filterer_dp_resolve = sqlValue("select cat3 from class_dynamicstorypoints where id='" . makeSafe($filterer_mc_resolve) . "'");
	if($filterer_illust_resolve && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from chr_scenes where id='" . makeSafe($filterer_illust_resolve) . "'");
	if($filterer_illust_growth && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from chr_scenes where id='" . makeSafe($filterer_illust_growth) . "'");
	if($filterer_illust_approach && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from chr_scenes where id='" . makeSafe($filterer_illust_approach) . "'");
	if($filterer_illust_ps_style && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from chr_scenes where id='" . makeSafe($filterer_illust_ps_style) . "'");
	if($filterer_illust_nt && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from chr_scenes where id='" . makeSafe($filterer_illust_nt) . "'");
	if($filterer_illust_im && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from chr_scenes where id='" . makeSafe($filterer_illust_im) . "'");
	if($filterer_mcs_problem && !$filterer_) $filterer_ = sqlValue("select  from storystatic where id='" . makeSafe($filterer_mcs_problem) . "'");
	if($filterer_illust_mcs_problem && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from chr_scenes where id='" . makeSafe($filterer_illust_mcs_problem) . "'");
	if($filterer_illust_mcs_solution && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from chr_scenes where id='" . makeSafe($filterer_illust_mcs_solution) . "'");
	if($filterer_illust_mcs_symptom && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from chr_scenes where id='" . makeSafe($filterer_illust_mcs_symptom) . "'");
	if($filterer_illust_mcs_response && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from chr_scenes where id='" . makeSafe($filterer_illust_mcs_response) . "'");
	...
	// combobox: dp_resolve
	$combo_dp_resolve = new DataCombo;
	// combobox: mc_resolve, filterable by: dp_resolve
	...
	$combo_mc_growth->ListType = 3;
	$combo_mc_growth->MultipleSeparator = ', ';
	$combo_mc_growth->ListBoxHeight = 10;
	$combo_mc_growth->RadiosPerLine = 1;
	if(is_file(dirname(__FILE__).'/hooks/chr_dev.mc_growth.csv')){
		$mc_growth_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/chr_dev.mc_growth.csv')));
	...
	$combo_mc_approach->ListType = 3;
	$combo_mc_approach->MultipleSeparator = ', ';
	$combo_mc_approach->ListBoxHeight = 10;
	$combo_mc_approach->RadiosPerLine = 1;
	if(is_file(dirname(__FILE__).'/hooks/chr_dev.mc_approach.csv')){
		$mc_approach_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/chr_dev.mc_approach.csv')));
	...
	$combo_mc_ps_style->ListType = 3;
	$combo_mc_ps_style->MultipleSeparator = ', ';
	$combo_mc_ps_style->ListBoxHeight = 10;
	$combo_mc_ps_style->RadiosPerLine = 1;
	if(is_file(dirname(__FILE__).'/hooks/chr_dev.mc_ps_style.csv')){
		$mc_ps_style_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/chr_dev.mc_ps_style.csv')));
	...
	if($selected_id){
		...
		$res = sql("select * from `chr_dev` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'chr_dev_view.php', false);
		}
		$urow = $row; /* unsanitized data */
		$hc = new CI_Input();
		$row = $hc->xss_clean($row); /* sanitize data */
		$combo_agent_id->SelectedData = $row['agent_id'];
		$combo_dp_resolve->SelectedData = $row['dp_resolve'];
		...
		$combo_dp_resolve->SelectedData = $filterer_dp_resolve;
	...
	$combo_dp_resolve->HTML = '<span id="dp_resolve-container' . $rnd1 . '"></span><input type="hidden" name="dp_resolve" id="dp_resolve' . $rnd1 . '" value="' . html_attr($combo_dp_resolve->SelectedData) . '">';
	$combo_dp_resolve->MatchText = '<span id="dp_resolve-container-readonly' . $rnd1 . '"></span><input type="hidden" name="dp_resolve" id="dp_resolve' . $rnd1 . '" value="' . html_attr($combo_dp_resolve->SelectedData) . '">';
	...
	<script>
		...
		AppGini.current_dp_resolve__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['dp_resolve'] : $filterer_dp_resolve); ?>"};
		...
		jQuery(function() {
			setTimeout(function(){
				...
				if(typeof(dp_resolve_reload__RAND__) == 'function') dp_resolve_reload__RAND__();
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(mc_resolve_reload__RAND__) == \'function\') mc_resolve_reload__RAND__(AppGini.current_dp_resolve__RAND__.value);' : ''); ?>
			...
			$j("#agent_id-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_agent_id__RAND__.value, t: 'chr_dev', f: 'agent_id' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'chr_dev', f: 'agent_id' }; },
		...
		function dp_resolve_reload__RAND__(){
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>

			$j("#dp_resolve-container__RAND__").select2({
				/* initial default value */
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_dp_resolve__RAND__.value, t: 'chr_dev', f: 'dp_resolve' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
							});
							$j('[name="dp_resolve"]').val(resp.results[0].id);
							$j('[id=dp_resolve-container-readonly__RAND__]').html('<span id="dp_resolve-match-text">' + resp.results[0].text + '</span>');
						...
						if(typeof(mc_resolve_reload__RAND__) == 'function') mc_resolve_reload__RAND__(AppGini.current_dp_resolve__RAND__.value);

							if(typeof(dp_resolve_update_autofills__RAND__) == 'function') dp_resolve_update_autofills__RAND__();
						}
					});
				},
				width: '100%',
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'chr_dev', f: 'dp_resolve' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
				AppGini.current_dp_resolve__RAND__.value = e.added.id;
				AppGini.current_dp_resolve__RAND__.text = e.added.text;
				$j('[name="dp_resolve"]').val(e.added.id);
			...
			if(!$j("#dp_resolve-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_dp_resolve__RAND__.value, t: 'chr_dev', f: 'dp_resolve' },
		...
		<?php }else{ ?>
			...
			$j.ajax({
				...
				data: { id: AppGini.current_dp_resolve__RAND__.value, t: 'chr_dev', f: 'dp_resolve' },
				success: function(resp){
					$j('[id=dp_resolve-container__RAND__], [id=dp_resolve-container-readonly__RAND__]').html('<span id="dp_resolve-match-text">' + resp.results[0].text + '</span>');
					if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=class_dramatica_storypoints3_view_parent]').hide(); }else{ $j('.btn[id=class_dramatica_storypoints3_view_parent]').show(); }
		...
		function mc_resolve_reload__RAND__(filterer_dp_resolve){
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#mc_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp_resolve: filterer_dp_resolve, id: AppGini.current_mc_resolve__RAND__.value, t: 'chr_dev', f: 'mc_resolve' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
							...
							if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=class_dynamicstorypoints_view_parent]').hide(); }else{ $j('.btn[id=class_dynamicstorypoints_view_parent]').show(); }
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_dp_resolve: filterer_dp_resolve, s: term, p: page, t: 'chr_dev', f: 'mc_resolve' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
				...
				if(e.added.id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=class_dynamicstorypoints_view_parent]').hide(); }else{ $j('.btn[id=class_dynamicstorypoints_view_parent]').show(); }
			...
			if(!$j("#mc_resolve-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mc_resolve__RAND__.value, t: 'chr_dev', f: 'mc_resolve' },
					success: function(resp){
						$j('[name="mc_resolve"]').val(resp.results[0].id);
						$j('[id=mc_resolve-container-readonly__RAND__]').html('<span id="mc_resolve-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=class_dynamicstorypoints_view_parent]').hide(); }else{ $j('.btn[id=class_dynamicstorypoints_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#illust_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_resolve__RAND__.value, t: 'chr_dev', f: 'illust_resolve' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'chr_dev', f: 'illust_resolve' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#illust_resolve-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_resolve__RAND__.value, t: 'chr_dev', f: 'illust_resolve' },
					success: function(resp){
						$j('[name="illust_resolve"]').val(resp.results[0].id);
						$j('[id=illust_resolve-container-readonly__RAND__]').html('<span id="illust_resolve-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=chr_scenes_view_parent]').hide(); }else{ $j('.btn[id=chr_scenes_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#illust_growth-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_growth__RAND__.value, t: 'chr_dev', f: 'illust_growth' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'chr_dev', f: 'illust_growth' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#illust_growth-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_growth__RAND__.value, t: 'chr_dev', f: 'illust_growth' },
					success: function(resp){
						$j('[name="illust_growth"]').val(resp.results[0].id);
						$j('[id=illust_growth-container-readonly__RAND__]').html('<span id="illust_growth-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=chr_scenes_view_parent]').hide(); }else{ $j('.btn[id=chr_scenes_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#illust_approach-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_approach__RAND__.value, t: 'chr_dev', f: 'illust_approach' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'chr_dev', f: 'illust_approach' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#illust_approach-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_approach__RAND__.value, t: 'chr_dev', f: 'illust_approach' },
					success: function(resp){
						$j('[name="illust_approach"]').val(resp.results[0].id);
						$j('[id=illust_approach-container-readonly__RAND__]').html('<span id="illust_approach-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=chr_scenes_view_parent]').hide(); }else{ $j('.btn[id=chr_scenes_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#illust_ps_style-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_ps_style__RAND__.value, t: 'chr_dev', f: 'illust_ps_style' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'chr_dev', f: 'illust_ps_style' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#illust_ps_style-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_ps_style__RAND__.value, t: 'chr_dev', f: 'illust_ps_style' },
					success: function(resp){
						$j('[name="illust_ps_style"]').val(resp.results[0].id);
						$j('[id=illust_ps_style-container-readonly__RAND__]').html('<span id="illust_ps_style-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=chr_scenes_view_parent]').hide(); }else{ $j('.btn[id=chr_scenes_view_parent]').show(); }
			...
			$j("#noetictension-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_noetictension__RAND__.value, t: 'chr_dev', f: 'noetictension' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'chr_dev', f: 'noetictension' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#illust_nt-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_nt__RAND__.value, t: 'chr_dev', f: 'illust_nt' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'chr_dev', f: 'illust_nt' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#illust_nt-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_nt__RAND__.value, t: 'chr_dev', f: 'illust_nt' },
					success: function(resp){
						$j('[name="illust_nt"]').val(resp.results[0].id);
						$j('[id=illust_nt-container-readonly__RAND__]').html('<span id="illust_nt-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=chr_scenes_view_parent]').hide(); }else{ $j('.btn[id=chr_scenes_view_parent]').show(); }
			...
			$j("#impression-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_impression__RAND__.value, t: 'chr_dev', f: 'impression' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'chr_dev', f: 'impression' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#illust_im-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_im__RAND__.value, t: 'chr_dev', f: 'illust_im' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'chr_dev', f: 'illust_im' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#illust_im-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_im__RAND__.value, t: 'chr_dev', f: 'illust_im' },
					success: function(resp){
						$j('[name="illust_im"]').val(resp.results[0].id);
						$j('[id=illust_im-container-readonly__RAND__]').html('<span id="illust_im-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=chr_scenes_view_parent]').hide(); }else{ $j('.btn[id=chr_scenes_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#mcs_problem-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_story: filterer_story, id: AppGini.current_mcs_problem__RAND__.value, t: 'chr_dev', f: 'mcs_problem' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_story: filterer_story, s: term, p: page, t: 'chr_dev', f: 'mcs_problem' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#mcs_problem-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mcs_problem__RAND__.value, t: 'chr_dev', f: 'mcs_problem' },
					success: function(resp){
						$j('[name="mcs_problem"]').val(resp.results[0].id);
						$j('[id=mcs_problem-container-readonly__RAND__]').html('<span id="mcs_problem-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=storystatic_view_parent]').hide(); }else{ $j('.btn[id=storystatic_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#illust_mcs_problem-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_problem__RAND__.value, t: 'chr_dev', f: 'illust_mcs_problem' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'chr_dev', f: 'illust_mcs_problem' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#illust_mcs_problem-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_problem__RAND__.value, t: 'chr_dev', f: 'illust_mcs_problem' },
					success: function(resp){
						$j('[name="illust_mcs_problem"]').val(resp.results[0].id);
						$j('[id=illust_mcs_problem-container-readonly__RAND__]').html('<span id="illust_mcs_problem-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=chr_scenes_view_parent]').hide(); }else{ $j('.btn[id=chr_scenes_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#illust_mcs_solution-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_solution__RAND__.value, t: 'chr_dev', f: 'illust_mcs_solution' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'chr_dev', f: 'illust_mcs_solution' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#illust_mcs_solution-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_solution__RAND__.value, t: 'chr_dev', f: 'illust_mcs_solution' },
					success: function(resp){
						$j('[name="illust_mcs_solution"]').val(resp.results[0].id);
						$j('[id=illust_mcs_solution-container-readonly__RAND__]').html('<span id="illust_mcs_solution-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=chr_scenes_view_parent]').hide(); }else{ $j('.btn[id=chr_scenes_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#illust_mcs_symptom-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_symptom__RAND__.value, t: 'chr_dev', f: 'illust_mcs_symptom' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'chr_dev', f: 'illust_mcs_symptom' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#illust_mcs_symptom-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_symptom__RAND__.value, t: 'chr_dev', f: 'illust_mcs_symptom' },
					success: function(resp){
						$j('[name="illust_mcs_symptom"]').val(resp.results[0].id);
						$j('[id=illust_mcs_symptom-container-readonly__RAND__]').html('<span id="illust_mcs_symptom-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=chr_scenes_view_parent]').hide(); }else{ $j('.btn[id=chr_scenes_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#illust_mcs_response-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_response__RAND__.value, t: 'chr_dev', f: 'illust_mcs_response' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'chr_dev', f: 'illust_mcs_response' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#illust_mcs_response-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_response__RAND__.value, t: 'chr_dev', f: 'illust_mcs_response' },
					success: function(resp){
						$j('[name="illust_mcs_response"]').val(resp.results[0].id);
						$j('[id=illust_mcs_response-container-readonly__RAND__]').html('<span id="illust_mcs_response-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=chr_scenes_view_parent]').hide(); }else{ $j('.btn[id=chr_scenes_view_parent]').show(); }
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/chr_dev_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/chr_dev_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return chr_dev_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return chr_dev_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return chr_dev_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)){
		...
		$jsReadOnly .= "\tjQuery('#dp_resolve').prop('disabled', true).css({ color: '#555', backgroundColor: '#fff' });\n";
		$jsReadOnly .= "\tjQuery('#dp_resolve_caption').prop('disabled', true).css({ color: '#555', backgroundColor: 'white' });\n";
		...
		$jsReadOnly .= "\tjQuery('#s2id_mc_growth').remove();\n";
		...
		$jsReadOnly .= "\tjQuery('#s2id_mc_approach').remove();\n";
		...
		$jsReadOnly .= "\tjQuery('#s2id_mc_ps_style').remove();\n";
	...
	$templateCode = str_replace('<%%COMBO(dp_resolve)%%>', $combo_dp_resolve->HTML, $templateCode);
	$templateCode = str_replace('<%%COMBOTEXT(dp_resolve)%%>', $combo_dp_resolve->MatchText, $templateCode);
	$templateCode = str_replace('<%%URLCOMBOTEXT(dp_resolve)%%>', urlencode($combo_dp_resolve->MatchText), $templateCode);
	...
	$lookup_fields = array(  'agent_id' => array('biblio_author', 'MemberID'), 'dp_resolve' => array('class_dramatica_storypoints3', 'Dp resolve'), 'mc_resolve' => array('class_dynamicstorypoints', 'Resolve'), 'illust_resolve' => array('chr_scenes', 'Illustration of MC Resolve'), 'illust_growth' => array('chr_scenes', 'Illustration of MC Growth'), 'illust_approach' => array('chr_scenes', 'Illustration of MC Approach'), 'illust_ps_style' => array('chr_scenes', 'Illustration of MC PS Style'), 'noetictension' => array('class_nt', 'Noetic tension'), 'illust_nt' => array('chr_scenes', 'Illustration of Noetic Tension'), 'impression' => array('class_im', 'Overall Impression Management'), 'illust_im' => array('chr_scenes', 'Illustration of Impression Mnmt.'), 'mcs_problem' => array('storystatic', 'Problem'), 'illust_mcs_problem' => array('chr_scenes', 'Illustration of MCS Problem'), 'illust_mcs_solution' => array('chr_scenes', 'Illustration of MCS Solution'), 'illust_mcs_symptom' => array('chr_scenes', 'Illustration of MCS Symptome'), 'illust_mcs_response' => array('chr_scenes', 'Illustration of MCS Response'));
	...
	$templateCode = str_replace('<%%UPLOADFILE(dp_resolve)%%>', '', $templateCode);
	...
	if($selected_id){
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(dp_resolve)%%>', safe_html($urow['dp_resolve']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(dp_resolve)%%>', html_attr($row['dp_resolve']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(dp_resolve)%%>', urlencode($urow['dp_resolve']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(dp_resolve)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(dp_resolve)%%>', urlencode(''), $templateCode);
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'chr_dev_autofill.php?rnd1=$rnd1&mfk=agent_id&id=' + encodeURIComponent('".addslashes($row['agent_id'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'chr_dev_autofill.php?rnd1=$rnd1&mfk=agent_id&id=' + encodeURIComponent(AppGini.current_agent_id{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'chr_dev_autofill.php?rnd1=$rnd1&mfk=mcs_problem&id=' + encodeURIComponent('".addslashes($row['mcs_problem'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'chr_dev_autofill.php?rnd1=$rnd1&mfk=mcs_problem&id=' + encodeURIComponent(AppGini.current_mcs_problem{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$rdata = $jdata = get_defaults('chr_dev');
	if($selected_id){
		$jdata = get_joined_record('chr_dev', $selected_id);
		if($jdata === false) $jdata = get_defaults('chr_dev');
		$rdata = $row;
	}
	$templateCode .= loadView('chr_dev-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: chr_dev_dv
	if(function_exists('chr_dev_dv')){
		$args=array();
		chr_dev_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
