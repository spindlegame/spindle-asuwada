...
function class_rights_insert(){
	...
	$data['right'] = makeSafe($_REQUEST['right']);
	...
	$data['certification'] = makeSafe($_REQUEST['certification']);
...
function class_rights_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$res = sql("select `id` from `class_rights` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `biblio_transcript` where `ip_rights`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
