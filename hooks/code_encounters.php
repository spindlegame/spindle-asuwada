<?php
	...
	/**
	 * @file
	 * This file contains hook functions that get called when data operations are performed on 'code_encounters' table. 
	...
	*/
	...
	function code_encounters_init(&$options, $memberInfo, &$args){
	...
	*/
	...
	function code_encounters_header($contentType, $memberInfo, &$args){
		$header='';
	...
	*/
	...
	function code_encounters_footer($contentType, $memberInfo, &$args){
		$footer='';
	 ...
	 *     $data['agentA'], $data['authorA'], $data['bibliographyA'], $data['transcriptA'], $data['tokenA'], $data['sceneA'], $data['agentB'], $data['authorB'], $data['bibliographyB'], $data['transcriptB'], $data['tokenB'], $data['sceneB'], $data['type'], $data['conflicttype'], $data['story_scene'], $data['nd_color'], $data['nd_width'], $data['nd_style'], $data['nd_opacity'], $data['nd_visibility'], $data['lbl_lable'], $data['lbl_color'], $data['lbl_size']
	...
	*/
	...
	function code_encounters_before_insert(&$data, $memberInfo, &$args){
	...
	*/
	...
	function code_encounters_after_insert($data, $memberInfo, &$args){
	 ...
	 * Note: if a field is set as read-only or hidden in detail view, it can't be modified through $data. You should use a direct SQL statement instead.
	 * For this table, the array items are: 
	 *     $data['id'], $data['agentA'], $data['authorA'], $data['bibliographyA'], $data['transcriptA'], $data['tokenA'], $data['sceneA'], $data['agentB'], $data['authorB'], $data['bibliographyB'], $data['transcriptB'], $data['tokenB'], $data['sceneB'], $data['type'], $data['conflicttype'], $data['story_scene'], $data['nd_color'], $data['nd_width'], $data['nd_style'], $data['nd_opacity'], $data['nd_visibility'], $data['lbl_lable'], $data['lbl_color'], $data['lbl_size']
	...
	*/
	...
	function code_encounters_before_update(&$data, $memberInfo, &$args){
	...
	*/
	...
	function code_encounters_after_update($data, $memberInfo, &$args){
	...
	*/
	...
	function code_encounters_before_delete($selectedID, &$skipChecks, $memberInfo, &$args){
	...
	*/
	...
	function code_encounters_after_delete($selectedID, $memberInfo, &$args){
	...
	*/
	...
	function code_encounters_dv($selectedID, $memberInfo, &$html, &$args){
	...
	*/
	...
	function code_encounters_csv($query, $memberInfo, &$args){
	...
	*/
	...
	function code_encounters_batch_actions(&$args){
