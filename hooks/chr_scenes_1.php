<?php
	...
	/**
	 * @file
	 * This file contains hook functions that get called when data operations are performed on 'chr_scenes_1' table. 
	...
	*/
	...
	function chr_scenes_1_init(&$options, $memberInfo, &$args){
	...
	*/
	...
	function chr_scenes_1_header($contentType, $memberInfo, &$args){
		$header='';
	...
	*/
	...
	function chr_scenes_1_footer($contentType, $memberInfo, &$args){
		$footer='';
	 ...
	 *     $data['author_id'], $data['author_name'], $data['bibliography'], $data['transcript'], $data['token_sequence'], $data['token'], $data['invivo_code'], $data['startdate'], $data['enddate'], $data['person'], $data['place'], $data['herme_code'], $data['impression'], $data['noetictension'], $data['pc'], $data['chr_element'], $data['comment'], $data['illustration'], $data['scene']
	...
	*/
	...
	function chr_scenes_1_before_insert(&$data, $memberInfo, &$args){
	...
	*/
	...
	function chr_scenes_1_after_insert($data, $memberInfo, &$args){
	 ...
	 * Note: if a field is set as read-only or hidden in detail view, it can't be modified through $data. You should use a direct SQL statement instead.
	 * For this table, the array items are: 
	 *     $data['id'], $data['author_id'], $data['author_name'], $data['bibliography'], $data['transcript'], $data['token_sequence'], $data['token'], $data['invivo_code'], $data['startdate'], $data['enddate'], $data['person'], $data['place'], $data['herme_code'], $data['impression'], $data['noetictension'], $data['pc'], $data['chr_element'], $data['comment'], $data['illustration'], $data['scene']
	...
	*/
	...
	function chr_scenes_1_before_update(&$data, $memberInfo, &$args){
	...
	*/
	...
	function chr_scenes_1_after_update($data, $memberInfo, &$args){
	...
	*/
	...
	function chr_scenes_1_before_delete($selectedID, &$skipChecks, $memberInfo, &$args){
	...
	*/
	...
	function chr_scenes_1_after_delete($selectedID, $memberInfo, &$args){
	...
	*/
	...
	function chr_scenes_1_dv($selectedID, $memberInfo, &$html, &$args){
	...
	*/
	...
	function chr_scenes_1_csv($query, $memberInfo, &$args){
	...
	*/
	...
	function chr_scenes_1_batch_actions(&$args){
