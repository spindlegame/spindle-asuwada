...
function class_story_acts_insert(){
	global $Translation;
	...
	if(function_exists('class_story_acts_before_insert')){
		$args=array();
		if(!class_story_acts_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `class_story_acts` set       `act`=' . (($data['act'] !== '' && $data['act'] !== NULL) ? "'{$data['act']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"class_story_acts_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
...
function class_story_acts_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('class_story_acts');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='class_story_acts' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='class_story_acts' and pkValue='$selected_id'");
	...
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `storylines` where `story_act`='".addslashes($id[0])."'", $eo);
	...
	$res = sql("select `id` from `class_story_acts` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `class_dramatica_steps` where `act`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
...
function class_story_acts_update($selected_id){
	global $Translation;
	...
	if(function_exists('class_story_acts_before_update')){
		$args=array();
		if(!class_story_acts_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `class_story_acts` set       `act`=' . (($data['act'] !== '' && $data['act'] !== NULL) ? "'{$data['act']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
