...
function biblio_doc_insert(&$error_message = '') {
	...
	$data = [
		...
		'format' => br2nl(Request::val('format', '')),
	...
	if($data['title'] === '') {
		echo StyleSheet() . "\n\n<div class=\"alert alert-danger\">{$Translation['error:']} 'T&#237;tulo': {$Translation['field not null']}<br><br>";
		echo '<a href="" onclick="history.go(-1); return false;">' . $Translation['< back'] . '</a></div>';
		exit;
	}
...
function biblio_doc_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	$res = sql("SELECT `id` FROM `biblio_doc` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `assignments` WHERE `biblio_doc`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
...
function biblio_doc_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$filterer_medium = thisOr($_REQUEST['filterer_medium'], '');
	...
	if($selected_id) {
		...
		$combo_medium->SelectedData = $row['medium'];
		...
		$combo_medium->SelectedData = $filterer_medium;
	...
	$combo_medium->HTML = '<span id="medium-container' . $rnd1 . '"></span><input type="hidden" name="medium" id="medium' . $rnd1 . '" value="' . html_attr($combo_medium->SelectedData) . '">';
	$combo_medium->MatchText = '<span id="medium-container-readonly' . $rnd1 . '"></span><input type="hidden" name="medium" id="medium' . $rnd1 . '" value="' . html_attr($combo_medium->SelectedData) . '">';
	...
	<script>
		...
		AppGini.current_medium__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['medium'] : $filterer_medium); ?>"};
		...
		jQuery(function() {
			setTimeout(function() {
				...
				if(typeof(medium_reload__RAND__) == 'function') medium_reload__RAND__();
		...
		function medium_reload__RAND__() {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>

			$j("#medium-container__RAND__").select2({
				/* initial default value */
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_medium__RAND__.value, t: 'biblio_doc', f: 'medium' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
							});
							$j('[name="medium"]').val(resp.results[0].id);
							$j('[id=medium-container-readonly__RAND__]').html('<span id="medium-match-text">' + resp.results[0].text + '</span>');
							if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_bibliography_media_view_parent]').hide(); } else { $j('.btn[id=class_bibliography_media_view_parent]').show(); }


							if(typeof(medium_update_autofills__RAND__) == 'function') medium_update_autofills__RAND__();
						}
					});
				},
				width: '100%',
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'biblio_doc', f: 'medium' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
				AppGini.current_medium__RAND__.value = e.added.id;
				AppGini.current_medium__RAND__.text = e.added.text;
				$j('[name="medium"]').val(e.added.id);
				if(e.added.id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_bibliography_media_view_parent]').hide(); } else { $j('.btn[id=class_bibliography_media_view_parent]').show(); }


				if(typeof(medium_update_autofills__RAND__) == 'function') medium_update_autofills__RAND__();
			});

			if(!$j("#medium-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_medium__RAND__.value, t: 'biblio_doc', f: 'medium' },
		...
		<?php } else { ?>
			...
			$j.ajax({
				...
				data: { id: AppGini.current_medium__RAND__.value, t: 'biblio_doc', f: 'medium' },
				success: function(resp) {
					$j('[id=medium-container__RAND__], [id=medium-container-readonly__RAND__]').html('<span id="medium-match-text">' + resp.results[0].text + '</span>');
					if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_bibliography_media_view_parent]').hide(); } else { $j('.btn[id=class_bibliography_media_view_parent]').show(); }
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)) {
		...
		$jsReadOnly .= "\tjQuery('#medium').prop('disabled', true).css({ color: '#555', backgroundColor: '#fff' });\n";
		$jsReadOnly .= "\tjQuery('#medium_caption').prop('disabled', true).css({ color: '#555', backgroundColor: 'white' });\n";
	...
	$templateCode = str_replace('<%%COMBO(medium)%%>', $combo_medium->HTML, $templateCode);
	$templateCode = str_replace('<%%COMBOTEXT(medium)%%>', $combo_medium->MatchText, $templateCode);
	$templateCode = str_replace('<%%URLCOMBOTEXT(medium)%%>', urlencode($combo_medium->MatchText), $templateCode);
	...
	$lookup_fields = array('author_id' => array('biblio_author', 'ID de autor'), 'type' => array('class_bibliography_type', 'Tipo de texto'), 'genre' => array('class_bibliography_genre', 'G&#233;nero'), 'medium' => array('class_bibliography_media', 'Medio'), 'language' => array('class_language', 'Idioma'), 'rights' => array('class_rights', 'Derechos de autor'), 'authority_organization' => array('class_authority_library', 'Organizaci&#243;n control de autoridades'), 'team' => array('biblio_team', 'Equipo bibliogr&#225;fico'), 'biblio_lead' => array('biblio_analyst', 'Bibl&#237;ografo analista principal'), 'data_evaluation' => array('class_evaluation', 'Evaluaci&#243;n de datos'), );
	...
	if($selected_id) {
		...
		if($dvprint || (!$AllowUpdate && !$AllowInsert)) {
			$templateCode = str_replace('<%%VALUE(format)%%>', safe_html($urow['format']), $templateCode);
		} else {
			$templateCode = str_replace('<%%VALUE(format)%%>', html_attr($row['format']), $templateCode);
		}
