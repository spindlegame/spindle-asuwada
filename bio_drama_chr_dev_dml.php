...
// Data functions (insert, update, delete, form) for table bio_drama_chr_dev

// This script and data application were generated by AppGini 5.94
// Download AppGini for free from https://bigprof.com/appgini/download/

function bio_drama_chr_dev_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('bio_drama_chr_dev');
	...
	// hook: bio_drama_chr_dev_before_insert
	if(function_exists('bio_drama_chr_dev_before_insert')) {
		$args = [];
		if(!bio_drama_chr_dev_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('bio_drama_chr_dev', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('bio_drama_chr_dev', $recID, calculated_fields()['bio_drama_chr_dev']);

	// hook: bio_drama_chr_dev_after_insert
	if(function_exists('bio_drama_chr_dev_after_insert')) {
		$res = sql("SELECT * FROM `bio_drama_chr_dev` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!bio_drama_chr_dev_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('bio_drama_chr_dev', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) bio_drama_chr_dev_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function bio_drama_chr_dev_copy_children($destination_id, $source_id) {
	global $Translation;
...
function bio_drama_chr_dev_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('bio_drama_chr_dev', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: bio_drama_chr_dev_before_delete
	if(function_exists('bio_drama_chr_dev_before_delete')) {
		$args = [];
		if(!bio_drama_chr_dev_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	$res = sql("SELECT `id` FROM `bio_drama_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'bio_storydynamic', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="button" value="' . $Translation['yes'] . '" onClick="window.location = \'bio_drama_chr_dev_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="button" value="' . $Translation[ 'no'] . '" onClick="window.location = \'bio_drama_chr_dev_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
	...
	sql("DELETE FROM `bio_drama_chr_dev` WHERE `id`='{$selected_id}'", $eo);

	// hook: bio_drama_chr_dev_after_delete
	if(function_exists('bio_drama_chr_dev_after_delete')) {
		$args = [];
		bio_drama_chr_dev_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='bio_drama_chr_dev' AND `pkValue`='{$selected_id}'", $eo);
}

function bio_drama_chr_dev_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('bio_drama_chr_dev', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('bio_drama_chr_dev', $selected_id);
	...
	// hook: bio_drama_chr_dev_before_update
	if(function_exists('bio_drama_chr_dev_before_update')) {
		$args = ['old_data' => $old_data];
		if(!bio_drama_chr_dev_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="bio_drama_chr_dev_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('bio_drama_chr_dev', $data['selectedID'], calculated_fields()['bio_drama_chr_dev']);

	// hook: bio_drama_chr_dev_after_update
	if(function_exists('bio_drama_chr_dev_after_update')) {
		$res = sql("SELECT * FROM `bio_drama_chr_dev` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!bio_drama_chr_dev_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='bio_drama_chr_dev' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function bio_drama_chr_dev_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($filterer_dp2_resolve && !$filterer_dp1_resolve) $filterer_dp1_resolve = sqlValue("select cat1 from class_dramatica_storypoints2 where id='" . makeSafe($filterer_dp2_resolve) . "'");
	if($filterer_dp3_resolve && !$filterer_dp1_resolve) $filterer_dp1_resolve = sqlValue("select cat1 from class_dramatica_storypoints3 where id='" . makeSafe($filterer_dp3_resolve) . "'");
	if($filterer_dp3_resolve && !$filterer_dp2_resolve) $filterer_dp2_resolve = sqlValue("select cat2 from class_dramatica_storypoints3 where id='" . makeSafe($filterer_dp3_resolve) . "'");
	...
	if($filterer_dp3_growth && !$filterer_dp1_resolve) $filterer_dp1_resolve = sqlValue("select cat1 from class_dramatica_storypoints3 where id='" . makeSafe($filterer_dp3_growth) . "'");
	if($filterer_dp3_growth && !$filterer_dp2_resolve) $filterer_dp2_resolve = sqlValue("select cat2 from class_dramatica_storypoints3 where id='" . makeSafe($filterer_dp3_growth) . "'");
	...
	if($filterer_dp3_approach && !$filterer_dp1_resolve) $filterer_dp1_resolve = sqlValue("select cat1 from class_dramatica_storypoints3 where id='" . makeSafe($filterer_dp3_approach) . "'");
	if($filterer_dp3_approach && !$filterer_dp2_resolve) $filterer_dp2_resolve = sqlValue("select cat2 from class_dramatica_storypoints3 where id='" . makeSafe($filterer_dp3_approach) . "'");
	...
	if($filterer_mcs_problem && !$filterer_) $filterer_ = sqlValue("select  from bio_storystatic where id='" . makeSafe($filterer_mcs_problem) . "'");
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='bio_drama_chr_dev' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='bio_drama_chr_dev' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `bio_drama_chr_dev` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'bio_drama_chr_dev_view.php', false);
		}
			...
			$j("#agent_id-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_agent_id__RAND__.value, t: 'bio_drama_chr_dev', f: 'agent_id' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_drama_chr_dev', f: 'agent_id' }; },
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#bio_story-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_bio_story__RAND__.value, t: 'bio_drama_chr_dev', f: 'bio_story' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'bio_story' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#bio_story-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bio_story__RAND__.value, t: 'bio_drama_chr_dev', f: 'bio_story' },
					success: function(resp) {
						$j('[name="bio_story"]').val(resp.results[0].id);
						$j('[id=bio_story-container-readonly__RAND__]').html('<span id="bio_story-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_story_view_parent]').hide(); } else { $j('.btn[id=bio_story_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#cw_name-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_bio_story: filterer_bio_story, id: AppGini.current_cw_name__RAND__.value, t: 'bio_drama_chr_dev', f: 'cw_name' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_bio_story: filterer_bio_story, s: term, p: page, t: 'bio_drama_chr_dev', f: 'cw_name' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#cw_name-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_cw_name__RAND__.value, t: 'bio_drama_chr_dev', f: 'cw_name' },
					success: function(resp) {
						$j('[name="cw_name"]').val(resp.results[0].id);
						$j('[id=cw_name-container-readonly__RAND__]').html('<span id="cw_name-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_view_parent]').hide(); } else { $j('.btn[id=bio_chr_view_parent]').show(); }
			...
			$j("#dp1_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_dp1_resolve__RAND__.value, t: 'bio_drama_chr_dev', f: 'dp1_resolve' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_drama_chr_dev', f: 'dp1_resolve' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#dp2_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, id: AppGini.current_dp2_resolve__RAND__.value, t: 'bio_drama_chr_dev', f: 'dp2_resolve' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, s: term, p: page, t: 'bio_drama_chr_dev', f: 'dp2_resolve' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#dp2_resolve-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_dp2_resolve__RAND__.value, t: 'bio_drama_chr_dev', f: 'dp2_resolve' },
					success: function(resp) {
						$j('[name="dp2_resolve"]').val(resp.results[0].id);
						$j('[id=dp2_resolve-container-readonly__RAND__]').html('<span id="dp2_resolve-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dramatica_storypoints2_view_parent]').hide(); } else { $j('.btn[id=class_dramatica_storypoints2_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#dp3_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, id: AppGini.current_dp3_resolve__RAND__.value, t: 'bio_drama_chr_dev', f: 'dp3_resolve' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, s: term, p: page, t: 'bio_drama_chr_dev', f: 'dp3_resolve' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#dp3_resolve-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_dp3_resolve__RAND__.value, t: 'bio_drama_chr_dev', f: 'dp3_resolve' },
					success: function(resp) {
						$j('[name="dp3_resolve"]').val(resp.results[0].id);
						$j('[id=dp3_resolve-container-readonly__RAND__]').html('<span id="dp3_resolve-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dramatica_storypoints3_view_parent]').hide(); } else { $j('.btn[id=class_dramatica_storypoints3_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#mc_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_resolve: filterer_dp3_resolve, id: AppGini.current_mc_resolve__RAND__.value, t: 'bio_drama_chr_dev', f: 'mc_resolve' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_resolve: filterer_dp3_resolve, s: term, p: page, t: 'bio_drama_chr_dev', f: 'mc_resolve' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#mc_resolve-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mc_resolve__RAND__.value, t: 'bio_drama_chr_dev', f: 'mc_resolve' },
					success: function(resp) {
						$j('[name="mc_resolve"]').val(resp.results[0].id);
						$j('[id=mc_resolve-container-readonly__RAND__]').html('<span id="mc_resolve-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints4_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints4_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_resolve__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_resolve' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'illust_resolve' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_resolve-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_resolve__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_resolve' },
					success: function(resp) {
						$j('[name="illust_resolve"]').val(resp.results[0].id);
						$j('[id=illust_resolve-container-readonly__RAND__]').html('<span id="illust_resolve-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#dp3_growth-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, id: AppGini.current_dp3_growth__RAND__.value, t: 'bio_drama_chr_dev', f: 'dp3_growth' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, s: term, p: page, t: 'bio_drama_chr_dev', f: 'dp3_growth' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#dp3_growth-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_dp3_growth__RAND__.value, t: 'bio_drama_chr_dev', f: 'dp3_growth' },
					success: function(resp) {
						$j('[name="dp3_growth"]').val(resp.results[0].id);
						$j('[id=dp3_growth-container-readonly__RAND__]').html('<span id="dp3_growth-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dramatica_storypoints3_view_parent]').hide(); } else { $j('.btn[id=class_dramatica_storypoints3_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#mc_growth-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_growth: filterer_dp3_growth, id: AppGini.current_mc_growth__RAND__.value, t: 'bio_drama_chr_dev', f: 'mc_growth' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_growth: filterer_dp3_growth, s: term, p: page, t: 'bio_drama_chr_dev', f: 'mc_growth' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#mc_growth-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mc_growth__RAND__.value, t: 'bio_drama_chr_dev', f: 'mc_growth' },
					success: function(resp) {
						$j('[name="mc_growth"]').val(resp.results[0].id);
						$j('[id=mc_growth-container-readonly__RAND__]').html('<span id="mc_growth-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints4_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints4_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_growth-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_growth__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_growth' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'illust_growth' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_growth-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_growth__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_growth' },
					success: function(resp) {
						$j('[name="illust_growth"]').val(resp.results[0].id);
						$j('[id=illust_growth-container-readonly__RAND__]').html('<span id="illust_growth-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#dp3_approach-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, id: AppGini.current_dp3_approach__RAND__.value, t: 'bio_drama_chr_dev', f: 'dp3_approach' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, s: term, p: page, t: 'bio_drama_chr_dev', f: 'dp3_approach' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#dp3_approach-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_dp3_approach__RAND__.value, t: 'bio_drama_chr_dev', f: 'dp3_approach' },
					success: function(resp) {
						$j('[name="dp3_approach"]').val(resp.results[0].id);
						$j('[id=dp3_approach-container-readonly__RAND__]').html('<span id="dp3_approach-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dramatica_storypoints3_view_parent]').hide(); } else { $j('.btn[id=class_dramatica_storypoints3_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#mc_approach-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_approach: filterer_dp3_approach, id: AppGini.current_mc_approach__RAND__.value, t: 'bio_drama_chr_dev', f: 'mc_approach' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_approach: filterer_dp3_approach, s: term, p: page, t: 'bio_drama_chr_dev', f: 'mc_approach' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#mc_approach-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mc_approach__RAND__.value, t: 'bio_drama_chr_dev', f: 'mc_approach' },
					success: function(resp) {
						$j('[name="mc_approach"]').val(resp.results[0].id);
						$j('[id=mc_approach-container-readonly__RAND__]').html('<span id="mc_approach-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints4_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints4_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_approach-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_approach__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_approach' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'illust_approach' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_approach-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_approach__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_approach' },
					success: function(resp) {
						$j('[name="illust_approach"]').val(resp.results[0].id);
						$j('[id=illust_approach-container-readonly__RAND__]').html('<span id="illust_approach-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
			...
			$j("#dp3_psstyle-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_dp3_psstyle__RAND__.value, t: 'bio_drama_chr_dev', f: 'dp3_psstyle' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_drama_chr_dev', f: 'dp3_psstyle' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#mc_ps_style-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, id: AppGini.current_mc_ps_style__RAND__.value, t: 'bio_drama_chr_dev', f: 'mc_ps_style' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, s: term, p: page, t: 'bio_drama_chr_dev', f: 'mc_ps_style' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#mc_ps_style-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mc_ps_style__RAND__.value, t: 'bio_drama_chr_dev', f: 'mc_ps_style' },
					success: function(resp) {
						$j('[name="mc_ps_style"]').val(resp.results[0].id);
						$j('[id=mc_ps_style-container-readonly__RAND__]').html('<span id="mc_ps_style-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints4_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints4_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_ps_style-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_ps_style__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_ps_style' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'illust_ps_style' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_ps_style-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_ps_style__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_ps_style' },
					success: function(resp) {
						$j('[name="illust_ps_style"]').val(resp.results[0].id);
						$j('[id=illust_ps_style-container-readonly__RAND__]').html('<span id="illust_ps_style-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
			...
			$j("#noetictension-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_noetictension__RAND__.value, t: 'bio_drama_chr_dev', f: 'noetictension' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_drama_chr_dev', f: 'noetictension' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_nt-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_nt__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_nt' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'illust_nt' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_nt-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_nt__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_nt' },
					success: function(resp) {
						$j('[name="illust_nt"]').val(resp.results[0].id);
						$j('[id=illust_nt-container-readonly__RAND__]').html('<span id="illust_nt-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
			...
			$j("#impression-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_impression__RAND__.value, t: 'bio_drama_chr_dev', f: 'impression' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_drama_chr_dev', f: 'impression' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_im-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_im__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_im' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'illust_im' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_im-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_im__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_im' },
					success: function(resp) {
						$j('[name="illust_im"]').val(resp.results[0].id);
						$j('[id=illust_im-container-readonly__RAND__]').html('<span id="illust_im-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#mcs_problem-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_story: filterer_story, id: AppGini.current_mcs_problem__RAND__.value, t: 'bio_drama_chr_dev', f: 'mcs_problem' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_story: filterer_story, s: term, p: page, t: 'bio_drama_chr_dev', f: 'mcs_problem' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#mcs_problem-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mcs_problem__RAND__.value, t: 'bio_drama_chr_dev', f: 'mcs_problem' },
					success: function(resp) {
						$j('[name="mcs_problem"]').val(resp.results[0].id);
						$j('[id=mcs_problem-container-readonly__RAND__]').html('<span id="mcs_problem-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_storystatic_view_parent]').hide(); } else { $j('.btn[id=bio_storystatic_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_mcs_problem-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_problem__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_mcs_problem' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'illust_mcs_problem' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_mcs_problem-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_problem__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_mcs_problem' },
					success: function(resp) {
						$j('[name="illust_mcs_problem"]').val(resp.results[0].id);
						$j('[id=illust_mcs_problem-container-readonly__RAND__]').html('<span id="illust_mcs_problem-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_mcs_solution-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_solution__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_mcs_solution' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'illust_mcs_solution' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_mcs_solution-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_solution__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_mcs_solution' },
					success: function(resp) {
						$j('[name="illust_mcs_solution"]').val(resp.results[0].id);
						$j('[id=illust_mcs_solution-container-readonly__RAND__]').html('<span id="illust_mcs_solution-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_mcs_symptom-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_symptom__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_mcs_symptom' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'illust_mcs_symptom' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_mcs_symptom-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_symptom__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_mcs_symptom' },
					success: function(resp) {
						$j('[name="illust_mcs_symptom"]').val(resp.results[0].id);
						$j('[id=illust_mcs_symptom-container-readonly__RAND__]').html('<span id="illust_mcs_symptom-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_mcs_response-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_response__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_mcs_response' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_drama_chr_dev', f: 'illust_mcs_response' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_mcs_response-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_response__RAND__.value, t: 'bio_drama_chr_dev', f: 'illust_mcs_response' },
					success: function(resp) {
						$j('[name="illust_mcs_response"]').val(resp.results[0].id);
						$j('[id=illust_mcs_response-container-readonly__RAND__]').html('<span id="illust_mcs_response-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/bio_drama_chr_dev_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/bio_drama_chr_dev_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return bio_drama_chr_dev_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return bio_drama_chr_dev_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return bio_drama_chr_dev_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	$lookup_fields = array('agent_id' => array('biblio_author', 'MemberID'), 'bio_story' => array('bio_story', 'Bio story'), 'cw_name' => array('bio_chr', 'Cw name'), 'dp1_resolve' => array('class_dramatica_storypoints1', 'Dynamic point cat1'), 'dp2_resolve' => array('class_dramatica_storypoints2', 'Dynamic point cat2'), 'dp3_resolve' => array('class_dramatica_storypoints3', 'Dynamic point cat3'), 'mc_resolve' => array('class_dynamicstorypoints4', 'MC resolve'), 'illust_resolve' => array('bio_chr_scene', 'Illustration of MC Resolve'), 'dp3_growth' => array('class_dramatica_storypoints3', 'Dynamic point cat3'), 'mc_growth' => array('class_dynamicstorypoints4', 'Growth'), 'illust_growth' => array('bio_chr_scene', 'Illustration of MC Growth'), 'dp3_approach' => array('class_dramatica_storypoints3', 'Dynamic point cat3'), 'mc_approach' => array('class_dynamicstorypoints4', 'Approach'), 'illust_approach' => array('bio_chr_scene', 'Illustration of MC Approach'), 'dp3_psstyle' => array('class_dramatica_storypoints3', 'Dynamic point cat3'), 'mc_ps_style' => array('class_dynamicstorypoints4', 'Problem Solving Style'), 'illust_ps_style' => array('bio_chr_scene', 'Illustration of MC PS Style'), 'noetictension' => array('class_nt', 'Noetic tension'), 'illust_nt' => array('bio_chr_scene', 'Illustration of Noetic Tension'), 'impression' => array('class_im', 'Overall Impression Management'), 'illust_im' => array('bio_chr_scene', 'Illustration of Impression Mnmt.'), 'mcs_problem' => array('bio_storystatic', 'Problem'), 'illust_mcs_problem' => array('bio_chr_scene', 'Illustration of MCS Problem'), 'illust_mcs_solution' => array('bio_chr_scene', 'Illustration of MCS Solution'), 'illust_mcs_symptom' => array('bio_chr_scene', 'Illustration of MCS Symptome'), 'illust_mcs_response' => array('bio_chr_scene', 'Illustration of MCS Response'), );
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'bio_drama_chr_dev_autofill.php?rnd1=$rnd1&mfk=agent_id&id=' + encodeURIComponent('".addslashes($row['agent_id'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'bio_drama_chr_dev_autofill.php?rnd1=$rnd1&mfk=agent_id&id=' + encodeURIComponent(AppGini.current_agent_id{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'bio_drama_chr_dev_autofill.php?rnd1=$rnd1&mfk=mcs_problem&id=' + encodeURIComponent('".addslashes($row['mcs_problem'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'bio_drama_chr_dev_autofill.php?rnd1=$rnd1&mfk=mcs_problem&id=' + encodeURIComponent(AppGini.current_mcs_problem{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$rdata = $jdata = get_defaults('bio_drama_chr_dev');
	if($selected_id) {
		$jdata = get_joined_record('bio_drama_chr_dev', $selected_id);
		if($jdata === false) $jdata = get_defaults('bio_drama_chr_dev');
		$rdata = $row;
	}
	$templateCode .= loadView('bio_drama_chr_dev-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: bio_drama_chr_dev_dv
	if(function_exists('bio_drama_chr_dev_dv')) {
		$args=[];
		bio_drama_chr_dev_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
