...
// Data functions (insert, update, delete, form) for table storyweaving_scenes_1

// This script and data application were generated by AppGini 5.94
// Download AppGini for free from https://bigprof.com/appgini/download/

function storyweaving_scenes_1_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('storyweaving_scenes_1');
	...
	// hook: storyweaving_scenes_1_before_insert
	if(function_exists('storyweaving_scenes_1_before_insert')) {
		$args = [];
		if(!storyweaving_scenes_1_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('storyweaving_scenes_1', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('storyweaving_scenes_1', $recID, calculated_fields()['storyweaving_scenes_1']);

	// hook: storyweaving_scenes_1_after_insert
	if(function_exists('storyweaving_scenes_1_after_insert')) {
		$res = sql("SELECT * FROM `storyweaving_scenes_1` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!storyweaving_scenes_1_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('storyweaving_scenes_1', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) storyweaving_scenes_1_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function storyweaving_scenes_1_copy_children($destination_id, $source_id) {
	global $Translation;
...
function storyweaving_scenes_1_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('storyweaving_scenes_1', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: storyweaving_scenes_1_before_delete
	if(function_exists('storyweaving_scenes_1_before_delete')) {
		$args = [];
		if(!storyweaving_scenes_1_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `storyweaving_scenes_1` WHERE `id`='{$selected_id}'", $eo);

	// hook: storyweaving_scenes_1_after_delete
	if(function_exists('storyweaving_scenes_1_after_delete')) {
		$args = [];
		storyweaving_scenes_1_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='storyweaving_scenes_1' AND `pkValue`='{$selected_id}'", $eo);
}

function storyweaving_scenes_1_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('storyweaving_scenes_1', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('storyweaving_scenes_1', $selected_id);
	...
	// hook: storyweaving_scenes_1_before_update
	if(function_exists('storyweaving_scenes_1_before_update')) {
		$args = ['old_data' => $old_data];
		if(!storyweaving_scenes_1_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		'storyweaving_scenes_1', 
		backtick_keys_once($set), 
		['`id`' => $selected_id], 
		$error_message
	)) {
		echo $error_message;
		echo '<a href="storyweaving_scenes_1_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('storyweaving_scenes_1', $data['selectedID'], calculated_fields()['storyweaving_scenes_1']);

	// hook: storyweaving_scenes_1_after_update
	if(function_exists('storyweaving_scenes_1_after_update')) {
		$res = sql("SELECT * FROM `storyweaving_scenes_1` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!storyweaving_scenes_1_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='storyweaving_scenes_1' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function storyweaving_scenes_1_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='storyweaving_scenes_1' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='storyweaving_scenes_1' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `storyweaving_scenes_1` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'storyweaving_scenes_1_view.php', false);
		}
			...
			$j("#story-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_story__RAND__.value, t: 'storyweaving_scenes_1', f: 'story' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'storyweaving_scenes_1', f: 'story' }; },
			...
			$j("#step-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_step__RAND__.value, t: 'storyweaving_scenes_1', f: 'step' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'storyweaving_scenes_1', f: 'step' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#throughline-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_throughline__RAND__.value, t: 'storyweaving_scenes_1', f: 'throughline' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'storyweaving_scenes_1', f: 'throughline' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#domain-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_domain__RAND__.value, t: 'storyweaving_scenes_1', f: 'domain' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'storyweaving_scenes_1', f: 'domain' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#concern-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_concern__RAND__.value, t: 'storyweaving_scenes_1', f: 'concern' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'storyweaving_scenes_1', f: 'concern' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#issue-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_issue__RAND__.value, t: 'storyweaving_scenes_1', f: 'issue' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'storyweaving_scenes_1', f: 'issue' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#theme-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_theme__RAND__.value, t: 'storyweaving_scenes_1', f: 'theme' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'storyweaving_scenes_1', f: 'theme' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/storyweaving_scenes_1_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/storyweaving_scenes_1_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return storyweaving_scenes_1_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return storyweaving_scenes_1_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return storyweaving_scenes_1_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	$rdata = $jdata = get_defaults('storyweaving_scenes_1');
	if($selected_id) {
		$jdata = get_joined_record('storyweaving_scenes_1', $selected_id);
		if($jdata === false) $jdata = get_defaults('storyweaving_scenes_1');
		$rdata = $row;
	}
	$templateCode .= loadView('storyweaving_scenes_1-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: storyweaving_scenes_1_dv
	if(function_exists('storyweaving_scenes_1_dv')) {
		$args=[];
		storyweaving_scenes_1_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
