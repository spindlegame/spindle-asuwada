...
// Data functions (insert, update, delete, form) for table bio_encounters

// This script and data application were generated by AppGini 5.94
// Download AppGini for free from https://bigprof.com/appgini/download/

function bio_encounters_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('bio_encounters');
	...
	// hook: bio_encounters_before_insert
	if(function_exists('bio_encounters_before_insert')) {
		$args = [];
		if(!bio_encounters_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('bio_encounters', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('bio_encounters', $recID, calculated_fields()['bio_encounters']);

	// hook: bio_encounters_after_insert
	if(function_exists('bio_encounters_after_insert')) {
		$res = sql("SELECT * FROM `bio_encounters` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!bio_encounters_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('bio_encounters', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) bio_encounters_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function bio_encounters_copy_children($destination_id, $source_id) {
	global $Translation;
...
function bio_encounters_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('bio_encounters', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: bio_encounters_before_delete
	if(function_exists('bio_encounters_before_delete')) {
		$args = [];
		if(!bio_encounters_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `bio_encounters` WHERE `id`='{$selected_id}'", $eo);

	// hook: bio_encounters_after_delete
	if(function_exists('bio_encounters_after_delete')) {
		$args = [];
		bio_encounters_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='bio_encounters' AND `pkValue`='{$selected_id}'", $eo);
}

function bio_encounters_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('bio_encounters', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('bio_encounters', $selected_id);
	...
	// hook: bio_encounters_before_update
	if(function_exists('bio_encounters_before_update')) {
		$args = ['old_data' => $old_data];
		if(!bio_encounters_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="bio_encounters_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('bio_encounters', $data['selectedID'], calculated_fields()['bio_encounters']);

	// hook: bio_encounters_after_update
	if(function_exists('bio_encounters_after_update')) {
		$res = sql("SELECT * FROM `bio_encounters` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!bio_encounters_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='bio_encounters' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function bio_encounters_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($filterer_sceneA && !$filterer_authorA) $filterer_authorA = sqlValue("select  from bio_chr_scenes where id='" . makeSafe($filterer_sceneA) . "'");
	if($filterer_sceneA && !$filterer_bibliographyA) $filterer_bibliographyA = sqlValue("select bibliography from bio_chr_scenes where id='" . makeSafe($filterer_sceneA) . "'");
	if($filterer_sceneA && !$filterer_transcriptA) $filterer_transcriptA = sqlValue("select transcript from bio_chr_scenes where id='" . makeSafe($filterer_sceneA) . "'");
	if($filterer_sceneA && !$filterer_tokenA) $filterer_tokenA = sqlValue("select token from bio_chr_scenes where id='" . makeSafe($filterer_sceneA) . "'");
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='bio_encounters' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='bio_encounters' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `bio_encounters` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'bio_encounters_view.php', false);
		}
			...
			$j("#authorA-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_authorA__RAND__.value, t: 'bio_encounters', f: 'authorA' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_encounters', f: 'authorA' }; },
			...
			$j("#author_nameA-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_author_nameA__RAND__.value, t: 'bio_encounters', f: 'author_nameA' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_encounters', f: 'author_nameA' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#bibliographyA-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_authorA: filterer_authorA, id: AppGini.current_bibliographyA__RAND__.value, t: 'bio_encounters', f: 'bibliographyA' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_authorA: filterer_authorA, s: term, p: page, t: 'bio_encounters', f: 'bibliographyA' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#bibliographyA-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bibliographyA__RAND__.value, t: 'bio_encounters', f: 'bibliographyA' },
					success: function(resp) {
						$j('[name="bibliographyA"]').val(resp.results[0].id);
						$j('[id=bibliographyA-container-readonly__RAND__]').html('<span id="bibliographyA-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=biblio_doc_view_parent]').hide(); } else { $j('.btn[id=biblio_doc_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#transcriptA-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_authorA: filterer_authorA, filterer_bibliographyA: filterer_bibliographyA, id: AppGini.current_transcriptA__RAND__.value, t: 'bio_encounters', f: 'transcriptA' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_authorA: filterer_authorA, filterer_bibliographyA: filterer_bibliographyA, s: term, p: page, t: 'bio_encounters', f: 'transcriptA' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#transcriptA-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_transcriptA__RAND__.value, t: 'bio_encounters', f: 'transcriptA' },
					success: function(resp) {
						$j('[name="transcriptA"]').val(resp.results[0].id);
						$j('[id=transcriptA-container-readonly__RAND__]').html('<span id="transcriptA-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=biblio_transcript_view_parent]').hide(); } else { $j('.btn[id=biblio_transcript_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#tokenA-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_authorA: filterer_authorA, filterer_bibliographyA: filterer_bibliographyA, filterer_transcriptA: filterer_transcriptA, id: AppGini.current_tokenA__RAND__.value, t: 'bio_encounters', f: 'tokenA' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_authorA: filterer_authorA, filterer_bibliographyA: filterer_bibliographyA, filterer_transcriptA: filterer_transcriptA, s: term, p: page, t: 'bio_encounters', f: 'tokenA' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#tokenA-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_tokenA__RAND__.value, t: 'bio_encounters', f: 'tokenA' },
					success: function(resp) {
						$j('[name="tokenA"]').val(resp.results[0].id);
						$j('[id=tokenA-container-readonly__RAND__]').html('<span id="tokenA-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=biblio_token_view_parent]').hide(); } else { $j('.btn[id=biblio_token_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#sceneA-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_authorA: filterer_authorA, filterer_bibliographyA: filterer_bibliographyA, filterer_transcriptA: filterer_transcriptA, filterer_tokenA: filterer_tokenA, id: AppGini.current_sceneA__RAND__.value, t: 'bio_encounters', f: 'sceneA' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_authorA: filterer_authorA, filterer_bibliographyA: filterer_bibliographyA, filterer_transcriptA: filterer_transcriptA, filterer_tokenA: filterer_tokenA, s: term, p: page, t: 'bio_encounters', f: 'sceneA' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#sceneA-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_sceneA__RAND__.value, t: 'bio_encounters', f: 'sceneA' },
					success: function(resp) {
						$j('[name="sceneA"]').val(resp.results[0].id);
						$j('[id=sceneA-container-readonly__RAND__]').html('<span id="sceneA-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scenes_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scenes_view_parent]').show(); }
			...
			$j("#authorB-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_authorB__RAND__.value, t: 'bio_encounters', f: 'authorB' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_encounters', f: 'authorB' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#authornameB-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_authornameB__RAND__.value, t: 'bio_encounters', f: 'authornameB' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_encounters', f: 'authornameB' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#bibliographyB-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_authorB: filterer_authorB, id: AppGini.current_bibliographyB__RAND__.value, t: 'bio_encounters', f: 'bibliographyB' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_authorB: filterer_authorB, s: term, p: page, t: 'bio_encounters', f: 'bibliographyB' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#bibliographyB-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bibliographyB__RAND__.value, t: 'bio_encounters', f: 'bibliographyB' },
					success: function(resp) {
						$j('[name="bibliographyB"]').val(resp.results[0].id);
						$j('[id=bibliographyB-container-readonly__RAND__]').html('<span id="bibliographyB-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=biblio_doc_view_parent]').hide(); } else { $j('.btn[id=biblio_doc_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#transcriptB-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_authorB: filterer_authorB, id: AppGini.current_transcriptB__RAND__.value, t: 'bio_encounters', f: 'transcriptB' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_authorB: filterer_authorB, s: term, p: page, t: 'bio_encounters', f: 'transcriptB' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#transcriptB-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_transcriptB__RAND__.value, t: 'bio_encounters', f: 'transcriptB' },
					success: function(resp) {
						$j('[name="transcriptB"]').val(resp.results[0].id);
						$j('[id=transcriptB-container-readonly__RAND__]').html('<span id="transcriptB-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=biblio_transcript_view_parent]').hide(); } else { $j('.btn[id=biblio_transcript_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#tokenB-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_authorB: filterer_authorB, id: AppGini.current_tokenB__RAND__.value, t: 'bio_encounters', f: 'tokenB' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_authorB: filterer_authorB, s: term, p: page, t: 'bio_encounters', f: 'tokenB' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#tokenB-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_tokenB__RAND__.value, t: 'bio_encounters', f: 'tokenB' },
					success: function(resp) {
						$j('[name="tokenB"]').val(resp.results[0].id);
						$j('[id=tokenB-container-readonly__RAND__]').html('<span id="tokenB-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=biblio_token_view_parent]').hide(); } else { $j('.btn[id=biblio_token_view_parent]').show(); }
			...
			$j("#sceneB-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_sceneB__RAND__.value, t: 'bio_encounters', f: 'sceneB' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_encounters', f: 'sceneB' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/bio_encounters_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/bio_encounters_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return bio_encounters_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return bio_encounters_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return bio_encounters_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	$lookup_fields = array('authorA' => array('game_agents', 'Agent A'), 'author_nameA' => array('game_agents', 'Agent name A'), 'bibliographyA' => array('biblio_doc', 'Bibliography A'), 'transcriptA' => array('biblio_transcript', 'Transcript A'), 'tokenA' => array('biblio_token', 'Token A'), 'sceneA' => array('bio_chr_scenes', 'Scene A'), 'authorB' => array('game_agents', 'Agent B'), 'authornameB' => array('game_agents', 'Agent name B'), 'bibliographyB' => array('biblio_doc', 'Bibliography B'), 'transcriptB' => array('biblio_transcript', 'Transcript B'), 'tokenB' => array('biblio_token', 'Token B'), 'sceneB' => array('bio_chr_scenes', 'Scene B'), );
	...
	$rdata = $jdata = get_defaults('bio_encounters');
	if($selected_id) {
		$jdata = get_joined_record('bio_encounters', $selected_id);
		if($jdata === false) $jdata = get_defaults('bio_encounters');
		$rdata = $row;
	}
	$templateCode .= loadView('bio_encounters-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: bio_encounters_dv
	if(function_exists('bio_encounters_dv')) {
		$args=[];
		bio_encounters_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
