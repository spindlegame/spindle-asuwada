...
// Data functions (insert, update, delete, form) for table class_dynamicstorypoints4

// This script and data application were generated by AppGini 5.94
// Download AppGini for free from https://bigprof.com/appgini/download/

function class_dynamicstorypoints4_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('class_dynamicstorypoints4');
	...
	// hook: class_dynamicstorypoints4_before_insert
	if(function_exists('class_dynamicstorypoints4_before_insert')) {
		$args = [];
		if(!class_dynamicstorypoints4_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('class_dynamicstorypoints4', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('class_dynamicstorypoints4', $recID, calculated_fields()['class_dynamicstorypoints4']);

	// hook: class_dynamicstorypoints4_after_insert
	if(function_exists('class_dynamicstorypoints4_after_insert')) {
		$res = sql("SELECT * FROM `class_dynamicstorypoints4` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!class_dynamicstorypoints4_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('class_dynamicstorypoints4', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) class_dynamicstorypoints4_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function class_dynamicstorypoints4_copy_children($destination_id, $source_id) {
	global $Translation;
...
function class_dynamicstorypoints4_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('class_dynamicstorypoints4', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: class_dynamicstorypoints4_before_delete
	if(function_exists('class_dynamicstorypoints4_before_delete')) {
		$args = [];
		if(!class_dynamicstorypoints4_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_chr_dev` WHERE `mc_resolve`='" . makeSafe($id[0]) . "'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'bio_chr_dev', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="button" value="' . $Translation['yes'] . '" onClick="window.location = \'class_dynamicstorypoints4_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="button" value="' . $Translation[ 'no'] . '" onClick="window.location = \'class_dynamicstorypoints4_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_chr_dev` WHERE `mc_growth`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_chr_dev` WHERE `mc_approach`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_chr_dev` WHERE `mc_ps_style`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_storydynamic` WHERE `os_driver`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_storydynamic` WHERE `os_limit`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_storydynamic` WHERE `os_outcome`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_storydynamic` WHERE `os_judgement`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_chr_dev` WHERE `mc_resolve`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_chr_dev` WHERE `mc_growth`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_chr_dev` WHERE `mc_approach`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_chr_dev` WHERE `mc_ps_style`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storydynamic` WHERE `os_driver`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storydynamic` WHERE `os_limit`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storydynamic` WHERE `os_outcome`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storydynamic` WHERE `os_judgement`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	sql("DELETE FROM `class_dynamicstorypoints4` WHERE `id`='{$selected_id}'", $eo);

	// hook: class_dynamicstorypoints4_after_delete
	if(function_exists('class_dynamicstorypoints4_after_delete')) {
		$args = [];
		class_dynamicstorypoints4_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='class_dynamicstorypoints4' AND `pkValue`='{$selected_id}'", $eo);
}

function class_dynamicstorypoints4_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('class_dynamicstorypoints4', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('class_dynamicstorypoints4', $selected_id);
	...
	// hook: class_dynamicstorypoints4_before_update
	if(function_exists('class_dynamicstorypoints4_before_update')) {
		$args = ['old_data' => $old_data];
		if(!class_dynamicstorypoints4_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="class_dynamicstorypoints4_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('class_dynamicstorypoints4', $data['selectedID'], calculated_fields()['class_dynamicstorypoints4']);

	// hook: class_dynamicstorypoints4_after_update
	if(function_exists('class_dynamicstorypoints4_after_update')) {
		$res = sql("SELECT * FROM `class_dynamicstorypoints4` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!class_dynamicstorypoints4_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='class_dynamicstorypoints4' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function class_dynamicstorypoints4_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($filterer_cat3 && !$filterer_cat1) $filterer_cat1 = sqlValue("select cat1 from class_dynamicstorypoints3 where id='" . makeSafe($filterer_cat3) . "'");
	if($filterer_cat3 && !$filterer_cat2) $filterer_cat2 = sqlValue("select cat2 from class_dynamicstorypoints3 where id='" . makeSafe($filterer_cat3) . "'");
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='class_dynamicstorypoints4' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='class_dynamicstorypoints4' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `class_dynamicstorypoints4` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'class_dynamicstorypoints4_view.php', false);
		}
			...
			$j("#cat1-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_cat1__RAND__.value, t: 'class_dynamicstorypoints4', f: 'cat1' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'class_dynamicstorypoints4', f: 'cat1' }; },
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#cat2-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_cat1: filterer_cat1, id: AppGini.current_cat2__RAND__.value, t: 'class_dynamicstorypoints4', f: 'cat2' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_cat1: filterer_cat1, s: term, p: page, t: 'class_dynamicstorypoints4', f: 'cat2' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#cat2-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_cat2__RAND__.value, t: 'class_dynamicstorypoints4', f: 'cat2' },
					success: function(resp) {
						$j('[name="cat2"]').val(resp.results[0].id);
						$j('[id=cat2-container-readonly__RAND__]').html('<span id="cat2-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints2_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints2_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#cat3-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_cat1: filterer_cat1, filterer_cat2: filterer_cat2, id: AppGini.current_cat3__RAND__.value, t: 'class_dynamicstorypoints4', f: 'cat3' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_cat1: filterer_cat1, filterer_cat2: filterer_cat2, s: term, p: page, t: 'class_dynamicstorypoints4', f: 'cat3' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#cat3-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_cat3__RAND__.value, t: 'class_dynamicstorypoints4', f: 'cat3' },
					success: function(resp) {
						$j('[name="cat3"]').val(resp.results[0].id);
						$j('[id=cat3-container-readonly__RAND__]').html('<span id="cat3-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints3_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints3_view_parent]').show(); }
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/class_dynamicstorypoints4_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/class_dynamicstorypoints4_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return class_dynamicstorypoints4_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return class_dynamicstorypoints4_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return class_dynamicstorypoints4_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	$lookup_fields = array('cat1' => array('class_dynamicstorypoints1', 'Class storypoint1'), 'cat2' => array('class_dynamicstorypoints2', 'Class storypoint2'), 'cat3' => array('class_dynamicstorypoints3', 'Cat3'), );
	...
	$rdata = $jdata = get_defaults('class_dynamicstorypoints4');
	if($selected_id) {
		$jdata = get_joined_record('class_dynamicstorypoints4', $selected_id);
		if($jdata === false) $jdata = get_defaults('class_dynamicstorypoints4');
		$rdata = $row;
	}
	$templateCode .= loadView('class_dynamicstorypoints4-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: class_dynamicstorypoints4_dv
	if(function_exists('class_dynamicstorypoints4_dv')) {
		$args=[];
		class_dynamicstorypoints4_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
