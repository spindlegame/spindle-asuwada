...
function biblio_author_insert(&$error_message = '') {
	...
	$data = [
		'game_agent_id' => Request::val('game_agent_id', ''),
		'memberID' => Request::val('memberID', ''),
		'agent_name' => Request::lookup('game_agent_id'),
		'img' => Request::fileUpload('img', [
			...
			'success' => function($name, $selected_id) {
				createThumbnail($name, getThumbnailSpecs('biblio_author', 'img', 'tv'));
				createThumbnail($name, getThumbnailSpecs('biblio_author', 'img', 'dv'));
			},
			'failure' => function($selected_id, $fileRemoved) {
				if(!isset($_REQUEST['SelectedID'])) return '';

				/* for empty upload fields, when saving a copy of an existing record, copy the original upload field */
				return existing_value('biblio_author', 'img', $_REQUEST['SelectedID']);
			},
		]),
	...
	$data = [
		...
		'img' => Request::fileUpload('img', [
			...
			'failure' => function($selected_id, $fileRemoved) {
				...
				return existing_value('biblio_author', 'img', $selected_id);
			},
		]),
		'groupID' => Request::val('groupID', ''),
		'selection_class' => Request::val('selection_class', ''),
...
function biblio_author_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$filterer_game_agent_id = thisOr($_REQUEST['filterer_game_agent_id'], '');
	...
	// combobox: game_agent_id
	$combo_game_agent_id = new DataCombo;
	...
	if($selected_id) {
		...
		$combo_game_agent_id->SelectedData = $row['game_agent_id'];
		...
		$combo_game_agent_id->SelectedData = $filterer_game_agent_id;
	...
	$combo_game_agent_id->HTML = '<span id="game_agent_id-container' . $rnd1 . '"></span><input type="hidden" name="game_agent_id" id="game_agent_id' . $rnd1 . '" value="' . html_attr($combo_game_agent_id->SelectedData) . '">';
	$combo_game_agent_id->MatchText = '<span id="game_agent_id-container-readonly' . $rnd1 . '"></span><input type="hidden" name="game_agent_id" id="game_agent_id' . $rnd1 . '" value="' . html_attr($combo_game_agent_id->SelectedData) . '">';
	...
	<script>
		...
		AppGini.current_game_agent_id__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['game_agent_id'] : $filterer_game_agent_id); ?>"};
		...
		jQuery(function() {
			setTimeout(function() {
				if(typeof(game_agent_id_reload__RAND__) == 'function') game_agent_id_reload__RAND__();
		...
		function game_agent_id_reload__RAND__() {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>

			$j("#game_agent_id-container__RAND__").select2({
				/* initial default value */
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_game_agent_id__RAND__.value, t: 'biblio_author', f: 'game_agent_id' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
							});
							$j('[name="game_agent_id"]').val(resp.results[0].id);
							$j('[id=game_agent_id-container-readonly__RAND__]').html('<span id="game_agent_id-match-text">' + resp.results[0].text + '</span>');
							...
							if(typeof(game_agent_id_update_autofills__RAND__) == 'function') game_agent_id_update_autofills__RAND__();
						}
					});
				},
				width: '100%',
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'biblio_author', f: 'game_agent_id' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
				AppGini.current_game_agent_id__RAND__.value = e.added.id;
				AppGini.current_game_agent_id__RAND__.text = e.added.text;
				$j('[name="game_agent_id"]').val(e.added.id);
			...
			if(!$j("#game_agent_id-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_game_agent_id__RAND__.value, t: 'biblio_author', f: 'game_agent_id' },
		...
		<?php } else { ?>
			...
			$j.ajax({
				...
				data: { id: AppGini.current_game_agent_id__RAND__.value, t: 'biblio_author', f: 'game_agent_id' },
				success: function(resp) {
					$j('[id=game_agent_id-container__RAND__], [id=game_agent_id-container-readonly__RAND__]').html('<span id="game_agent_id-match-text">' + resp.results[0].text + '</span>');
					if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=game_agent_view_parent]').hide(); } else { $j('.btn[id=game_agent_view_parent]').show(); }
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)) {
		$jsReadOnly .= "\tjQuery('#game_agent_id').prop('disabled', true).css({ color: '#555', backgroundColor: '#fff' });\n";
		$jsReadOnly .= "\tjQuery('#game_agent_id_caption').prop('disabled', true).css({ color: '#555', backgroundColor: 'white' });\n";
	...
	// process combos
	$templateCode = str_replace('<%%COMBO(game_agent_id)%%>', $combo_game_agent_id->HTML, $templateCode);
	$templateCode = str_replace('<%%COMBOTEXT(game_agent_id)%%>', $combo_game_agent_id->MatchText, $templateCode);
	$templateCode = str_replace('<%%URLCOMBOTEXT(game_agent_id)%%>', urlencode($combo_game_agent_id->MatchText), $templateCode);
	...
	// process images
	$templateCode = str_replace('<%%UPLOADFILE(game_agent_id)%%>', '', $templateCode);
	...
	// process values
	if($selected_id) {
		if( $dvprint) $templateCode = str_replace('<%%VALUE(game_agent_id)%%>', safe_html($urow['game_agent_id']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(game_agent_id)%%>', html_attr($row['game_agent_id']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(game_agent_id)%%>', urlencode($urow['game_agent_id']), $templateCode);
		...
		$templateCode = str_replace('<%%VALUE(game_agent_id)%%>', '', $templateCode);
		$templateCode = str_replace('<%%URLVALUE(game_agent_id)%%>', urlencode(''), $templateCode);
	...
	$templateCode .= "\tgame_agent_id_update_autofills$rnd1 = function() {\n";
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'biblio_author_autofill.php?rnd1=$rnd1&mfk=game_agent_id&id=' + encodeURIComponent('".addslashes($row['game_agent_id'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'biblio_author_autofill.php?rnd1=$rnd1&mfk=game_agent_id&id=' + encodeURIComponent(AppGini.current_game_agent_id{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
		$templateCode .= "\t\t\tbeforeSend: function() { \$j('#game_agent_id$rnd1').prop('disabled', true); \$j('#game_agent_idLoading').html('<img src=loading.gif align=top>'); },\n";
		$templateCode .= "\t\t\tcomplete: function() { " . (($arrPerm[1] || (($arrPerm[3] == 1 && $ownerMemberID == getLoggedMemberID()) || ($arrPerm[3] == 2 && $ownerGroupID == getLoggedGroupID()) || $arrPerm[3] == 3)) ? "\$j('#game_agent_id$rnd1').prop('disabled', false); " : "\$j('#game_agent_id$rnd1').prop('disabled', true); ")."\$j('#game_agent_idLoading').html(''); \$j(window).resize(); }\n";
	}
	$templateCode .= "\t\t});\n";
	$templateCode .= "\t};\n";
	if(!$dvprint) $templateCode .= "\tif(\$j('#game_agent_id_caption').length) \$j('#game_agent_id_caption').click(function() { game_agent_id_update_autofills$rnd1(); });\n";
