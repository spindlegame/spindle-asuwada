...
function class_dictionary3_insert(&$error_message = '') {
	global $Translation;
	...
	if(function_exists('class_dictionary3_before_insert')) {
		$args = [];
		if(!class_dictionary3_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('class_dictionary3', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('class_dictionary3', $recID, calculated_fields()['class_dictionary3']);
	...
	if(!empty($_REQUEST['SelectedID'])) class_dictionary3_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function class_dictionary3_copy_children($destination_id, $source_id) {
	global $Translation;
...
function class_dictionary3_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('class_dictionary3', $selected_id, 'delete')) {
	...
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `class_dictionary4` WHERE `class3`='" . makeSafe($id[0]) . "'", $eo);
	...
	$res = sql("SELECT `id` FROM `class_dictionary3` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `class_dictionary5` WHERE `class3`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
...
function class_dictionary3_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('class_dictionary3', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('class_dictionary3', $selected_id);
	...
	if(function_exists('class_dictionary3_before_update')) {
		...
		if(!class_dictionary3_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('class_dictionary3', $data['selectedID'], calculated_fields()['class_dictionary3']);
