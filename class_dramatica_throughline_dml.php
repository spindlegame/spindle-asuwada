...
function class_dramatica_throughline_insert(&$error_message = '') {
	global $Translation;
	...
	if(function_exists('class_dramatica_throughline_before_insert')) {
		$args = [];
		if(!class_dramatica_throughline_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('class_dramatica_throughline', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('class_dramatica_throughline', $recID, calculated_fields()['class_dramatica_throughline']);

	// hook: class_dramatica_throughline_after_insert
	if(function_exists('class_dramatica_throughline_after_insert')) {
		$res = sql("SELECT * FROM `class_dramatica_throughline` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!class_dramatica_throughline_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('class_dramatica_throughline', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) class_dramatica_throughline_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function class_dramatica_throughline_copy_children($destination_id, $source_id) {
	global $Translation;
...
function class_dramatica_throughline_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	if(!check_record_permission('class_dramatica_throughline', $selected_id, 'delete')) {
	...
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_storystatic` WHERE `throughline`='" . makeSafe($id[0]) . "'", $eo);
	...
	$res = sql("SELECT `id` FROM `class_dramatica_throughline` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_storyweaving_scene` WHERE `throughline`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dramatica_throughline` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storystatic` WHERE `throughline`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `class_dramatica_throughline` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storyweaving_scene` WHERE `throughline`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
...
function class_dramatica_throughline_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('class_dramatica_throughline', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('class_dramatica_throughline', $selected_id);
	...
	if(function_exists('class_dramatica_throughline_before_update')) {
		...
		if(!class_dramatica_throughline_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	update_calc_fields('class_dramatica_throughline', $data['selectedID'], calculated_fields()['class_dramatica_throughline']);
...
function class_dramatica_throughline_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'class_dramatica_throughline_view.php', false);
		}
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/class_dramatica_throughline_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/class_dramatica_throughline_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return class_dramatica_throughline_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	$rdata = $jdata = get_defaults('class_dramatica_throughline');
	if($selected_id) {
		$jdata = get_joined_record('class_dramatica_throughline', $selected_id);
		if($jdata === false) $jdata = get_defaults('class_dramatica_throughline');
		$rdata = $row;
	}
	$templateCode .= loadView('class_dramatica_throughline-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
