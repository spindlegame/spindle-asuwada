...
// Data functions (insert, update, delete, form) for table chr_scenes

// This script and data application were generated by AppGini 5.76
// Download AppGini for free from https://bigprof.com/appgini/download/

function chr_scenes_insert(){
	global $Translation;

	// mm: can member insert record?
	$arrPerm=getTablePermissions('chr_scenes');
	if(!$arrPerm[1]){
		return false;
	}
	...
	$data['token_sequence'] = makeSafe($_REQUEST['token']);
	...
	$data['chr_element'] = makeSafe($_REQUEST['chr_element']);
		if($data['chr_element'] == empty_lookup_value){ $data['chr_element'] = ''; }
	$data['comment'] = br2nl(makeSafe($_REQUEST['comment']));
	$data['illustration'] = br2nl(makeSafe($_REQUEST['illustration']));
	$data['scene'] = br2nl(makeSafe($_REQUEST['scene']));
	if($data['startdate'] == '') $data['startdate'] = "1";
	if($data['enddate'] == '') $data['enddate'] = "1";

	// hook: chr_scenes_before_insert
	if(function_exists('chr_scenes_before_insert')){
		$args=array();
		if(!chr_scenes_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `chr_scenes` set       `author_id`=' . (($data['author_id'] !== '' && $data['author_id'] !== NULL) ? "'{$data['author_id']}'" : 'NULL') . ', `author_name`=' . (($data['author_name'] !== '' && $data['author_name'] !== NULL) ? "'{$data['author_name']}'" : 'NULL') . ', `bibliography`=' . (($data['bibliography'] !== '' && $data['bibliography'] !== NULL) ? "'{$data['bibliography']}'" : 'NULL') . ', `transcript`=' . (($data['transcript'] !== '' && $data['transcript'] !== NULL) ? "'{$data['transcript']}'" : 'NULL') . ', `token_sequence`=' . (($data['token_sequence'] !== '' && $data['token_sequence'] !== NULL) ? "'{$data['token_sequence']}'" : 'NULL') . ', `token`=' . (($data['token'] !== '' && $data['token'] !== NULL) ? "'{$data['token']}'" : 'NULL') . ', `invivo_code`=' . (($data['invivo_code'] !== '' && $data['invivo_code'] !== NULL) ? "'{$data['invivo_code']}'" : 'NULL') . ', `startdate`=' . (($data['startdate'] !== '' && $data['startdate'] !== NULL) ? "'{$data['startdate']}'" : 'NULL') . ', `enddate`=' . (($data['enddate'] !== '' && $data['enddate'] !== NULL) ? "'{$data['enddate']}'" : 'NULL') . ', `person`=' . (($data['person'] !== '' && $data['person'] !== NULL) ? "'{$data['person']}'" : 'NULL') . ', `place`=' . (($data['place'] !== '' && $data['place'] !== NULL) ? "'{$data['place']}'" : 'NULL') . ', `herme_code`=' . (($data['herme_code'] !== '' && $data['herme_code'] !== NULL) ? "'{$data['herme_code']}'" : 'NULL') . ', `impression`=' . (($data['impression'] !== '' && $data['impression'] !== NULL) ? "'{$data['impression']}'" : 'NULL') . ', `noetictension`=' . (($data['noetictension'] !== '' && $data['noetictension'] !== NULL) ? "'{$data['noetictension']}'" : 'NULL') . ', `pc`=' . (($data['pc'] !== '' && $data['pc'] !== NULL) ? "'{$data['pc']}'" : 'NULL') . ', `chr_element`=' . (($data['chr_element'] !== '' && $data['chr_element'] !== NULL) ? "'{$data['chr_element']}'" : 'NULL') . ', `comment`=' . (($data['comment'] !== '' && $data['comment'] !== NULL) ? "'{$data['comment']}'" : 'NULL') . ', `illustration`=' . (($data['illustration'] !== '' && $data['illustration'] !== NULL) ? "'{$data['illustration']}'" : 'NULL') . ', `scene`=' . (($data['scene'] !== '' && $data['scene'] !== NULL) ? "'{$data['scene']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"chr_scenes_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}

	$recID = db_insert_id(db_link());

	// hook: chr_scenes_after_insert
	if(function_exists('chr_scenes_after_insert')){
		$res = sql("select * from `chr_scenes` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!chr_scenes_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('chr_scenes', $recID, getLoggedMemberID());

	return $recID;
}

function chr_scenes_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('chr_scenes');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='chr_scenes' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='chr_scenes' and pkValue='$selected_id'");
	...
	// hook: chr_scenes_before_delete
	if(function_exists('chr_scenes_before_delete')){
		$args=array();
		if(!chr_scenes_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	// child table: chr_dev
	$res = sql("select `id` from `chr_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `illust_resolve`='".addslashes($id[0])."'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		...
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "chr_dev", $RetMsg);
		$RetMsg = str_replace("<Delete>", "<input type=\"button\" class=\"button\" value=\"".$Translation['yes']."\" onClick=\"window.location='chr_scenes_view.php?SelectedID=".urlencode($selected_id)."&delete_x=1&confirmed=1';\">", $RetMsg);
		$RetMsg = str_replace("<Cancel>", "<input type=\"button\" class=\"button\" value=\"".$Translation['no']."\" onClick=\"window.location='chr_scenes_view.php?SelectedID=".urlencode($selected_id)."';\">", $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("select `id` from `chr_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `illust_growth`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `illust_approach`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `illust_ps_style`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `illust_nt`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `illust_im`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `illust_mcs_problem`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `illust_mcs_solution`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `illust_mcs_symptom`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	$res = sql("select `id` from `chr_scenes` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_dev` where `illust_mcs_response`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	sql("delete from `chr_scenes` where `id`='$selected_id'", $eo);

	// hook: chr_scenes_after_delete
	if(function_exists('chr_scenes_after_delete')){
		$args=array();
		chr_scenes_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='chr_scenes' and pkValue='$selected_id'", $eo);
}

function chr_scenes_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('chr_scenes');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='chr_scenes' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='chr_scenes' and pkValue='".makeSafe($selected_id)."'");
	...
	// hook: chr_scenes_before_update
	if(function_exists('chr_scenes_before_update')){
		$args=array();
		if(!chr_scenes_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `chr_scenes` set       `author_id`=' . (($data['author_id'] !== '' && $data['author_id'] !== NULL) ? "'{$data['author_id']}'" : 'NULL') . ', `author_name`=' . (($data['author_name'] !== '' && $data['author_name'] !== NULL) ? "'{$data['author_name']}'" : 'NULL') . ', `bibliography`=' . (($data['bibliography'] !== '' && $data['bibliography'] !== NULL) ? "'{$data['bibliography']}'" : 'NULL') . ', `transcript`=' . (($data['transcript'] !== '' && $data['transcript'] !== NULL) ? "'{$data['transcript']}'" : 'NULL') . ', `token_sequence`=' . (($data['token_sequence'] !== '' && $data['token_sequence'] !== NULL) ? "'{$data['token_sequence']}'" : 'NULL') . ', `token`=' . (($data['token'] !== '' && $data['token'] !== NULL) ? "'{$data['token']}'" : 'NULL') . ', `invivo_code`=' . (($data['invivo_code'] !== '' && $data['invivo_code'] !== NULL) ? "'{$data['invivo_code']}'" : 'NULL') . ', `startdate`=' . (($data['startdate'] !== '' && $data['startdate'] !== NULL) ? "'{$data['startdate']}'" : 'NULL') . ', `enddate`=' . (($data['enddate'] !== '' && $data['enddate'] !== NULL) ? "'{$data['enddate']}'" : 'NULL') . ', `person`=' . (($data['person'] !== '' && $data['person'] !== NULL) ? "'{$data['person']}'" : 'NULL') . ', `place`=' . (($data['place'] !== '' && $data['place'] !== NULL) ? "'{$data['place']}'" : 'NULL') . ', `herme_code`=' . (($data['herme_code'] !== '' && $data['herme_code'] !== NULL) ? "'{$data['herme_code']}'" : 'NULL') . ', `impression`=' . (($data['impression'] !== '' && $data['impression'] !== NULL) ? "'{$data['impression']}'" : 'NULL') . ', `noetictension`=' . (($data['noetictension'] !== '' && $data['noetictension'] !== NULL) ? "'{$data['noetictension']}'" : 'NULL') . ', `pc`=' . (($data['pc'] !== '' && $data['pc'] !== NULL) ? "'{$data['pc']}'" : 'NULL') . ', `chr_element`=' . (($data['chr_element'] !== '' && $data['chr_element'] !== NULL) ? "'{$data['chr_element']}'" : 'NULL') . ', `comment`=' . (($data['comment'] !== '' && $data['comment'] !== NULL) ? "'{$data['comment']}'" : 'NULL') . ', `illustration`=' . (($data['illustration'] !== '' && $data['illustration'] !== NULL) ? "'{$data['illustration']}'" : 'NULL') . ', `scene`=' . (($data['scene'] !== '' && $data['scene'] !== NULL) ? "'{$data['scene']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="chr_scenes_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: chr_scenes_after_update
	if(function_exists('chr_scenes_after_update')){
		$res = sql("SELECT * FROM `chr_scenes` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!chr_scenes_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='chr_scenes' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function chr_scenes_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	$filterer_chr_element = thisOr(undo_magic_quotes($_REQUEST['filterer_chr_element']), '');
	...
	if($filterer_invivo_code && !$filterer_author_id) $filterer_author_id = sqlValue("select author from code_invivo where id='" . makeSafe($filterer_invivo_code) . "'");
	...
	if($filterer_startdate && !$filterer_author_id) $filterer_author_id = sqlValue("select author from code_invivo where id='" . makeSafe($filterer_startdate) . "'");
	...
	if($filterer_herme_code && !$filterer_author_id) $filterer_author_id = sqlValue("select author_id from code_herme where id='" . makeSafe($filterer_herme_code) . "'");
	...
	if($selected_id){
		...
		$res = sql("select * from `chr_scenes` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'chr_scenes_view.php', false);
		}
			...
			$j("#author_id-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_author_id__RAND__.value, t: 'chr_scenes', f: 'author_id' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'chr_scenes', f: 'author_id' }; },
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#bibliography-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author_id: filterer_author_id, id: AppGini.current_bibliography__RAND__.value, t: 'chr_scenes', f: 'bibliography' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author_id: filterer_author_id, s: term, p: page, t: 'chr_scenes', f: 'bibliography' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#bibliography-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bibliography__RAND__.value, t: 'chr_scenes', f: 'bibliography' },
					success: function(resp){
						$j('[name="bibliography"]').val(resp.results[0].id);
						$j('[id=bibliography-container-readonly__RAND__]').html('<span id="bibliography-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=biblio_doc_view_parent]').hide(); }else{ $j('.btn[id=biblio_doc_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#transcript-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author_id: filterer_author_id, filterer_bibliography: filterer_bibliography, id: AppGini.current_transcript__RAND__.value, t: 'chr_scenes', f: 'transcript' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author_id: filterer_author_id, filterer_bibliography: filterer_bibliography, s: term, p: page, t: 'chr_scenes', f: 'transcript' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#transcript-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_transcript__RAND__.value, t: 'chr_scenes', f: 'transcript' },
					success: function(resp){
						$j('[name="transcript"]').val(resp.results[0].id);
						$j('[id=transcript-container-readonly__RAND__]').html('<span id="transcript-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=biblio_transcript_view_parent]').hide(); }else{ $j('.btn[id=biblio_transcript_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#token-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author_id: filterer_author_id, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, id: AppGini.current_token__RAND__.value, t: 'chr_scenes', f: 'token' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author_id: filterer_author_id, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, s: term, p: page, t: 'chr_scenes', f: 'token' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#token-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_token__RAND__.value, t: 'chr_scenes', f: 'token' },
					success: function(resp){
						$j('[name="token"]').val(resp.results[0].id);
						$j('[id=token-container-readonly__RAND__]').html('<span id="token-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=biblio_token_view_parent]').hide(); }else{ $j('.btn[id=biblio_token_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#invivo_code-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author_id: filterer_author_id, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, id: AppGini.current_invivo_code__RAND__.value, t: 'chr_scenes', f: 'invivo_code' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author_id: filterer_author_id, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, s: term, p: page, t: 'chr_scenes', f: 'invivo_code' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#invivo_code-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_invivo_code__RAND__.value, t: 'chr_scenes', f: 'invivo_code' },
					success: function(resp){
						$j('[name="invivo_code"]').val(resp.results[0].id);
						$j('[id=invivo_code-container-readonly__RAND__]').html('<span id="invivo_code-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=code_invivo_view_parent]').hide(); }else{ $j('.btn[id=code_invivo_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#startdate-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						...
						<?php if(!$selected_id && !$filterer_startdate){ ?>
							data: { text: '1', t: 'chr_scenes', f: 'startdate' },
						<?php }else{ ?>
							data: { filterer_author_id: filterer_author_id, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, id: AppGini.current_startdate__RAND__.value, t: 'chr_scenes', f: 'startdate' },
						<?php } ?>
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author_id: filterer_author_id, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, s: term, p: page, t: 'chr_scenes', f: 'startdate' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#startdate-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_startdate__RAND__.value, t: 'chr_scenes', f: 'startdate' },
					success: function(resp){
						$j('[name="startdate"]').val(resp.results[0].id);
						$j('[id=startdate-container-readonly__RAND__]').html('<span id="startdate-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=code_invivo_view_parent]').hide(); }else{ $j('.btn[id=code_invivo_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#herme_code-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author_id: filterer_author_id, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, id: AppGini.current_herme_code__RAND__.value, t: 'chr_scenes', f: 'herme_code' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_author_id: filterer_author_id, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, filterer_token_sequence: filterer_token_sequence, s: term, p: page, t: 'chr_scenes', f: 'herme_code' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#herme_code-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_herme_code__RAND__.value, t: 'chr_scenes', f: 'herme_code' },
					success: function(resp){
						$j('[name="herme_code"]').val(resp.results[0].id);
						$j('[id=herme_code-container-readonly__RAND__]').html('<span id="herme_code-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=code_herme_view_parent]').hide(); }else{ $j('.btn[id=code_herme_view_parent]').show(); }
			...
			$j("#chr_element-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_chr_element__RAND__.value, t: 'chr_scenes', f: 'chr_element' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'chr_scenes', f: 'chr_element' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/chr_scenes_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/chr_scenes_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return chr_scenes_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return chr_scenes_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return chr_scenes_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$lookup_fields = array(  'author_id' => array('biblio_author', 'Author'), 'bibliography' => array('biblio_doc', 'Bibliography'), 'transcript' => array('biblio_transcript', 'Transcript'), 'token' => array('biblio_token', 'Token'), 'invivo_code' => array('code_invivo', 'Invivo_code'), 'startdate' => array('code_invivo', 'Startdate'), 'herme_code' => array('code_herme', 'Herme_code'), 'chr_element' => array('class_character_element', 'Chr element'));
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'chr_scenes_autofill.php?rnd1=$rnd1&mfk=author_id&id=' + encodeURIComponent('".addslashes($row['author_id'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'chr_scenes_autofill.php?rnd1=$rnd1&mfk=author_id&id=' + encodeURIComponent(AppGini.current_author_id{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'chr_scenes_autofill.php?rnd1=$rnd1&mfk=token&id=' + encodeURIComponent('".addslashes($row['token'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'chr_scenes_autofill.php?rnd1=$rnd1&mfk=token&id=' + encodeURIComponent(AppGini.current_token{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'chr_scenes_autofill.php?rnd1=$rnd1&mfk=invivo_code&id=' + encodeURIComponent('".addslashes($row['invivo_code'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'chr_scenes_autofill.php?rnd1=$rnd1&mfk=invivo_code&id=' + encodeURIComponent(AppGini.current_invivo_code{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'chr_scenes_autofill.php?rnd1=$rnd1&mfk=herme_code&id=' + encodeURIComponent('".addslashes($row['herme_code'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'chr_scenes_autofill.php?rnd1=$rnd1&mfk=herme_code&id=' + encodeURIComponent(AppGini.current_herme_code{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$rdata = $jdata = get_defaults('chr_scenes');
	if($selected_id){
		$jdata = get_joined_record('chr_scenes', $selected_id);
		if($jdata === false) $jdata = get_defaults('chr_scenes');
		$rdata = $row;
	}
	$templateCode .= loadView('chr_scenes-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: chr_scenes_dv
	if(function_exists('chr_scenes_dv')){
		$args=array();
		chr_scenes_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
