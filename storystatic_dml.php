...
// Data functions (insert, update, delete, form) for table storystatic

// This script and data application were generated by AppGini 5.76
// Download AppGini for free from https://bigprof.com/appgini/download/

function storystatic_insert(){
	global $Translation;

	// mm: can member insert record?
	$arrPerm=getTablePermissions('storystatic');
	if(!$arrPerm[1]){
		return false;
	}
	...
	$data['story_character_mc'] = makeSafe($_REQUEST['story_character_mc']);
		if($data['story_character_mc'] == empty_lookup_value){ $data['story_character_mc'] = ''; }
	...
	$data['signpost1'] = makeSafe($_REQUEST['signpost1']);
		if($data['signpost1'] == empty_lookup_value){ $data['signpost1'] = ''; }
	$data['signpost2'] = makeSafe($_REQUEST['signpost2']);
		if($data['signpost2'] == empty_lookup_value){ $data['signpost2'] = ''; }
	$data['signpost3'] = makeSafe($_REQUEST['signpost3']);
		if($data['signpost3'] == empty_lookup_value){ $data['signpost3'] = ''; }
	$data['signpost4'] = makeSafe($_REQUEST['signpost4']);
		if($data['signpost4'] == empty_lookup_value){ $data['signpost4'] = ''; }

	// hook: storystatic_before_insert
	if(function_exists('storystatic_before_insert')){
		$args=array();
		if(!storystatic_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `storystatic` set       `story`=' . (($data['story'] !== '' && $data['story'] !== NULL) ? "'{$data['story']}'" : 'NULL') . ', `throughline`=' . (($data['throughline'] !== '' && $data['throughline'] !== NULL) ? "'{$data['throughline']}'" : 'NULL') . ', `story_character_mc`=' . (($data['story_character_mc'] !== '' && $data['story_character_mc'] !== NULL) ? "'{$data['story_character_mc']}'" : 'NULL') . ', `throughline_domain`=' . (($data['throughline_domain'] !== '' && $data['throughline_domain'] !== NULL) ? "'{$data['throughline_domain']}'" : 'NULL') . ', `concern`=' . (($data['concern'] !== '' && $data['concern'] !== NULL) ? "'{$data['concern']}'" : 'NULL') . ', `issue`=' . (($data['issue'] !== '' && $data['issue'] !== NULL) ? "'{$data['issue']}'" : 'NULL') . ', `problem`=' . (($data['problem'] !== '' && $data['problem'] !== NULL) ? "'{$data['problem']}'" : 'NULL') . ', `solution`=' . (($data['solution'] !== '' && $data['solution'] !== NULL) ? "'{$data['solution']}'" : 'NULL') . ', `symptom`=' . (($data['symptom'] !== '' && $data['symptom'] !== NULL) ? "'{$data['symptom']}'" : 'NULL') . ', `response`=' . (($data['response'] !== '' && $data['response'] !== NULL) ? "'{$data['response']}'" : 'NULL') . ', `catalyst`=' . (($data['catalyst'] !== '' && $data['catalyst'] !== NULL) ? "'{$data['catalyst']}'" : 'NULL') . ', `inhibitor`=' . (($data['inhibitor'] !== '' && $data['inhibitor'] !== NULL) ? "'{$data['inhibitor']}'" : 'NULL') . ', `benchmark`=' . (($data['benchmark'] !== '' && $data['benchmark'] !== NULL) ? "'{$data['benchmark']}'" : 'NULL') . ', `signpost1`=' . (($data['signpost1'] !== '' && $data['signpost1'] !== NULL) ? "'{$data['signpost1']}'" : 'NULL') . ', `signpost2`=' . (($data['signpost2'] !== '' && $data['signpost2'] !== NULL) ? "'{$data['signpost2']}'" : 'NULL') . ', `signpost3`=' . (($data['signpost3'] !== '' && $data['signpost3'] !== NULL) ? "'{$data['signpost3']}'" : 'NULL') . ', `signpost4`=' . (($data['signpost4'] !== '' && $data['signpost4'] !== NULL) ? "'{$data['signpost4']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"storystatic_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}

	$recID = db_insert_id(db_link());

	// hook: storystatic_after_insert
	if(function_exists('storystatic_after_insert')){
		$res = sql("select * from `storystatic` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!storystatic_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('storystatic', $recID, getLoggedMemberID());

	return $recID;
}

function storystatic_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('storystatic');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='storystatic' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='storystatic' and pkValue='$selected_id'");
	...
	// hook: storystatic_before_delete
	if(function_exists('storystatic_before_delete')){
		$args=array();
		if(!storystatic_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	// child table: bio_drama_chr_dev
	$res = sql("select `id` from `storystatic` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `bio_drama_chr_dev` where `mcs_problem`='".addslashes($id[0])."'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		...
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "bio_drama_chr_dev", $RetMsg);
		$RetMsg = str_replace("<Delete>", "<input type=\"button\" class=\"button\" value=\"".$Translation['yes']."\" onClick=\"window.location='storystatic_view.php?SelectedID=".urlencode($selected_id)."&delete_x=1&confirmed=1';\">", $RetMsg);
		$RetMsg = str_replace("<Cancel>", "<input type=\"button\" class=\"button\" value=\"".$Translation['no']."\" onClick=\"window.location='storystatic_view.php?SelectedID=".urlencode($selected_id)."';\">", $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("select `id` from `storystatic` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `storydynamic` where `storystatic_ost`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
	...
	sql("delete from `storystatic` where `id`='$selected_id'", $eo);

	// hook: storystatic_after_delete
	if(function_exists('storystatic_after_delete')){
		$args=array();
		storystatic_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='storystatic' and pkValue='$selected_id'", $eo);
}

function storystatic_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('storystatic');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='storystatic' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='storystatic' and pkValue='".makeSafe($selected_id)."'");
	...
	// hook: storystatic_before_update
	if(function_exists('storystatic_before_update')){
		$args=array();
		if(!storystatic_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `storystatic` set       `story`=' . (($data['story'] !== '' && $data['story'] !== NULL) ? "'{$data['story']}'" : 'NULL') . ', `throughline`=' . (($data['throughline'] !== '' && $data['throughline'] !== NULL) ? "'{$data['throughline']}'" : 'NULL') . ', `story_character_mc`=' . (($data['story_character_mc'] !== '' && $data['story_character_mc'] !== NULL) ? "'{$data['story_character_mc']}'" : 'NULL') . ', `throughline_domain`=' . (($data['throughline_domain'] !== '' && $data['throughline_domain'] !== NULL) ? "'{$data['throughline_domain']}'" : 'NULL') . ', `concern`=' . (($data['concern'] !== '' && $data['concern'] !== NULL) ? "'{$data['concern']}'" : 'NULL') . ', `issue`=' . (($data['issue'] !== '' && $data['issue'] !== NULL) ? "'{$data['issue']}'" : 'NULL') . ', `problem`=' . (($data['problem'] !== '' && $data['problem'] !== NULL) ? "'{$data['problem']}'" : 'NULL') . ', `solution`=' . (($data['solution'] !== '' && $data['solution'] !== NULL) ? "'{$data['solution']}'" : 'NULL') . ', `symptom`=' . (($data['symptom'] !== '' && $data['symptom'] !== NULL) ? "'{$data['symptom']}'" : 'NULL') . ', `response`=' . (($data['response'] !== '' && $data['response'] !== NULL) ? "'{$data['response']}'" : 'NULL') . ', `catalyst`=' . (($data['catalyst'] !== '' && $data['catalyst'] !== NULL) ? "'{$data['catalyst']}'" : 'NULL') . ', `inhibitor`=' . (($data['inhibitor'] !== '' && $data['inhibitor'] !== NULL) ? "'{$data['inhibitor']}'" : 'NULL') . ', `benchmark`=' . (($data['benchmark'] !== '' && $data['benchmark'] !== NULL) ? "'{$data['benchmark']}'" : 'NULL') . ', `signpost1`=' . (($data['signpost1'] !== '' && $data['signpost1'] !== NULL) ? "'{$data['signpost1']}'" : 'NULL') . ', `signpost2`=' . (($data['signpost2'] !== '' && $data['signpost2'] !== NULL) ? "'{$data['signpost2']}'" : 'NULL') . ', `signpost3`=' . (($data['signpost3'] !== '' && $data['signpost3'] !== NULL) ? "'{$data['signpost3']}'" : 'NULL') . ', `signpost4`=' . (($data['signpost4'] !== '' && $data['signpost4'] !== NULL) ? "'{$data['signpost4']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="storystatic_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: storystatic_after_update
	if(function_exists('storystatic_after_update')){
		$res = sql("SELECT * FROM `storystatic` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!storystatic_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='storystatic' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function storystatic_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	$filterer_story_character_mc = thisOr(undo_magic_quotes($_REQUEST['filterer_story_character_mc']), '');
	...
	$filterer_signpost1 = thisOr(undo_magic_quotes($_REQUEST['filterer_signpost1']), '');
	$filterer_signpost2 = thisOr(undo_magic_quotes($_REQUEST['filterer_signpost2']), '');
	$filterer_signpost3 = thisOr(undo_magic_quotes($_REQUEST['filterer_signpost3']), '');
	$filterer_signpost4 = thisOr(undo_magic_quotes($_REQUEST['filterer_signpost4']), '');

	// populate filterers, starting from children to grand-parents
	if($filterer_story_character_mc && !$filterer_story) $filterer_story = sqlValue("select story from story_chrs where id='" . makeSafe($filterer_story_character_mc) . "'");
	...
	if($selected_id){
		...
		$res = sql("select * from `storystatic` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'storystatic_view.php', false);
		}
			...
			$j("#story-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_story__RAND__.value, t: 'storystatic', f: 'story' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'storystatic', f: 'story' }; },
			...
			$j("#throughline-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_throughline__RAND__.value, t: 'storystatic', f: 'throughline' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'storystatic', f: 'throughline' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#story_character_mc-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_story: filterer_story, id: AppGini.current_story_character_mc__RAND__.value, t: 'storystatic', f: 'story_character_mc' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_story: filterer_story, s: term, p: page, t: 'storystatic', f: 'story_character_mc' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#story_character_mc-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_story_character_mc__RAND__.value, t: 'storystatic', f: 'story_character_mc' },
					success: function(resp){
						$j('[name="story_character_mc"]').val(resp.results[0].id);
						$j('[id=story_character_mc-container-readonly__RAND__]').html('<span id="story_character_mc-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=story_chrs_view_parent]').hide(); }else{ $j('.btn[id=story_chrs_view_parent]').show(); }
			...
			$j("#throughline_domain-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_throughline_domain__RAND__.value, t: 'storystatic', f: 'throughline_domain' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'storystatic', f: 'throughline_domain' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#concern-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_throughline_domain: filterer_throughline_domain, id: AppGini.current_concern__RAND__.value, t: 'storystatic', f: 'concern' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_throughline_domain: filterer_throughline_domain, s: term, p: page, t: 'storystatic', f: 'concern' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#concern-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_concern__RAND__.value, t: 'storystatic', f: 'concern' },
					success: function(resp){
						$j('[name="concern"]').val(resp.results[0].id);
						$j('[id=concern-container-readonly__RAND__]').html('<span id="concern-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=class_dramatica_concern_view_parent]').hide(); }else{ $j('.btn[id=class_dramatica_concern_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#issue-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_throughline_domain: filterer_throughline_domain, filterer_concern: filterer_concern, id: AppGini.current_issue__RAND__.value, t: 'storystatic', f: 'issue' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_throughline_domain: filterer_throughline_domain, filterer_concern: filterer_concern, s: term, p: page, t: 'storystatic', f: 'issue' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#issue-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_issue__RAND__.value, t: 'storystatic', f: 'issue' },
					success: function(resp){
						$j('[name="issue"]').val(resp.results[0].id);
						$j('[id=issue-container-readonly__RAND__]').html('<span id="issue-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=class_dramatica_issue_view_parent]').hide(); }else{ $j('.btn[id=class_dramatica_issue_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#problem-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_throughline_domain: filterer_throughline_domain, filterer_concern: filterer_concern, filterer_issue: filterer_issue, id: AppGini.current_problem__RAND__.value, t: 'storystatic', f: 'problem' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_throughline_domain: filterer_throughline_domain, filterer_concern: filterer_concern, filterer_issue: filterer_issue, s: term, p: page, t: 'storystatic', f: 'problem' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#problem-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_problem__RAND__.value, t: 'storystatic', f: 'problem' },
					success: function(resp){
						$j('[name="problem"]').val(resp.results[0].id);
						$j('[id=problem-container-readonly__RAND__]').html('<span id="problem-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=class_dramatica_themes_view_parent]').hide(); }else{ $j('.btn[id=class_dramatica_themes_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#solution-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_throughline_domain: filterer_throughline_domain, filterer_concern: filterer_concern, filterer_issue: filterer_issue, id: AppGini.current_solution__RAND__.value, t: 'storystatic', f: 'solution' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_throughline_domain: filterer_throughline_domain, filterer_concern: filterer_concern, filterer_issue: filterer_issue, s: term, p: page, t: 'storystatic', f: 'solution' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#solution-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_solution__RAND__.value, t: 'storystatic', f: 'solution' },
					success: function(resp){
						$j('[name="solution"]').val(resp.results[0].id);
						$j('[id=solution-container-readonly__RAND__]').html('<span id="solution-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=class_dramatica_themes_view_parent]').hide(); }else{ $j('.btn[id=class_dramatica_themes_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#symptom-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_throughline_domain: filterer_throughline_domain, filterer_concern: filterer_concern, id: AppGini.current_symptom__RAND__.value, t: 'storystatic', f: 'symptom' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_throughline_domain: filterer_throughline_domain, filterer_concern: filterer_concern, s: term, p: page, t: 'storystatic', f: 'symptom' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#symptom-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_symptom__RAND__.value, t: 'storystatic', f: 'symptom' },
					success: function(resp){
						$j('[name="symptom"]').val(resp.results[0].id);
						$j('[id=symptom-container-readonly__RAND__]').html('<span id="symptom-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=class_dramatica_themes_view_parent]').hide(); }else{ $j('.btn[id=class_dramatica_themes_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#response-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_throughline_domain: filterer_throughline_domain, filterer_concern: filterer_concern, id: AppGini.current_response__RAND__.value, t: 'storystatic', f: 'response' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_throughline_domain: filterer_throughline_domain, filterer_concern: filterer_concern, s: term, p: page, t: 'storystatic', f: 'response' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#response-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_response__RAND__.value, t: 'storystatic', f: 'response' },
					success: function(resp){
						$j('[name="response"]').val(resp.results[0].id);
						$j('[id=response-container-readonly__RAND__]').html('<span id="response-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=class_dramatica_themes_view_parent]').hide(); }else{ $j('.btn[id=class_dramatica_themes_view_parent]').show(); }
			...
			$j("#catalyst-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_catalyst__RAND__.value, t: 'storystatic', f: 'catalyst' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'storystatic', f: 'catalyst' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			$j("#inhibitor-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_inhibitor__RAND__.value, t: 'storystatic', f: 'inhibitor' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'storystatic', f: 'inhibitor' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			$j("#benchmark-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_benchmark__RAND__.value, t: 'storystatic', f: 'benchmark' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'storystatic', f: 'benchmark' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			$j("#signpost1-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_signpost1__RAND__.value, t: 'storystatic', f: 'signpost1' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'storystatic', f: 'signpost1' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			$j("#signpost2-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_signpost2__RAND__.value, t: 'storystatic', f: 'signpost2' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'storystatic', f: 'signpost2' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			$j("#signpost3-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_signpost3__RAND__.value, t: 'storystatic', f: 'signpost3' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'storystatic', f: 'signpost3' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			$j("#signpost4-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_signpost4__RAND__.value, t: 'storystatic', f: 'signpost4' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'storystatic', f: 'signpost4' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/storystatic_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/storystatic_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return storystatic_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return storystatic_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return storystatic_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$rdata = $jdata = get_defaults('storystatic');
	if($selected_id){
		$jdata = get_joined_record('storystatic', $selected_id);
		if($jdata === false) $jdata = get_defaults('storystatic');
		$rdata = $row;
	}
	$templateCode .= loadView('storystatic-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: storystatic_dv
	if(function_exists('storystatic_dv')){
		$args=array();
		storystatic_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
