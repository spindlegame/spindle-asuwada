...
<script>
	$j(function() {
		...
		var data = {
			...
			counterfactual: <?php echo json_encode($jdata['counterfactual']); ?>,
			goal: <?php echo json_encode($jdata['goal']); ?>,
			dilemma_ethics: <?php echo json_encode($jdata['dilemma_ethics']); ?>,
			sdg: <?php echo json_encode($jdata['sdg']); ?>,
		...
		cache.addCheck(function(u, d) {
			...
			if(d.mfk == 'herme_code' && d.id == data.herme_code.id) {
				...
				$j('#counterfactual' + d[rnd]).html(data.counterfactual);
				$j('#goal' + d[rnd]).html(data.goal);
				$j('#dilemma_ethics' + d[rnd]).html(data.dilemma_ethics);
				$j('#sdg' + d[rnd]).html(data.sdg);
				return true;
			}
