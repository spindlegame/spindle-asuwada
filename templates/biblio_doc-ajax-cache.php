...
<script>
	$j(function() {
		...
		var data = {
			...
			medium: <?php echo json_encode(array('id' => $rdata['medium'], 'value' => $rdata['medium'], 'text' => $jdata['medium'])); ?>,
		...
		/* saved value for medium */
		cache.addCheck(function(u, d) {
			if(u != 'ajax_combo.php') return false;
			if(d.t == tn && d.f == 'medium' && d.id == data.medium.id)
				return { results: [ data.medium ], more: false, elapsed: 0.01 };
			return false;
		});
