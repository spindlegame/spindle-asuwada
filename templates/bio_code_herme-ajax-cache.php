...
<script>
	$j(function() {
		...
		var data = {
			...
			bio_sdg: <?php echo json_encode(array('id' => $rdata['bio_sdg'], 'value' => $rdata['bio_sdg'], 'text' => $jdata['bio_sdg'])); ?>,
			bio_dilemma: <?php echo json_encode(array('id' => $rdata['bio_dilemma'], 'value' => $rdata['bio_dilemma'], 'text' => $jdata['bio_dilemma'])); ?>,
			bio_goals: <?php echo json_encode(array('id' => $rdata['bio_goals'], 'value' => $rdata['bio_goals'], 'text' => $jdata['bio_goals'])); ?>
		};
		...
		/* saved value for bio_sdg */
		cache.addCheck(function(u, d) {
			if(u != 'ajax_combo.php') return false;
			if(d.t == tn && d.f == 'bio_sdg' && d.id == data.bio_sdg.id)
				return { results: [ data.bio_sdg ], more: false, elapsed: 0.01 };
			return false;
		});

		/* saved value for bio_dilemma */
		cache.addCheck(function(u, d) {
			if(u != 'ajax_combo.php') return false;
			if(d.t == tn && d.f == 'bio_dilemma' && d.id == data.bio_dilemma.id)
				return { results: [ data.bio_dilemma ], more: false, elapsed: 0.01 };
			return false;
		});

		/* saved value for bio_goals */
		cache.addCheck(function(u, d) {
			if(u != 'ajax_combo.php') return false;
			if(d.t == tn && d.f == 'bio_goals' && d.id == data.bio_goals.id)
				return { results: [ data.bio_goals ], more: false, elapsed: 0.01 };
			return false;
		});
