...
<script>
	$j(function() {
		...
		var data = {
			...
			startdate: <?php echo json_encode($jdata['startdate']); ?>,
		...
		/* saved value for character_scene autofills */
		cache.addCheck(function(u, d) {
			if(u != tn + '_autofill.php') return false;

			for(var rnd in d) if(rnd.match(/^rnd/)) break;

			if(d.mfk == 'character_scene' && d.id == data.character_scene.id) {
				$j('#startdate' + d[rnd]).html(data.startdate);
