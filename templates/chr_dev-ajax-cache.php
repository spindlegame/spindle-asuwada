...
<script>
	$j(function(){
		...
		var data = {
			...
			dp_resolve: <?php echo json_encode(array('id' => $rdata['dp_resolve'], 'value' => $rdata['dp_resolve'], 'text' => $jdata['dp_resolve'])); ?>,
		...
		/* saved value for dp_resolve */
		cache.addCheck(function(u, d){
			if(u != 'ajax_combo.php') return false;
			if(d.t == tn && d.f == 'dp_resolve' && d.id == data.dp_resolve.id)
				return { results: [ data.dp_resolve ], more: false, elapsed: 0.01 };
			return false;
		});
