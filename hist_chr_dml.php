...
function hist_chr_insert(&$error_message = '') {
	...
	if($_REQUEST['filterer_hist_lead_memberid']) {
		sql("UPDATE `hist_chr` SET `hist_lead_memberid`='" . makeSafe($_REQUEST['filterer_hist_lead_memberid']) . "' WHERE `id`='" . makeSafe($recID, false) . "'", $eo);
	}
...
function hist_chr_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	...
	$res = sql("SELECT `id` FROM `hist_chr` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_chr_scene` WHERE `character`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
...
function hist_chr_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#hist_lead_id-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_team: filterer_team, id: AppGini.current_hist_lead_id__RAND__.value, t: 'hist_chr', f: 'hist_lead_id' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_team: filterer_team, s: term, p: page, t: 'hist_chr', f: 'hist_lead_id' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#hist_lead_id-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_hist_lead_id__RAND__.value, t: 'hist_chr', f: 'hist_lead_id' },
					success: function(resp) {
						$j('[name="hist_lead_id"]').val(resp.results[0].id);
						$j('[id=hist_lead_id-container-readonly__RAND__]').html('<span id="hist_lead_id-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=hist_author_view_parent]').hide(); } else { $j('.btn[id=hist_author_view_parent]').show(); }
	...
	$lookup_fields = array('team' => array('hist_team', 'Team'), 'hist_lead_id' => array('hist_author', 'Project lead id'), 'hist_story' => array('hist_story', 'Story'), 'agent_id' => array('game_agent', 'Agent id'), 'bio_story' => array('bio_story', 'Bio story'), 'story_character' => array('class_dramatica_character', 'Story character'), 'story_archetype' => array('class_dramatica_archetype', 'Story Archetype'), );
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'hist_chr_autofill.php?rnd1=$rnd1&mfk=hist_lead_id&id=' + encodeURIComponent('".addslashes($row['hist_lead_id'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'hist_chr_autofill.php?rnd1=$rnd1&mfk=hist_lead_id&id=' + encodeURIComponent(AppGini.current_hist_lead_id{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
