...
function code_invivo_insert(){
	...
	$data['invivo'] = makeSafe($_REQUEST['invivo']);
		if($data['invivo'] == empty_lookup_value){ $data['invivo'] = ''; }
	$data['start_date'] = mysql_datetime($_REQUEST['start_date']);
		if($data['start_date'] == empty_lookup_value){ $data['start_date'] = ''; }
	$data['end_date'] = mysql_datetime($_REQUEST['end_date']);
		if($data['end_date'] == empty_lookup_value){ $data['end_date'] = ''; }
	$data['person'] = makeSafe($_REQUEST['person']);
		if($data['person'] == empty_lookup_value){ $data['person'] = ''; }
	$data['place'] = makeSafe($_REQUEST['place']);
	...
	if(function_exists('code_invivo_after_insert')){
		$res = sql("select * from `code_invivo` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!code_invivo_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('code_invivo', $recID, getLoggedMemberID());
...
function code_invivo_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_scenes` where `invivo_code`='".addslashes($id[0])."'", $eo);
	...
	$res = sql("select `id` from `code_invivo` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `chr_scenes` where `startdate`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
...
function code_invivo_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'code_invivo_view.php', false);
		}
			...
			$j("#author-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_author__RAND__.value, t: 'code_invivo', f: 'author' },
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#bibliography-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, id: AppGini.current_bibliography__RAND__.value, t: 'code_invivo', f: 'bibliography' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
			...
			if(!$j("#bibliography-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bibliography__RAND__.value, t: 'code_invivo', f: 'bibliography' },
					success: function(resp){
						$j('[name="bibliography"]').val(resp.results[0].id);
						$j('[id=bibliography-container-readonly__RAND__]').html('<span id="bibliography-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=biblio_doc_view_parent]').hide(); }else{ $j('.btn[id=biblio_doc_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#transcript-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, id: AppGini.current_transcript__RAND__.value, t: 'code_invivo', f: 'transcript' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
			...
			if(!$j("#transcript-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_transcript__RAND__.value, t: 'code_invivo', f: 'transcript' },
					success: function(resp){
						$j('[name="transcript"]').val(resp.results[0].id);
						$j('[id=transcript-container-readonly__RAND__]').html('<span id="transcript-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=biblio_transcript_view_parent]').hide(); }else{ $j('.btn[id=biblio_transcript_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#token_sequence-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_author: filterer_author, filterer_bibliography: filterer_bibliography, filterer_transcript: filterer_transcript, id: AppGini.current_token_sequence__RAND__.value, t: 'code_invivo', f: 'token_sequence' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
			...
			if(!$j("#token_sequence-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_token_sequence__RAND__.value, t: 'code_invivo', f: 'token_sequence' },
					success: function(resp){
						$j('[name="token_sequence"]').val(resp.results[0].id);
						$j('[id=token_sequence-container-readonly__RAND__]').html('<span id="token_sequence-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=biblio_token_view_parent]').hide(); }else{ $j('.btn[id=biblio_token_view_parent]').show(); }
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/code_invivo_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/code_invivo_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return code_invivo_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'code_invivo_autofill.php?rnd1=$rnd1&mfk=token_sequence&id=' + encodeURIComponent('".addslashes($row['token_sequence'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'code_invivo_autofill.php?rnd1=$rnd1&mfk=token_sequence&id=' + encodeURIComponent(AppGini.current_token_sequence{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$rdata = $jdata = get_defaults('code_invivo');
	if($selected_id){
		$jdata = get_joined_record('code_invivo', $selected_id);
		if($jdata === false) $jdata = get_defaults('code_invivo');
		$rdata = $row;
	}
	$templateCode .= loadView('code_invivo-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
