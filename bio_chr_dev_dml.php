...
// Data functions (insert, update, delete, form) for table bio_chr_dev

// This script and data application were generated by AppGini 5.94
// Download AppGini for free from https://bigprof.com/appgini/download/

function bio_chr_dev_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('bio_chr_dev');
	...
	$data = [
		...
		'locations' => Request::multipleChoice('locations', ''),
		'persons' => Request::multipleChoice('persons', ''),
	...
	// hook: bio_chr_dev_before_insert
	if(function_exists('bio_chr_dev_before_insert')) {
		$args = [];
		if(!bio_chr_dev_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('bio_chr_dev', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('bio_chr_dev', $recID, calculated_fields()['bio_chr_dev']);

	// hook: bio_chr_dev_after_insert
	if(function_exists('bio_chr_dev_after_insert')) {
		$res = sql("SELECT * FROM `bio_chr_dev` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!bio_chr_dev_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('bio_chr_dev', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) bio_chr_dev_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function bio_chr_dev_copy_children($destination_id, $source_id) {
	global $Translation;
...
function bio_chr_dev_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('bio_chr_dev', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: bio_chr_dev_before_delete
	if(function_exists('bio_chr_dev_before_delete')) {
		$args = [];
		if(!bio_chr_dev_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	$res = sql("SELECT `id` FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		...
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
		$RetMsg = str_replace('<TableName>', 'bio_storydynamic', $RetMsg);
		$RetMsg = str_replace('<Delete>', '<input type="button" class="button" value="' . $Translation['yes'] . '" onClick="window.location = \'bio_chr_dev_view.php?SelectedID=' . urlencode($selected_id) . '&delete_x=1&confirmed=1\';">', $RetMsg);
		$RetMsg = str_replace('<Cancel>', '<input type="button" class="button" value="' . $Translation[ 'no'] . '" onClick="window.location = \'bio_chr_dev_view.php?SelectedID=' . urlencode($selected_id) . '\';">', $RetMsg);
		return $RetMsg;
	}
	...
	$res = sql("SELECT `id` FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_storydynamic` WHERE `mc_growth`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_storydynamic` WHERE `mc_approach`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_storydynamic` WHERE `mc_ps_style`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `bio_storydynamic` WHERE `ic_resolve`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storydynamic` WHERE `mc_resolve`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storydynamic` WHERE `mc_growth`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storydynamic` WHERE `mc_approach`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storydynamic` WHERE `mc_ps_style`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	$res = sql("SELECT `id` FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("SELECT COUNT(1) FROM `hist_storydynamic` WHERE `ic_resolve`='" . makeSafe($id[0]) . "'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks) {
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace('<RelatedRecords>', $rirow[0], $RetMsg);
	...
	sql("DELETE FROM `bio_chr_dev` WHERE `id`='{$selected_id}'", $eo);

	// hook: bio_chr_dev_after_delete
	if(function_exists('bio_chr_dev_after_delete')) {
		$args = [];
		bio_chr_dev_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='bio_chr_dev' AND `pkValue`='{$selected_id}'", $eo);
}

function bio_chr_dev_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('bio_chr_dev', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('bio_chr_dev', $selected_id);
	...
	// hook: bio_chr_dev_before_update
	if(function_exists('bio_chr_dev_before_update')) {
		$args = ['old_data' => $old_data];
		if(!bio_chr_dev_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="bio_chr_dev_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('bio_chr_dev', $data['selectedID'], calculated_fields()['bio_chr_dev']);

	// hook: bio_chr_dev_after_update
	if(function_exists('bio_chr_dev_after_update')) {
		$res = sql("SELECT * FROM `bio_chr_dev` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!bio_chr_dev_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='bio_chr_dev' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function bio_chr_dev_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($filterer_bio_story && !$filterer_agent_id) $filterer_agent_id = sqlValue("select  from bio_story where id='" . makeSafe($filterer_bio_story) . "'");
	if($filterer_cw_name && !$filterer_bio_story) $filterer_bio_story = sqlValue("select bio_story from bio_chr where id='" . makeSafe($filterer_cw_name) . "'");
	...
	if($filterer_illust_resolve && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from bio_chr_scene where id='" . makeSafe($filterer_illust_resolve) . "'");
	...
	if($filterer_mc_growth && !$filterer_dp3_growth) $filterer_dp3_growth = sqlValue("select cat3 from class_dynamicstorypoints4 where id='" . makeSafe($filterer_mc_growth) . "'");
	if($filterer_illust_growth && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from bio_chr_scene where id='" . makeSafe($filterer_illust_growth) . "'");
	...
	if($filterer_mc_approach && !$filterer_dp3_approach) $filterer_dp3_approach = sqlValue("select cat3 from class_dynamicstorypoints4 where id='" . makeSafe($filterer_mc_approach) . "'");
	if($filterer_illust_approach && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from bio_chr_scene where id='" . makeSafe($filterer_illust_approach) . "'");
	...
	if($filterer_illust_ps_style && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from bio_chr_scene where id='" . makeSafe($filterer_illust_ps_style) . "'");
	if($filterer_illust_nt && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from bio_chr_scene where id='" . makeSafe($filterer_illust_nt) . "'");
	if($filterer_illust_im && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from bio_chr_scene where id='" . makeSafe($filterer_illust_im) . "'");
	if($filterer_mcs_problem && !$filterer_bio_story) $filterer_bio_story = sqlValue("select story from bio_storystatic where id='" . makeSafe($filterer_mcs_problem) . "'");
	if($filterer_illust_mcs_problem && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from bio_chr_scene where id='" . makeSafe($filterer_illust_mcs_problem) . "'");
	if($filterer_illust_mcs_solution && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from bio_chr_scene where id='" . makeSafe($filterer_illust_mcs_solution) . "'");
	if($filterer_illust_mcs_symptom && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from bio_chr_scene where id='" . makeSafe($filterer_illust_mcs_symptom) . "'");
	if($filterer_illust_mcs_response && !$filterer_agent_id) $filterer_agent_id = sqlValue("select author_id from bio_chr_scene where id='" . makeSafe($filterer_illust_mcs_response) . "'");
	...
	// combobox: cw_name, filterable by: bio_story
	...
	// combobox: mc_growth, filterable by: dp1_resolve,dp2_resolve,dp3_growth
	...
	// combobox: mc_approach, filterable by: dp1_resolve,dp2_resolve,dp3_approach
	$combo_mc_approach = new DataCombo;
	// combobox: illust_approach, filterable by: agent_id
	$combo_illust_approach = new DataCombo;
	// combobox: dp3_psstyle
	$combo_dp3_psstyle = new DataCombo;
	// combobox: mc_ps_style, filterable by: dp1_resolve,dp2_resolve
	$combo_mc_ps_style = new DataCombo;
	// combobox: illust_ps_style, filterable by: agent_id
	$combo_illust_ps_style = new DataCombo;
	// combobox: cw_gender
	$combo_cw_gender = new DataCombo;
	// combobox: locations
	$combo_locations = new Combo;
	$combo_locations->ListType = 3;
	$combo_locations->MultipleSeparator = ', ';
	$combo_locations->ListBoxHeight = 10;
	$combo_locations->RadiosPerLine = 1;
	if(is_file(dirname(__FILE__).'/hooks/bio_chr_dev.locations.csv')) {
		$locations_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/bio_chr_dev.locations.csv')));
		$combo_locations->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions($locations_data)));
		$combo_locations->ListData = $combo_locations->ListItem;
	} else {
		$combo_locations->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions("madrid;;tomelloso")));
		$combo_locations->ListData = $combo_locations->ListItem;
	}
	$combo_locations->SelectName = 'locations';
	// combobox: persons
	$combo_persons = new Combo;
	$combo_persons->ListType = 3;
	$combo_persons->MultipleSeparator = ', ';
	$combo_persons->ListBoxHeight = 10;
	$combo_persons->RadiosPerLine = 1;
	if(is_file(dirname(__FILE__).'/hooks/bio_chr_dev.persons.csv')) {
		$persons_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/bio_chr_dev.persons.csv')));
		$combo_persons->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions($persons_data)));
		$combo_persons->ListData = $combo_persons->ListItem;
	} else {
		$combo_persons->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions("persona;;person;;")));
		$combo_persons->ListData = $combo_persons->ListItem;
	}
	$combo_persons->SelectName = 'persons';
	// combobox: noetictension
	$combo_noetictension = new DataCombo;
	// combobox: illust_nt, filterable by: agent_id
	$combo_illust_nt = new DataCombo;
	// combobox: impression
	$combo_impression = new DataCombo;
	// combobox: illust_im, filterable by: agent_id
	$combo_illust_im = new DataCombo;
	// combobox: mcs_problem, filterable by: bio_story
	$combo_mcs_problem = new DataCombo;
	// combobox: illust_mcs_problem, filterable by: agent_id
	$combo_illust_mcs_problem = new DataCombo;
	// combobox: illust_mcs_solution, filterable by: agent_id
	$combo_illust_mcs_solution = new DataCombo;
	// combobox: illust_mcs_symptom, filterable by: agent_id
	$combo_illust_mcs_symptom = new DataCombo;
	// combobox: illust_mcs_response, filterable by: agent_id
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='bio_chr_dev' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='bio_chr_dev' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `bio_chr_dev` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'bio_chr_dev_view.php', false);
		}
		...
		$combo_locations->SelectedData = $row['locations'];
		$combo_persons->SelectedData = $row['persons'];
	...
	$combo_locations->Render();
	$combo_persons->Render();
	...
	<script>
		...
		jQuery(function() {
			setTimeout(function() {
				...
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(cw_name_reload__RAND__) == \'function\') cw_name_reload__RAND__(AppGini.current_bio_story__RAND__.value);' : ''); ?>
				...
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(mc_growth_reload__RAND__) == \'function\') mc_growth_reload__RAND__(AppGini.current_dp1_resolve__RAND__.value, AppGini.current_dp2_resolve__RAND__.value, AppGini.current_dp3_growth__RAND__.value);' : ''); ?>
				...
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(mc_approach_reload__RAND__) == \'function\') mc_approach_reload__RAND__(AppGini.current_dp1_resolve__RAND__.value, AppGini.current_dp2_resolve__RAND__.value, AppGini.current_dp3_approach__RAND__.value);' : ''); ?>
				...
				if(typeof(dp3_psstyle_reload__RAND__) == 'function') dp3_psstyle_reload__RAND__();
				...
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(illust_ps_style_reload__RAND__) == \'function\') illust_ps_style_reload__RAND__(AppGini.current_agent_id__RAND__.value);' : ''); ?>
				...
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(illust_nt_reload__RAND__) == \'function\') illust_nt_reload__RAND__(AppGini.current_agent_id__RAND__.value);' : ''); ?>
				if(typeof(impression_reload__RAND__) == 'function') impression_reload__RAND__();
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(illust_im_reload__RAND__) == \'function\') illust_im_reload__RAND__(AppGini.current_agent_id__RAND__.value);' : ''); ?>
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(mcs_problem_reload__RAND__) == \'function\') mcs_problem_reload__RAND__(AppGini.current_bio_story__RAND__.value);' : ''); ?>
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(illust_mcs_problem_reload__RAND__) == \'function\') illust_mcs_problem_reload__RAND__(AppGini.current_agent_id__RAND__.value);' : ''); ?>
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(illust_mcs_solution_reload__RAND__) == \'function\') illust_mcs_solution_reload__RAND__(AppGini.current_agent_id__RAND__.value);' : ''); ?>
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(illust_mcs_symptom_reload__RAND__) == \'function\') illust_mcs_symptom_reload__RAND__(AppGini.current_agent_id__RAND__.value);' : ''); ?>
				<?php echo (!$AllowUpdate || $dvprint ? 'if(typeof(illust_mcs_response_reload__RAND__) == \'function\') illust_mcs_response_reload__RAND__(AppGini.current_agent_id__RAND__.value);' : ''); ?>
			...
			$j("#agent_id-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_agent_id__RAND__.value, t: 'bio_chr_dev', f: 'agent_id' },
						...
						if(typeof(illust_ps_style_reload__RAND__) == 'function') illust_ps_style_reload__RAND__(AppGini.current_agent_id__RAND__.value);
						if(typeof(illust_nt_reload__RAND__) == 'function') illust_nt_reload__RAND__(AppGini.current_agent_id__RAND__.value);
						if(typeof(illust_im_reload__RAND__) == 'function') illust_im_reload__RAND__(AppGini.current_agent_id__RAND__.value);
						if(typeof(illust_mcs_problem_reload__RAND__) == 'function') illust_mcs_problem_reload__RAND__(AppGini.current_agent_id__RAND__.value);
						if(typeof(illust_mcs_solution_reload__RAND__) == 'function') illust_mcs_solution_reload__RAND__(AppGini.current_agent_id__RAND__.value);
						if(typeof(illust_mcs_symptom_reload__RAND__) == 'function') illust_mcs_symptom_reload__RAND__(AppGini.current_agent_id__RAND__.value);
						if(typeof(illust_mcs_response_reload__RAND__) == 'function') illust_mcs_response_reload__RAND__(AppGini.current_agent_id__RAND__.value);
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_chr_dev', f: 'agent_id' }; },
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#bio_story-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_bio_story__RAND__.value, t: 'bio_chr_dev', f: 'bio_story' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
						...
						if(typeof(cw_name_reload__RAND__) == 'function') cw_name_reload__RAND__(AppGini.current_bio_story__RAND__.value);
						if(typeof(mcs_problem_reload__RAND__) == 'function') mcs_problem_reload__RAND__(AppGini.current_bio_story__RAND__.value);
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'bio_story' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#bio_story-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_bio_story__RAND__.value, t: 'bio_chr_dev', f: 'bio_story' },
					success: function(resp) {
						$j('[name="bio_story"]').val(resp.results[0].id);
						$j('[id=bio_story-container-readonly__RAND__]').html('<span id="bio_story-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_story_view_parent]').hide(); } else { $j('.btn[id=bio_story_view_parent]').show(); }
		...
		function cw_name_reload__RAND__(filterer_bio_story) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#cw_name-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_bio_story: filterer_bio_story, id: AppGini.current_cw_name__RAND__.value, t: 'bio_chr_dev', f: 'cw_name' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_bio_story: filterer_bio_story, s: term, p: page, t: 'bio_chr_dev', f: 'cw_name' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#cw_name-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_cw_name__RAND__.value, t: 'bio_chr_dev', f: 'cw_name' },
					success: function(resp) {
						$j('[name="cw_name"]').val(resp.results[0].id);
						$j('[id=cw_name-container-readonly__RAND__]').html('<span id="cw_name-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_view_parent]').hide(); } else { $j('.btn[id=bio_chr_view_parent]').show(); }
			...
			$j("#dp1_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_dp1_resolve__RAND__.value, t: 'bio_chr_dev', f: 'dp1_resolve' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_chr_dev', f: 'dp1_resolve' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#dp2_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, id: AppGini.current_dp2_resolve__RAND__.value, t: 'bio_chr_dev', f: 'dp2_resolve' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, s: term, p: page, t: 'bio_chr_dev', f: 'dp2_resolve' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#dp2_resolve-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_dp2_resolve__RAND__.value, t: 'bio_chr_dev', f: 'dp2_resolve' },
					success: function(resp) {
						$j('[name="dp2_resolve"]').val(resp.results[0].id);
						$j('[id=dp2_resolve-container-readonly__RAND__]').html('<span id="dp2_resolve-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints2_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints2_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#dp3_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, id: AppGini.current_dp3_resolve__RAND__.value, t: 'bio_chr_dev', f: 'dp3_resolve' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, s: term, p: page, t: 'bio_chr_dev', f: 'dp3_resolve' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#dp3_resolve-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_dp3_resolve__RAND__.value, t: 'bio_chr_dev', f: 'dp3_resolve' },
					success: function(resp) {
						$j('[name="dp3_resolve"]').val(resp.results[0].id);
						$j('[id=dp3_resolve-container-readonly__RAND__]').html('<span id="dp3_resolve-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints3_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints3_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#mc_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_resolve: filterer_dp3_resolve, id: AppGini.current_mc_resolve__RAND__.value, t: 'bio_chr_dev', f: 'mc_resolve' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_resolve: filterer_dp3_resolve, s: term, p: page, t: 'bio_chr_dev', f: 'mc_resolve' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#mc_resolve-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mc_resolve__RAND__.value, t: 'bio_chr_dev', f: 'mc_resolve' },
					success: function(resp) {
						$j('[name="mc_resolve"]').val(resp.results[0].id);
						$j('[id=mc_resolve-container-readonly__RAND__]').html('<span id="mc_resolve-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints4_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints4_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_resolve-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_resolve__RAND__.value, t: 'bio_chr_dev', f: 'illust_resolve' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'illust_resolve' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_resolve-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_resolve__RAND__.value, t: 'bio_chr_dev', f: 'illust_resolve' },
					success: function(resp) {
						$j('[name="illust_resolve"]').val(resp.results[0].id);
						$j('[id=illust_resolve-container-readonly__RAND__]').html('<span id="illust_resolve-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#dp3_growth-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, id: AppGini.current_dp3_growth__RAND__.value, t: 'bio_chr_dev', f: 'dp3_growth' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
						...
						if(typeof(mc_growth_reload__RAND__) == 'function') mc_growth_reload__RAND__($j('#dp1_resolve').val(), $j('#dp2_resolve').val(), AppGini.current_dp3_growth__RAND__.value);
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, s: term, p: page, t: 'bio_chr_dev', f: 'dp3_growth' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#dp3_growth-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_dp3_growth__RAND__.value, t: 'bio_chr_dev', f: 'dp3_growth' },
					success: function(resp) {
						$j('[name="dp3_growth"]').val(resp.results[0].id);
						$j('[id=dp3_growth-container-readonly__RAND__]').html('<span id="dp3_growth-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints3_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints3_view_parent]').show(); }
		...
		function mc_growth_reload__RAND__(filterer_dp1_resolve, filterer_dp2_resolve, filterer_dp3_growth) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#mc_growth-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_growth: filterer_dp3_growth, id: AppGini.current_mc_growth__RAND__.value, t: 'bio_chr_dev', f: 'mc_growth' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_growth: filterer_dp3_growth, s: term, p: page, t: 'bio_chr_dev', f: 'mc_growth' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#mc_growth-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mc_growth__RAND__.value, t: 'bio_chr_dev', f: 'mc_growth' },
					success: function(resp) {
						$j('[name="mc_growth"]').val(resp.results[0].id);
						$j('[id=mc_growth-container-readonly__RAND__]').html('<span id="mc_growth-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints4_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints4_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_growth-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_growth__RAND__.value, t: 'bio_chr_dev', f: 'illust_growth' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'illust_growth' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_growth-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_growth__RAND__.value, t: 'bio_chr_dev', f: 'illust_growth' },
					success: function(resp) {
						$j('[name="illust_growth"]').val(resp.results[0].id);
						$j('[id=illust_growth-container-readonly__RAND__]').html('<span id="illust_growth-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#dp3_approach-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, id: AppGini.current_dp3_approach__RAND__.value, t: 'bio_chr_dev', f: 'dp3_approach' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
						...
						if(typeof(mc_approach_reload__RAND__) == 'function') mc_approach_reload__RAND__($j('#dp1_resolve').val(), $j('#dp2_resolve').val(), AppGini.current_dp3_approach__RAND__.value);
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, s: term, p: page, t: 'bio_chr_dev', f: 'dp3_approach' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#dp3_approach-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_dp3_approach__RAND__.value, t: 'bio_chr_dev', f: 'dp3_approach' },
					success: function(resp) {
						$j('[name="dp3_approach"]').val(resp.results[0].id);
						$j('[id=dp3_approach-container-readonly__RAND__]').html('<span id="dp3_approach-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints3_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints3_view_parent]').show(); }
		...
		function mc_approach_reload__RAND__(filterer_dp1_resolve, filterer_dp2_resolve, filterer_dp3_approach) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#mc_approach-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_approach: filterer_dp3_approach, id: AppGini.current_mc_approach__RAND__.value, t: 'bio_chr_dev', f: 'mc_approach' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, filterer_dp3_approach: filterer_dp3_approach, s: term, p: page, t: 'bio_chr_dev', f: 'mc_approach' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#mc_approach-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mc_approach__RAND__.value, t: 'bio_chr_dev', f: 'mc_approach' },
					success: function(resp) {
						$j('[name="mc_approach"]').val(resp.results[0].id);
						$j('[id=mc_approach-container-readonly__RAND__]').html('<span id="mc_approach-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints4_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints4_view_parent]').show(); }
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_approach-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_approach__RAND__.value, t: 'bio_chr_dev', f: 'illust_approach' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'illust_approach' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_approach-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_approach__RAND__.value, t: 'bio_chr_dev', f: 'illust_approach' },
					success: function(resp) {
						$j('[name="illust_approach"]').val(resp.results[0].id);
						$j('[id=illust_approach-container-readonly__RAND__]').html('<span id="illust_approach-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		function dp3_psstyle_reload__RAND__() {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#dp3_psstyle-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_dp3_psstyle__RAND__.value, t: 'bio_chr_dev', f: 'dp3_psstyle' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_chr_dev', f: 'dp3_psstyle' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#mc_ps_style-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, id: AppGini.current_mc_ps_style__RAND__.value, t: 'bio_chr_dev', f: 'mc_ps_style' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_dp1_resolve: filterer_dp1_resolve, filterer_dp2_resolve: filterer_dp2_resolve, s: term, p: page, t: 'bio_chr_dev', f: 'mc_ps_style' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#mc_ps_style-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mc_ps_style__RAND__.value, t: 'bio_chr_dev', f: 'mc_ps_style' },
					success: function(resp) {
						$j('[name="mc_ps_style"]').val(resp.results[0].id);
						$j('[id=mc_ps_style-container-readonly__RAND__]').html('<span id="mc_ps_style-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=class_dynamicstorypoints4_view_parent]').hide(); } else { $j('.btn[id=class_dynamicstorypoints4_view_parent]').show(); }
		...
		function illust_ps_style_reload__RAND__(filterer_agent_id) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_ps_style-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_ps_style__RAND__.value, t: 'bio_chr_dev', f: 'illust_ps_style' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'illust_ps_style' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_ps_style-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_ps_style__RAND__.value, t: 'bio_chr_dev', f: 'illust_ps_style' },
					success: function(resp) {
						$j('[name="illust_ps_style"]').val(resp.results[0].id);
						$j('[id=illust_ps_style-container-readonly__RAND__]').html('<span id="illust_ps_style-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
			...
			$j("#cw_gender-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_cw_gender__RAND__.value, t: 'bio_chr_dev', f: 'cw_gender' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_chr_dev', f: 'cw_gender' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#noetictension-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_noetictension__RAND__.value, t: 'bio_chr_dev', f: 'noetictension' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_chr_dev', f: 'noetictension' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		function illust_nt_reload__RAND__(filterer_agent_id) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_nt-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_nt__RAND__.value, t: 'bio_chr_dev', f: 'illust_nt' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'illust_nt' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_nt-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_nt__RAND__.value, t: 'bio_chr_dev', f: 'illust_nt' },
					success: function(resp) {
						$j('[name="illust_nt"]').val(resp.results[0].id);
						$j('[id=illust_nt-container-readonly__RAND__]').html('<span id="illust_nt-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
			...
			$j("#impression-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_impression__RAND__.value, t: 'bio_chr_dev', f: 'impression' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'bio_chr_dev', f: 'impression' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		function illust_im_reload__RAND__(filterer_agent_id) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_im-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_im__RAND__.value, t: 'bio_chr_dev', f: 'illust_im' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'illust_im' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_im-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_im__RAND__.value, t: 'bio_chr_dev', f: 'illust_im' },
					success: function(resp) {
						$j('[name="illust_im"]').val(resp.results[0].id);
						$j('[id=illust_im-container-readonly__RAND__]').html('<span id="illust_im-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		function mcs_problem_reload__RAND__(filterer_bio_story) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#mcs_problem-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_bio_story: filterer_bio_story, id: AppGini.current_mcs_problem__RAND__.value, t: 'bio_chr_dev', f: 'mcs_problem' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_bio_story: filterer_bio_story, s: term, p: page, t: 'bio_chr_dev', f: 'mcs_problem' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#mcs_problem-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_mcs_problem__RAND__.value, t: 'bio_chr_dev', f: 'mcs_problem' },
					success: function(resp) {
						$j('[name="mcs_problem"]').val(resp.results[0].id);
						$j('[id=mcs_problem-container-readonly__RAND__]').html('<span id="mcs_problem-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_storystatic_view_parent]').hide(); } else { $j('.btn[id=bio_storystatic_view_parent]').show(); }
		...
		function illust_mcs_problem_reload__RAND__(filterer_agent_id) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_mcs_problem-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_problem__RAND__.value, t: 'bio_chr_dev', f: 'illust_mcs_problem' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'illust_mcs_problem' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_mcs_problem-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_problem__RAND__.value, t: 'bio_chr_dev', f: 'illust_mcs_problem' },
					success: function(resp) {
						$j('[name="illust_mcs_problem"]').val(resp.results[0].id);
						$j('[id=illust_mcs_problem-container-readonly__RAND__]').html('<span id="illust_mcs_problem-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		function illust_mcs_solution_reload__RAND__(filterer_agent_id) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_mcs_solution-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_solution__RAND__.value, t: 'bio_chr_dev', f: 'illust_mcs_solution' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'illust_mcs_solution' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_mcs_solution-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_solution__RAND__.value, t: 'bio_chr_dev', f: 'illust_mcs_solution' },
					success: function(resp) {
						$j('[name="illust_mcs_solution"]').val(resp.results[0].id);
						$j('[id=illust_mcs_solution-container-readonly__RAND__]').html('<span id="illust_mcs_solution-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		function illust_mcs_symptom_reload__RAND__(filterer_agent_id) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_mcs_symptom-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_symptom__RAND__.value, t: 'bio_chr_dev', f: 'illust_mcs_symptom' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'illust_mcs_symptom' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_mcs_symptom-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_symptom__RAND__.value, t: 'bio_chr_dev', f: 'illust_mcs_symptom' },
					success: function(resp) {
						$j('[name="illust_mcs_symptom"]').val(resp.results[0].id);
						$j('[id=illust_mcs_symptom-container-readonly__RAND__]').html('<span id="illust_mcs_symptom-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
		...
		function illust_mcs_response_reload__RAND__(filterer_agent_id) {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#illust_mcs_response-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_agent_id: filterer_agent_id, id: AppGini.current_illust_mcs_response__RAND__.value, t: 'bio_chr_dev', f: 'illust_mcs_response' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { filterer_agent_id: filterer_agent_id, s: term, p: page, t: 'bio_chr_dev', f: 'illust_mcs_response' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			if(!$j("#illust_mcs_response-container__RAND__").length) {
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_illust_mcs_response__RAND__.value, t: 'bio_chr_dev', f: 'illust_mcs_response' },
					success: function(resp) {
						$j('[name="illust_mcs_response"]').val(resp.results[0].id);
						$j('[id=illust_mcs_response-container-readonly__RAND__]').html('<span id="illust_mcs_response-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>') { $j('.btn[id=bio_chr_scene_view_parent]').hide(); } else { $j('.btn[id=bio_chr_scene_view_parent]').show(); }
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/bio_chr_dev_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/bio_chr_dev_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return bio_chr_dev_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return bio_chr_dev_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return bio_chr_dev_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	if(($selected_id && !$AllowUpdate && !$AllowInsert) || (!$selected_id && !$AllowInsert)) {
		...
		$jsReadOnly .= "\tjQuery('#locations').replaceWith('<div class=\"form-control-static\" id=\"locations\">' + (jQuery('#locations').val() || '') + '</div>'); jQuery('#locations-multi-selection-help').hide();\n";
		$jsReadOnly .= "\tjQuery('#s2id_locations').remove();\n";
		$jsReadOnly .= "\tjQuery('#persons').replaceWith('<div class=\"form-control-static\" id=\"persons\">' + (jQuery('#persons').val() || '') + '</div>'); jQuery('#persons-multi-selection-help').hide();\n";
		$jsReadOnly .= "\tjQuery('#s2id_persons').remove();\n";
	...
	$templateCode = str_replace('<%%COMBO(locations)%%>', $combo_locations->HTML, $templateCode);
	$templateCode = str_replace('<%%COMBOTEXT(locations)%%>', $combo_locations->SelectedData, $templateCode);
	$templateCode = str_replace('<%%COMBO(persons)%%>', $combo_persons->HTML, $templateCode);
	$templateCode = str_replace('<%%COMBOTEXT(persons)%%>', $combo_persons->SelectedData, $templateCode);
	...
	$lookup_fields = array('agent_id' => array('biblio_author', 'ID de agente'), 'bio_story' => array('bio_story', 'Biograf&#237;a'), 'cw_name' => array('bio_chr', 'Nombre de personaje'), 'dp1_resolve' => array('class_dynamicstorypoints1', 'Punto din&#225;mico categor&#237;a 1'), 'dp2_resolve' => array('class_dynamicstorypoints2', 'Punto din&#225;mico categor&#237;a 2'), 'dp3_resolve' => array('class_dynamicstorypoints3', 'Punto din&#225;mico Resoluci&#243;n'), 'mc_resolve' => array('class_dynamicstorypoints4', 'Resoluci&#243;n de personaje principal'), 'illust_resolve' => array('bio_chr_scene', 'Ilustraci&#243;n de la resoluci&#243;n'), 'dp3_growth' => array('class_dynamicstorypoints3', 'Punto din&#225;mico Crecimiento'), 'mc_growth' => array('class_dynamicstorypoints4', 'Crecimiento de personaje principal'), 'illust_growth' => array('bio_chr_scene', 'Ilustraci&#243;n del crecimiento'), 'dp3_approach' => array('class_dynamicstorypoints3', 'Punto din&#225;mico Enfoque'), 'mc_approach' => array('class_dynamicstorypoints4', 'Enfoque de personaje principal'), 'illust_approach' => array('bio_chr_scene', 'Ilustraci&#243;n del enfoque'), 'dp3_psstyle' => array('class_dynamicstorypoints3', 'Punto din&#225;mico estilo de soluci&#243;n de problemas'), 'mc_ps_style' => array('class_dynamicstorypoints4', 'Estilo de resoluci&#243;n de problemas del P.P.'), 'illust_ps_style' => array('bio_chr_scene', 'Ilustraci&#243;n del estilo de resoluci&#243;n de problemas'), 'cw_gender' => array('class_gender', 'G&#233;nero'), 'noetictension' => array('class_nt', 'Tensi&#243;n no&#233;tica'), 'illust_nt' => array('bio_chr_scene', 'Ilustraci&#243;n de la tensi&#243;n no&#233;tica'), 'impression' => array('class_im', 'Gesti&#243;n general de impresiones'), 'illust_im' => array('bio_chr_scene', 'Ilustraci&#243;n de la impresi&#243;n'), 'mcs_problem' => array('bio_storystatic', 'Problema'), 'illust_mcs_problem' => array('bio_chr_scene', 'Ilustraci&#243;n del problema'), 'illust_mcs_solution' => array('bio_chr_scene', 'Ilustraci&#243;n de la soluci&#243;n'), 'illust_mcs_symptom' => array('bio_chr_scene', 'Ilustraci&#243;n del s&#237;ntoma'), 'illust_mcs_response' => array('bio_chr_scene', 'Ilustraci&#243;n de la respuesta'), );
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'bio_chr_dev_autofill.php?rnd1=$rnd1&mfk=agent_id&id=' + encodeURIComponent('".addslashes($row['agent_id'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'bio_chr_dev_autofill.php?rnd1=$rnd1&mfk=agent_id&id=' + encodeURIComponent(AppGini.current_agent_id{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'bio_chr_dev_autofill.php?rnd1=$rnd1&mfk=mcs_problem&id=' + encodeURIComponent('".addslashes($row['mcs_problem'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'bio_chr_dev_autofill.php?rnd1=$rnd1&mfk=mcs_problem&id=' + encodeURIComponent(AppGini.current_mcs_problem{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$rdata = $jdata = get_defaults('bio_chr_dev');
	if($selected_id) {
		$jdata = get_joined_record('bio_chr_dev', $selected_id);
		if($jdata === false) $jdata = get_defaults('bio_chr_dev');
		$rdata = $row;
	}
	$templateCode .= loadView('bio_chr_dev-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: bio_chr_dev_dv
	if(function_exists('bio_chr_dev_dv')) {
		$args=[];
		bio_chr_dev_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
