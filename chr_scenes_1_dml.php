...
// Data functions (insert, update, delete, form) for table chr_scenes_1

// This script and data application were generated by AppGini 5.94
// Download AppGini for free from https://bigprof.com/appgini/download/

function chr_scenes_1_insert(&$error_message = '') {
	global $Translation;

	// mm: can member insert record?
	$arrPerm = getTablePermissions('chr_scenes_1');
	...
	// hook: chr_scenes_1_before_insert
	if(function_exists('chr_scenes_1_before_insert')) {
		$args = [];
		if(!chr_scenes_1_before_insert($data, getMemberInfo(), $args)) {
	...
	insert('chr_scenes_1', backtick_keys_once($data), $error);
	if($error)
		die("{$error}<br><a href=\"#\" onclick=\"history.go(-1);\">{$Translation['< back']}</a>");

	$recID = db_insert_id(db_link());

	update_calc_fields('chr_scenes_1', $recID, calculated_fields()['chr_scenes_1']);

	// hook: chr_scenes_1_after_insert
	if(function_exists('chr_scenes_1_after_insert')) {
		$res = sql("SELECT * FROM `chr_scenes_1` WHERE `id`='" . makeSafe($recID, false) . "' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) {
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=[];
		if(!chr_scenes_1_after_insert($data, getMemberInfo(), $args)) { return $recID; }
	}

	// mm: save ownership data
	set_record_owner('chr_scenes_1', $recID, getLoggedMemberID());

	// if this record is a copy of another record, copy children if applicable
	if(!empty($_REQUEST['SelectedID'])) chr_scenes_1_copy_children($recID, $_REQUEST['SelectedID']);

	return $recID;
}

function chr_scenes_1_copy_children($destination_id, $source_id) {
	global $Translation;
...
function chr_scenes_1_delete($selected_id, $AllowDeleteOfParents = false, $skipChecks = false) {
	// insure referential integrity ...
	global $Translation;
	$selected_id = makeSafe($selected_id);

	// mm: can member delete record?
	if(!check_record_permission('chr_scenes_1', $selected_id, 'delete')) {
		return $Translation['You don\'t have enough permissions to delete this record'];
	}

	// hook: chr_scenes_1_before_delete
	if(function_exists('chr_scenes_1_before_delete')) {
		$args = [];
		if(!chr_scenes_1_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
	...
	sql("DELETE FROM `chr_scenes_1` WHERE `id`='{$selected_id}'", $eo);

	// hook: chr_scenes_1_after_delete
	if(function_exists('chr_scenes_1_after_delete')) {
		$args = [];
		chr_scenes_1_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("DELETE FROM `membership_userrecords` WHERE `tableName`='chr_scenes_1' AND `pkValue`='{$selected_id}'", $eo);
}

function chr_scenes_1_update(&$selected_id, &$error_message = '') {
	global $Translation;

	// mm: can member edit record?
	if(!check_record_permission('chr_scenes_1', $selected_id, 'edit')) return false;
	...
	// get existing values
	$old_data = getRecord('chr_scenes_1', $selected_id);
	...
	// hook: chr_scenes_1_before_update
	if(function_exists('chr_scenes_1_before_update')) {
		$args = ['old_data' => $old_data];
		if(!chr_scenes_1_before_update($data, getMemberInfo(), $args)) {
			if(isset($args['error_message'])) $error_message = $args['error_message'];
			return false;
		}
	}
	...
	if(!update(
		...
		echo $error_message;
		echo '<a href="chr_scenes_1_view.php?SelectedID=' . urlencode($selected_id) . "\">{$Translation['< back']}</a>";
		exit;
	}
	...
	update_calc_fields('chr_scenes_1', $data['selectedID'], calculated_fields()['chr_scenes_1']);

	// hook: chr_scenes_1_after_update
	if(function_exists('chr_scenes_1_after_update')) {
		$res = sql("SELECT * FROM `chr_scenes_1` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)) $data = array_map('makeSafe', $row);

		$data['selectedID'] = $data['id'];
		$args = ['old_data' => $old_data];
		if(!chr_scenes_1_after_update($data, getMemberInfo(), $args)) return;
	}

	// mm: update ownership data
	sql("UPDATE `membership_userrecords` SET `dateUpdated`='" . time() . "' WHERE `tableName`='chr_scenes_1' AND `pkValue`='" . makeSafe($selected_id) . "'", $eo);
}

function chr_scenes_1_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		// mm: who is the owner?
		$ownerGroupID = sqlValue("SELECT `groupID` FROM `membership_userrecords` WHERE `tableName`='chr_scenes_1' AND `pkValue`='" . makeSafe($selected_id) . "'");
		$ownerMemberID = sqlValue("SELECT LCASE(`memberID`) FROM `membership_userrecords` WHERE `tableName`='chr_scenes_1' AND `pkValue`='" . makeSafe($selected_id) . "'");
		...
		$res = sql("SELECT * FROM `chr_scenes_1` WHERE `id`='" . makeSafe($selected_id) . "'", $eo);
		if(!($row = db_fetch_array($res))) {
			return error_message($Translation['No records found'], 'chr_scenes_1_view.php', false);
		}
	...
	<script>
		...
		jQuery(function() {
			setTimeout(function() {
				...
				if(typeof(transcript_reload__RAND__) == 'function') transcript_reload__RAND__();
				if(typeof(token_reload__RAND__) == 'function') token_reload__RAND__();
				if(typeof(invivo_code_reload__RAND__) == 'function') invivo_code_reload__RAND__();
				if(typeof(startdate_reload__RAND__) == 'function') startdate_reload__RAND__();
				if(typeof(herme_code_reload__RAND__) == 'function') herme_code_reload__RAND__();
			...
			$j("#author_id-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_author_id__RAND__.value, t: 'chr_scenes_1', f: 'author_id' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'chr_scenes_1', f: 'author_id' }; },
			...
			$j("#bibliography-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_bibliography__RAND__.value, t: 'chr_scenes_1', f: 'bibliography' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'chr_scenes_1', f: 'bibliography' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		function transcript_reload__RAND__() {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#transcript-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_transcript__RAND__.value, t: 'chr_scenes_1', f: 'transcript' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'chr_scenes_1', f: 'transcript' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#token-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_token__RAND__.value, t: 'chr_scenes_1', f: 'token' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'chr_scenes_1', f: 'token' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		function invivo_code_reload__RAND__() {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#invivo_code-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_invivo_code__RAND__.value, t: 'chr_scenes_1', f: 'invivo_code' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'chr_scenes_1', f: 'invivo_code' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		function startdate_reload__RAND__() {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#startdate-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						...
						<?php if(!$selected_id && !$filterer_startdate) { ?>
							data: { text: '1', t: 'chr_scenes_1', f: 'startdate' },
						<?php } else { ?>
							data: { id: AppGini.current_startdate__RAND__.value, t: 'chr_scenes_1', f: 'startdate' },
						<?php } ?>
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'chr_scenes_1', f: 'startdate' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
		...
		function herme_code_reload__RAND__() {
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint) { ?>
			...
			$j("#herme_code-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_herme_code__RAND__.value, t: 'chr_scenes_1', f: 'herme_code' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'chr_scenes_1', f: 'herme_code' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
			...
			$j("#chr_element-container__RAND__").select2({
				...
				initSelection: function(e, c) {
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_chr_element__RAND__.value, t: 'chr_scenes_1', f: 'chr_element' },
						success: function(resp) {
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page) { return { s: term, p: page, t: 'chr_scenes_1', f: 'chr_element' }; },
					results: function(resp, page) { return resp; }
				},
				escapeMarkup: function(str) { return str; }
			}).on('change', function(e) {
	...
	if($dvprint) {
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/chr_scenes_1_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	} else {
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/chr_scenes_1_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert) {
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return chr_scenes_1_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return chr_scenes_1_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	} else {
	...
	if($selected_id) {
		...
		if($AllowUpdate) {
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return chr_scenes_1_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		} else {
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'chr_scenes_1_autofill.php?rnd1=$rnd1&mfk=author_id&id=' + encodeURIComponent('".addslashes($row['author_id'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'chr_scenes_1_autofill.php?rnd1=$rnd1&mfk=author_id&id=' + encodeURIComponent(AppGini.current_author_id{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'chr_scenes_1_autofill.php?rnd1=$rnd1&mfk=token&id=' + encodeURIComponent('".addslashes($row['token'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'chr_scenes_1_autofill.php?rnd1=$rnd1&mfk=token&id=' + encodeURIComponent(AppGini.current_token{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'chr_scenes_1_autofill.php?rnd1=$rnd1&mfk=invivo_code&id=' + encodeURIComponent('".addslashes($row['invivo_code'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'chr_scenes_1_autofill.php?rnd1=$rnd1&mfk=invivo_code&id=' + encodeURIComponent(AppGini.current_invivo_code{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$templateCode .= "\t\t\$j.ajax({\n";
	if($dvprint) {
		$templateCode .= "\t\t\turl: 'chr_scenes_1_autofill.php?rnd1=$rnd1&mfk=herme_code&id=' + encodeURIComponent('".addslashes($row['herme_code'])."'),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET'\n";
	} else {
		$templateCode .= "\t\t\turl: 'chr_scenes_1_autofill.php?rnd1=$rnd1&mfk=herme_code&id=' + encodeURIComponent(AppGini.current_herme_code{$rnd1}.value),\n";
		$templateCode .= "\t\t\tcontentType: 'application/x-www-form-urlencoded; charset=" . datalist_db_encoding . "',\n";
		$templateCode .= "\t\t\ttype: 'GET',\n";
	...
	$rdata = $jdata = get_defaults('chr_scenes_1');
	if($selected_id) {
		$jdata = get_joined_record('chr_scenes_1', $selected_id);
		if($jdata === false) $jdata = get_defaults('chr_scenes_1');
		$rdata = $row;
	}
	$templateCode .= loadView('chr_scenes_1-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: chr_scenes_1_dv
	if(function_exists('chr_scenes_1_dv')) {
		$args=[];
		chr_scenes_1_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
